...
from spflow.modules.node import MultivariateGaussian as GeneralMultivariateGaussian
...
def updateBackend(leaf_node: MultivariateGaussian, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralMultivariateGaussian(
        scope=leaf_node.scope, mean=leaf_node.mean.data.numpy(), cov=leaf_node.cov.data.numpy()
    )
