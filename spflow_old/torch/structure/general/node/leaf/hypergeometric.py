...
from spflow.structure.general.node.leaf.general_hypergeometric import Hypergeometric as GeneralHypergeometric
...
def updateBackend(leaf_node: Hypergeometric, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralHypergeometric(
        scope=leaf_node.scope, N=leaf_node.N.data.item(), M=leaf_node.M.data.item(), n=leaf_node.n.data.item()
    )
