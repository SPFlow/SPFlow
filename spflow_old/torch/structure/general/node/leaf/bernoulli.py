...
from spflow.base.structure.general.node.leaf.bernoulli import (
    Bernoulli as BaseBernoulli,
)
from spflow.structure.general.node.leaf.general_bernoulli import Bernoulli as GeneralBernoulli
...
class Bernoulli(LeafNode):
    ...
    def p(self, p: float) -> None:
        ...
        self.p_aux.data = proj_bounded_to_real(
            torch.tensor(float(p), dtype=self.dtype, device=self.device), lb=0.0, ub=1.0
        )
