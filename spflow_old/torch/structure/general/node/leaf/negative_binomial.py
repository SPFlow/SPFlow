...
from spflow.modules.node import NegativeBinomial as GeneralNegativeBinomial
...
class NegativeBinomial(LeafNode):
    ...
    def dist(self) -> D.Distribution:
        ...
        return D.NegativeBinomial(
            total_count=self.n, probs=torch.ones(1, dtype=self.dtype, device=self.device) - self.p
        )
...
def updateBackend(leaf_node: NegativeBinomial, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralNegativeBinomial(
        scope=leaf_node.scope, n=leaf_node.n.data.item(), p=leaf_node.p.data.item()
    )
