...
from spflow.modules.layer import BinomialLayer as GeneralBinomialLayer
...
def updateBackend(leaf_node: BinomialLayer, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralBinomialLayer(
        scope=leaf_node.scopes_out, n=leaf_node.n.data.detach().numpy(), p=leaf_node.p.data.detach().numpy()
    )
