...
from spflow.modules.layer import CondNegativeBinomialLayer as GeneralCondNegativeBinomialLayer
...
class CondNegativeBinomialLayer(Module):
    ...
    def set_params(self, n: Union[int, list[int], np.ndarray, torch.Tensor]) -> None:
        ...
        node_scopes = torch.tensor(
            [s.query[0] for s in self.scopes_out], dtype=self.dtype, device=self.device
        )
...
def updateBackend(leaf_node: CondNegativeBinomialLayer, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    return GeneralCondNegativeBinomialLayer(
        scope=leaf_node.scopes_out, n=leaf_node.n.data.detach().numpy(), cond_f=cond_f
    )
