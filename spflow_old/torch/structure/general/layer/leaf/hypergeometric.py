...
from spflow.structure.general.layer.leaf import HypergeometricLayer as GeneralHypergeometricLayer
        ...
        valid[~nan_mask & valid] &= (
            (
                ...
                >= torch.max(
                    torch.vstack(
                        [
                            torch.zeros(scope_data.shape[1], dtype=self.dtype, device=self.device),
...
def updateBackend(leaf_node: HypergeometricLayer, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralHypergeometricLayer(
