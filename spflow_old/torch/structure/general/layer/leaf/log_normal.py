...
from spflow.modules.layer import LogNormalLayer as GeneralLogNormalLayer
...
from spflow.torch.structure.general.node.leaf.log_normal import LogNormal
...
def updateBackend(leaf_node: LogNormalLayer, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralLogNormalLayer(
        scope=leaf_node.scopes_out, mean=leaf_node.mean.detach().numpy(), std=leaf_node.std.detach().numpy()
    )
