...
from spflow.base.structure.general.node.leaf.gamma import Gamma
from spflow.structure.spn.layer.leaf import GammaLayer as GeneralGammaLayer
...
class GammaLayer(Module):
    ...
    def set_params(
        ...
        for node_mean, node_beta, node in zip(alpha, beta, self.nodes):
            ...
            node.set_parameters(node_mean, node_beta)
...
def updateBackend(leaf_node: GammaLayer, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralGammaLayer(scope=leaf_node.scopes_out, alpha=leaf_node.alpha, beta=leaf_node.beta)
