...
params = list(product(out_channels_values, features_values_multiplier, num_splits, split_type, num_repetitions))
...
def test_log_likelihood(out_channels: int, features_values_multiplier: int, num_splits: int, split_type, num_reps, device):
    ...
    for ll in lls:
        if num_reps is not None:
            assert ll.shape == (data.shape[0], module.out_features // num_splits, module.out_channels, num_reps)
        else:
