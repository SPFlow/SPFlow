"""Contract tests for LinsumLayer."""
...
class TestLinsumLayerContract(EinsumLikeContractTests):
    """Shared behavior contract implementation for LinsumLayer."""
    ...
    def make_single_input(self, in_channels: int, out_channels: int, in_features: int, num_reps: int):
        ...
        return LinsumLayer(inputs=inputs, out_channels=out_channels, num_repetitions=num_reps)
    ...
    def make_two_inputs(self, in_channels: int, out_channels: int, in_features: int, num_reps: int):
        ...
        return LinsumLayer(inputs=[left, right], out_channels=out_channels, num_repetitions=num_reps)

    def expected_single_weight_shape(
        self, in_channels: int, out_channels: int, in_features: int, num_reps: int
    ) -> tuple[int, ...]:
        return (in_features // 2, out_channels, num_reps, in_channels)
