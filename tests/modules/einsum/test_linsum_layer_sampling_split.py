...
class TestLinsumLayerSplitConsecutiveOptimization:
    ...
    def test_split_alternate_input_differentiable_sampling(self):
        ...
        split_alt = SplitInterleaved(leaf)
        linsum = LinsumLayer(inputs=split_alt, out_channels=3, num_repetitions=2)

        num_samples = 20
        sample_data = torch.full((num_samples, 4), torch.nan)
        channel_index = torch.randint(0, linsum.out_shape.channels, (num_samples, linsum.out_shape.features))
        repetition_index = torch.randint(0, linsum.out_shape.repetitions, (num_samples,))
        sampling_ctx = SamplingContext(
            channel_index=to_one_hot(channel_index, dim=-1, dim_size=linsum.out_shape.channels),
            repetition_index=to_one_hot(repetition_index, dim=-1, dim_size=linsum.out_shape.repetitions),
            mask=torch.ones((num_samples, linsum.out_shape.features), dtype=torch.bool),
            is_differentiable=True,
        )
