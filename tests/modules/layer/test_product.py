...
from spflow.modules.layer import ProductLayer
import pytest
import torch
...
def test_log_likelihood():
    ...
    assert lls.shape == (data.shape[0], product_layer.event_shape[-1])
...
def test_sample(n_samples=100):
    ...
    for i in range(product_layer.event_shape[-1]):
...
def test_constructor():
    with pytest.raises(ValueError):
        ...
        make_product(inputs=[[make_normal_leaf(scope=0), make_normal_leaf(scope=0)]])
        # input has to be a list of lists
        make_product(inputs=[make_normal_leaf(scope=0), make_normal_leaf(scope=1)])
"""
...
def test_marginalize(prune):
    ...
    marginalized_product_layer = marginalize(product_layer, [1], prune=prune)

    # Scope query should not contain [1]
    assert len(set(marginalized_product_layer.scope.query).intersection([1])) == 0

    marginalized_product_layer2 = marginalize(marginalized_product_layer, [2], prune=prune)

    if prune:
        # If pruning, NormalLayer should be returned
        assert isinstance(marginalized_product_layer2, NormalLayer)
    else:
        # Else ProductLayer should be returned
        assert isinstance(marginalized_product_layer2, ProductLayer)

    # Scope query should not contain [1, 2]
    assert len(set(marginalized_product_layer2.scope.query).intersection([1,2])) == 0

    marginalized_product_layer3 = marginalize(marginalized_product_layer2, [3], prune=prune)

    # Scope query should contain  None
    if NUM_SCOPES == 3:
        assert marginalized_product_layer3 is None
    else:
        assert len(set(marginalized_product_layer3.scope.query).intersection([1, 2, 3])) == 0
