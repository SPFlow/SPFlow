...
#
#     visualize(data.detach().cpu(), rat_spn, samples.detach().cpu())
#
# def digits_sampling():
#
#     # Parameters
...
#     visualize_digits(torch.clip(samples, min=0).detach().cpu())
#
# """
...
#     channel_index = torch.tensor([[5,2,3,7],[0,6,1,2],[4,8,7,1],[6,7,3,5]])#torch.randint(low=0, high=module.out_channels, size=(n_samples, module.out_features)):
#     repetition_index = torch.zeros((n_samples,), dtype=torch.long)
#
#     sampling_ctx = SamplingContext(channel_index=channel_index, mask=mask, repetition_index=repetition_index)
#     samples = sample(product_layer, data, sampling_ctx=sampling_ctx, is_mpe=True)
#     print(samples)
