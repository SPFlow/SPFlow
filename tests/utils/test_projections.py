...
class TestConvexProjections:
    ...
    def test_proj_convex_to_real_basic(self):
    ...
    def test_proj_convex_inverse(self):
        """Test convex projections are inverses."""
        # Start with probabilities
        x = torch.tensor([[0.1, 0.3, 0.6], [0.25, 0.25, 0.5]])
...
class TestBoundedProjectionsBothBounds:
    ...
    def test_proj_bounded_both_bounds(self):
    ...
    def test_proj_real_to_bounded_both_bounds(self):
        ...
        lb, ub = 0.0, 1.0
        y_real = torch.tensor([0.0, -1.0, 1.0])
    ...
    def test_proj_bounded_inverse_both_bounds(self):
        """Test both projections are inverses with both bounds."""
        lb, ub = -2.0, 3.0
        # Values in (lb, ub)
        x = torch.tensor([-1.5, 0.0, 1.5, 2.5])
    ...
    def test_proj_bounded_at_boundaries(self):
        ...
        x = torch.tensor([lb + 1e-6, ub - 1e-6])
        result = proj_bounded_to_real(x, lb=lb, ub=ub)
    ...
    def test_proj_bounded_midpoint(self):
        """Test projection at midpoint maps to zero."""
        lb, ub = -1.0, 1.0
        x = torch.tensor([0.0])  # Midpoint
        result = proj_bounded_to_real(x, lb=lb, ub=ub)
...
class TestBoundedProjectionsLowerOnly:
    ...
    def test_proj_bounded_lower_only(self):
        """Test proj_bounded_to_real with only lower bound."""
        lb = 0.0
        x = torch.tensor([1.0, 2.5, 10.0])
    ...
    def test_proj_real_to_bounded_lower_only(self):
        ...
        lb = 0.0
        y_real = torch.tensor([0.0, 1.0, 2.0])
    ...
    def test_proj_bounded_inverse_lower_only(self):
        """Test inverses with lower bound only."""
        lb = 1.0
        # Values > lb
        x = torch.tensor([1.5, 2.0, 5.0, 100.0])
    ...
    def test_proj_bounded_lower_at_boundary(self):
        ...
        lb = 0.0
        # Very close to lb
        x = torch.tensor([lb + 1e-6])
        result = proj_bounded_to_real(x, lb=lb, ub=None)

        # log(1e-6) ≈ -13.8
        assert result[0] < -10

    def test_proj_bounded_lower_large_values(self):
...
class TestBoundedProjectionsUpperOnly:
    ...
    def test_proj_bounded_upper_only(self):
        """Test proj_bounded_to_real with only upper bound."""
        ub = 1.0
        x = torch.tensor([0.5, 0.1, -5.0])
    ...
    def test_proj_bounded_inverse_upper_only(self):
        """Test inverses with upper bound only."""
        ub = 5.0
        # Values < ub
        x = torch.tensor([4.5, 3.0, 0.0, -10.0])
    ...
    def test_proj_bounded_upper_at_boundary(self):
        ...
        ub = 1.0
        # Very close to ub
        x = torch.tensor([ub - 1e-6])
        result = proj_bounded_to_real(x, lb=None, ub=ub)

        # log(1e-6) ≈ -13.8
        assert result[0] < -10

    def test_proj_bounded_upper_negative_values(self):
...
class TestBoundedProjectionsEdgeCases:
    ...
    def test_proj_bounded_scalar_bounds(self):
    ...
    def test_proj_bounded_broadcasted_bounds(self):
        ...
        lb = 0.0  # scalar
        ub = torch.tensor([1.0, 2.0, 3.0])  # tensor
    ...
    def test_proj_bounded_near_zero_width_bounds(self):
        """Test with lb ≈ ub (near-zero width)."""
        lb = 1.0
        ub = 1.0 + 1e-6  # Very narrow interval
        x = torch.tensor([1.0 + 5e-7])  # Midpoint
...
class TestBoundedProjectionsExtremeValues:
    ...
    def test_proj_bounded_very_small_values(self):
    ...
    def test_proj_bounded_very_large_values(self):
    ...
    def test_proj_bounded_near_zero(self):
        """Test values near zero with bounds crossing zero."""
        lb, ub = -1.0, 1.0
        x = torch.tensor([1e-15])
        result = proj_bounded_to_real(x, lb=lb, ub=ub)
        recovered = proj_real_to_bounded(result, lb=lb, ub=ub)
...
class TestBoundedProjectionsNumericalStability:
    ...
    def test_proj_bounded_numerical_stability(self):
    ...
    def test_proj_bounded_double_precision(self):
        """Test with float64 (higher precision)."""
        lb, ub = 0.0, 1.0
        x = torch.tensor([0.123456789012345], dtype=torch.float64)
...
class TestBoundedProjectionsWithGradients:
    ...
    def test_proj_bounded_with_gradients(self):
        """Test that projections maintain gradient flow."""
        lb, ub = 0.0, 1.0
    ...
    def test_proj_real_to_bounded_with_gradients(self):
        """Test proj_real_to_bounded gradient flow."""
        lb, ub = 0.0, 1.0
        y_real = torch.tensor([0.0], requires_grad=True)
    ...
    def test_proj_convex_with_gradients(self):
...
class TestBoundedProjectionsBatchedOperations:
    ...
    def test_proj_bounded_batched_operations(self):
        """Test batched projection operations."""
        lb, ub = 0.0, 1.0
        # Batch of values (10 samples, 5 features)
        x = torch.rand(10, 5) * (ub - lb) + lb
    ...
    def test_proj_bounded_per_feature_bounds(self):
        ...
        lb = torch.tensor([0.0, -1.0, 1.0])
        ub = torch.tensor([1.0, 1.0, 5.0])
...
class TestBoundedProjectionsMixedBounds:
    ...
    def test_proj_bounded_no_bounds_error_check(self):
    ...
    def test_proj_bounded_symmetric_bounds(self):
