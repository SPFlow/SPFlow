...
class TestApplyNanStrategy:
    ...
    def test_apply_nan_strategy_ignore_all_valid(self):
    ...
    def test_apply_nan_strategy_ignore_some_nan(self):
        ...
        data = torch.tensor([[1.0, 2.0], [float("nan"), 4.0], [5.0, 6.0]])
        weights = torch.ones((data.shape[0], data.shape[1], 1, 1))

        result_data, result_weights = apply_nan_strategy("ignore", data, weights)

        # Should remove row with NaN
        expected_data = torch.tensor([[1.0, 2.0], [5.0, 6.0]])
    ...
    def test_apply_nan_strategy_handles_multidimensional_weights(self):
        ...
        data = torch.tensor([[1.0, 2.0]] * 10)  # (10, 2)
        weights = torch.ones((data.shape[0], data.shape[1], 1, 1))
    ...
    def test_apply_nan_strategy_none_with_nan_error(self):
    ...
    def test_apply_nan_strategy_nan_in_different_columns(self):
        ...
        expected_data = torch.tensor([[1.0, 2.0, 3.0]])
        assert torch.equal(result_data, expected_data)
        assert result_weights.shape == (expected_data.shape[0], expected_data.shape[1], 1, 1)
...
class TestInitParameter:
    ...
    def test_init_parameter_basic(self):
    ...
    def test_init_parameter_with_provided_param(self):
        """Test init_parameter with already provided parameter."""
        provided_param = torch.tensor([1.0, 2.0, 3.0])
    ...
    def test_init_parameter_with_nan(self):
        """Test init_parameter with NaN value."""
        nan_param = torch.tensor([1.0, float("nan"), 3.0])
    ...
    def test_init_parameter_with_inf(self):
        ...
        inf_param = torch.tensor([1.0, float("inf"), 3.0])
    ...
    def test_init_parameter_with_neg_inf(self):
        ...
        neg_inf_param = torch.tensor([1.0, float("-inf"), 3.0])
    ...
    def test_init_parameter_batch_shape(self):
...
class TestHandleMleEdgeCases:
    ...
    def test_handle_mle_edge_cases_below_lower_bound(self):
        """Test _handle_mle_edge_cases with values below lower bound."""
        param_est = torch.tensor([1.0, -0.5, 3.0])
        lb = 0.0
    ...
    def test_handle_mle_edge_cases_above_upper_bound(self):
        """Test _handle_mle_edge_cases with values above upper bound."""
        param_est = torch.tensor([1.0, 15.0, 3.0])
        ub = 10.0
        result = _handle_mle_edge_cases(param_est, lb=0.0, ub=ub)

        # Value above ub should be clamped to ub - eps
        assert result[1].item() < ub
        ub_t = torch.tensor(ub, dtype=param_est.dtype)
        base_eps = torch.tensor(1e-8, dtype=param_est.dtype)
    ...
    def test_handle_mle_edge_cases_at_lower_bound(self):
        """Test _handle_mle_edge_cases with values at lower bound."""
        lb = 0.0
        param_est = torch.tensor([1.0, lb, 3.0])
        result = _handle_mle_edge_cases(param_est, lb=lb, ub=10.0)

        # Value at lb should be adjusted to lb + eps
        assert result[1] > lb

    def test_handle_mle_edge_cases_at_upper_bound(self):
        """Test _handle_mle_edge_cases with values at upper bound."""
        ub = 10.0
        param_est = torch.tensor([1.0, ub, 3.0])
        result = _handle_mle_edge_cases(param_est, lb=0.0, ub=ub)

        # Value at ub should be adjusted to ub - eps
        assert result[1] <= ub

    def test_handle_mle_edge_cases_zero_without_bounds(self):
        """Test _handle_mle_edge_cases with zero values when no bounds."""
        param_est = torch.tensor([1.0, 0.0, 3.0])
    ...
    def test_handle_mle_edge_cases_only_upper_bound(self):
...
class TestLeafUtilsIntegration:
    ...
    def test_leaves_utils_parameter_flow(self):
    ...
    def test_leaves_utils_nan_handling_flow(self):
        ...
        # Apply NaN strategy
        clean_data, weights = apply_nan_strategy("ignore", data, weights)
    ...
    def test_leaves_utils_with_gradients(self):
...
class TestLeafUtilsEdgeCases:
    ...
    def test_handle_mle_multiple_edge_cases(self):
        """Test handling multiple edge cases at once."""
        param_est = torch.tensor([float("nan"), -1.0, 5.0, 15.0, 0.0])
        result = _handle_mle_edge_cases(param_est, lb=0.0, ub=10.0)
    ...
    def test_handle_mle_dtype_preservation(self, dtype):
        """Test dtype preservation in edge case handling."""
        param_est = torch.tensor([1.0, 2.0], dtype=dtype)
        result = _handle_mle_edge_cases(param_est, lb=0.0, ub=10.0)
