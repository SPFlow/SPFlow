...
from spflow.base.structure.general.node.leaf.uniform import (
    Uniform as BaseUniform,
)
from spflow.modules.node import Uniform as GeneralUniform
...
def updateBackend(leaf_node: Uniform, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralUniform(
        scope=leaf_node.scope,
        start=leaf_node.start.data.item(),
        end=leaf_node.end.data.item(),
