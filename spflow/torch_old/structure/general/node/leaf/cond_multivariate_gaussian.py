...
from spflow.modules.node import CondMultivariateGaussian as GeneralCondMultivariateGaussian
    ...
    def retrieve_params(
        ...
        if cov is not None:
            ...
            if isinstance(cov, list):
                # convert numpy array to torch tensor
                cov = torch.tensor(
                    [[float(v) for v in row] for row in cov], dtype=self.dtype, device=self.device
                )
            ...
            if cov.ndim != 2 or (
                cov.ndim == 2
                and (cov.shape[0] != len(self.scope.query) or cov.shape[1] != len(self.scope.query))
            ):
                raise ValueError(
...
def updateBackend(leaf_node: CondMultivariateGaussian, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    return GeneralCondMultivariateGaussian(scope=leaf_node.scope, cond_f=cond_f)
