...
from spflow.base.structure.general.node.leaf.gamma import (
    Gamma as BaseGamma,
)
from spflow.modules.node import Gamma as GeneralGamma
...
class Gamma(LeafNode):
    ...
    def set_params(self, alpha: float, beta: float) -> None:
        ...
        self.alpha_aux.data = proj_bounded_to_real(
            torch.tensor(float(alpha), dtype=self.dtype, device=self.device), lb=0.0
        )
        self.beta_aux.data = proj_bounded_to_real(
            torch.tensor(float(beta), dtype=self.dtype, device=self.device), lb=0.0
        )
...
def updateBackend(leaf_node: Gamma, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralGamma(
        scope=leaf_node.scope, alpha=leaf_node.alpha.data.item(), beta=leaf_node.beta.data.item()
    )
