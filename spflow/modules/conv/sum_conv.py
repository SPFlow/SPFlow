...
class SumConv(Module):
    ...
    def rsample(
        ...
        """Differentiable sampling with per-position SIMPLE ST selection."""
        if cache is None:
            cache = Cache()
        if data is None:
            if num_samples is None:
        ...
        if sampling_ctx is None or not isinstance(sampling_ctx, DifferentiableSamplingContext):
            sampling_ctx = DifferentiableSamplingContext(
                channel_index=torch.zeros((batch_size, 1), dtype=torch.long, device=data.device),
                mask=torch.ones((batch_size, 1), dtype=torch.bool, device=data.device),
        ...
        if H * W != num_features:
            raise ValueError(
                f"SumConv requires square spatial dimensions. Got {num_features} features which is not a perfect square."
            )
        if H % K != 0 or W % K != 0:
            raise ValueError(f"Spatial dims ({H}, {W}) must be divisible by kernel_size {K}")
        ...
        channel_idx = sampling_ctx.channel_index.view(batch_size, H, W)
        logits = self.logits
        if sampling_ctx.repetition_idx is not None:
            rep_idx = sampling_ctx.repetition_idx.view(-1, 1, 1, 1, 1)
            rep_idx = rep_idx.expand(batch_size, logits.shape[0], logits.shape[1], K, K)
            ...
            logits = logits[..., 0].unsqueeze(0).expand(batch_size, -1, -1, -1, -1)
        ...
        row_pos = torch.arange(H, device=data.device).view(1, H, 1).expand(batch_size, H, W)
        col_pos = torch.arange(W, device=data.device).view(1, 1, W).expand(batch_size, H, W)
        k_flat = (row_pos % K) * K + (col_pos % K)

        logits_per_pos = logits.permute(0, 3, 4, 1, 2).view(
            batch_size, K * K, self.out_shape.channels, self.in_channels
        )
        logits_per_pos = (
            logits_per_pos.unsqueeze(2).unsqueeze(3).expand(-1, -1, H, W, -1, -1).permute(0, 2, 3, 1, 4, 5)
        )
        k_flat_exp = (
            k_flat.unsqueeze(-1)
            .unsqueeze(-1)
            .unsqueeze(-1)
            .expand(-1, -1, -1, -1, self.out_shape.channels, self.in_channels)
        )
        selected_logits = torch.gather(logits_per_pos, dim=3, index=k_flat_exp).squeeze(3)
        parent_ch = channel_idx.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, -1, -1, self.in_channels)
        selected_logits = torch.gather(selected_logits, dim=3, index=parent_ch).squeeze(3)  # (B,H,W,in_c)

        if input_lls is not None:
            selected_logits = selected_logits + input_lls
        selected_logits = F.log_softmax(selected_logits, dim=-1)
        ...
        sampled_channels = selector.argmax(dim=-1).view(batch_size, num_features)
        sampling_ctx.channel_index = sampled_channels
        sampling_ctx.channel_select = selector.view(batch_size, num_features, self.in_channels)
