...
class Normal(LeafNode):
    def __init__(self, scope: Scope, mean: float = 0.0, std: float = 1.0) -> None:
        ...
        if len(scope.query) != 1:
            raise ValueError(f"Query scope size for 'Normal' should be 1, but was: {len(scope.query)}.")
        if len(scope.evidence) != 0:
            raise ValueError(f"Evidence scope for 'Normal' should be empty, but was {scope.evidence}.")
    ...
    def from_signatures(cls, signatures: list[FeatureContext]) -> "Normal":
        ...
        return Normal(feature_ctx.scope, mean=mean, std=std)
...
def maximum_likelihood_estimation(
    ...
    r"""Maximum (weighted) likelihood estimation (MLE) of ``Normal`` node parameters in the ``torch`` backend.

    Estimates the mean and standard deviation :math:`\mu` and :math:`\sigma` of a Normal distribution from data, as follows:
