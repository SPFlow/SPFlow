...
class EinsumLayer(Module):
    ...
    def weights(self, values: Tensor) -> None:
        ...
        if not torch.allclose(sums, torch.ones_like(sums)):
            raise ValueError("Weights must sum to 1 over (i,j) channel pairs.")
    ...
    def sample(
        ...
        if self._two_inputs:
            ...
            # Single input with SplitHalves - need to expand indices for both halves
            # Left half: features 0, 2, 4, ...
            # Right half: features 1, 3, 5, ...
            ...
            # Interleave left and right indices
            full_indices[:, 0::2] = left_indices
            full_indices[:, 1::2] = right_indices
