...
class EinsumLayer(Module):
    ...
    def rsample(
        ...
        logits = select_parent_axis(logits, sampling_ctx=sampling_ctx, dim=2)  # (B,D,R,I,J)
        ...
        if left_ll is not None and right_ll is not None:
            ...
            joint_ll_flat = joint_ll.view(batch_size, self.out_shape.features, -1)
        logits_flat = condition_logits_with_evidence(logits_flat, joint_ll_flat, dim=-1)
        ...
        if self._two_inputs:
            ...
            update_ctx_channel_routing(
                left_ctx,
                channel_index=left_indices,
                channel_select=left_select,
                method=method,
                tau=tau,
                hard=hard,
            )
            ...
            update_ctx_channel_routing(
                right_ctx,
                channel_index=right_indices,
                channel_select=right_select,
                method=method,
                tau=tau,
                hard=hard,
            )
        ...
        full_mask = sampling_ctx.mask.repeat(1, 2)
        child_ctx = sampling_ctx.copy()
        child_select = self.inputs.merge_split_tensors(left_select, right_select)
        child_ctx.update(channel_index=full_indices, mask=full_mask)
        update_ctx_channel_routing(
            child_ctx,
