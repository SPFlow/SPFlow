...
from spflow.base.structure.general.node.leaf.log_normal import LogNormal
from spflow.structure.spn.layer.leaf import LogNormalLayer as GeneralLogNormalLayer
...
class LogNormalLayer(Module):
    ...
    def set_params(
        ...
        for node_mean, node_std, node in zip(mean, std, self.nodes):
            ...
            node.set_parameters(node_mean, node_std)
...
def updateBackend(leaf_node: LogNormalLayer, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralLogNormalLayer(scope=leaf_node.scopes_out, mean=leaf_node.mean, std=leaf_node.std)
