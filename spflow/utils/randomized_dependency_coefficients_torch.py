...
def randomized_dependency_coefficients(data, k: int = 20, s: float = 1 / 6, phi: Callable = torch.sin):
    ...
    Returns the randomized dependency coefficients (RDCs) computed from a specified data set, as described in (Lopez-Paz et al., 2013): "nphe Randomized Dependence Coefficient"
    ...
    ecdf_features = torch.stack([ecdf.T, torch.ones(ecdf.T.shape, device=data.device)], dim=-1)
    ...
    """
    # create matrix holding the pair-wise dependency coefficients
    rdcs = torch.eye(n, device=data.device)

    cca = CCA(n_components=1)

    # compute rdcs for all pairs of features
    for i, j in combinations(range(data.shape[1]), 2):
        i_cca, j_cca = cca.fit_transform(features[i].cpu().numpy(), features[j].cpu().numpy())
    ...
    rdcs_batch = torch.eye(n, device=data.device)
    rdcs_batch[i_indices, j_indices] = distances
