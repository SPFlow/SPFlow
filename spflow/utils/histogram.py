...
def _get_outer_edges_torch(a: Tensor, range_bounds: tuple[float, float] | None = None) -> tuple[float, float]:
    ...
    if range_bounds is not None:
        ...
        if not (torch.isfinite(torch.tensor(first_edge)) and torch.isfinite(torch.tensor(last_edge))):
            raise InvalidParameterError(f"Autodetected range [{first_edge}, {last_edge}] is not finite.")
...
def get_bin_edges_torch(
    ...
    # Compute bin edges
    bin_edges = torch.linspace(first_edge, last_edge, n_equal_bins + 1, dtype=torch.float32, device=a.device)
