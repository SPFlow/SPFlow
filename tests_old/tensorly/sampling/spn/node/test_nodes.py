...
from spflow.structure.spn import Gaussian  # , ProductNode, SumNode
...
def test_update_backend(do_for_all_backends):
    ...
    samples = sample(s, 1000)
    samples_mean = tle.toNumpy(samples).mean()
    for backend in backends:
        with tl.backend_context(backend):
            s_updated = updateBackend(s)
            samples_updated = sample(s_updated, 10000)
            samples_mean_updated = tle.toNumpy(samples_updated).mean()
