...
from spflow.modules.node import MultivariateGaussian
...
def test_conditional_sampling(do_for_all_backends):
    ...
    for i in range(10):
        ...
        cov_est = np.cov(tle.toNumpy(data[i * 10000 : (i + 1) * 10000, ~cond_mask].T))
        ...
        tc.assertTrue(np.allclose(tle.toNumpy(mean_exact), tle.toNumpy(mean_est), atol=0.01, rtol=0.1))
        tc.assertTrue(np.allclose(tle.toNumpy(cov_exact), tle.toNumpy(cov_est), atol=0.01, rtol=0.1))
...
def test_update_backend(do_for_all_backends):
    ...
    # estimate mean and covariance matrix for conditioned distribution from data
    mean_est = tle.toNumpy(tl.mean(data, axis=0))
    cov_est = tle.toNumpy(np.cov(data.T))
    ...
    for backend in backends:
        with tl.backend_context(backend):
            ...
            # estimate mean and covariance matrix for conditioned distribution from data
            mean_est_updated = tle.toNumpy(data_updated).mean(axis=0)
            cov_est_updated = np.cov(tle.toNumpy(data_updated).T)
