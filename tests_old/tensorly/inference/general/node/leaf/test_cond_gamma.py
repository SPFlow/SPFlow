...
from spflow.structure.spn import CondGamma
from spflow.meta.data import Scope
from spflow.meta.dispatch import DispatchContext
from spflow.modules.module import log_likelihood, likelihood
from spflow.base.structure.general.node.leaf.cond_gamma import CondGamma as BaseCondGamma
...
def test_inference(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(log_probs, tle.toNumpy(log_probs_torch), atol=0.001, rtol=0.001))
...
def test_change_dtype(do_for_all_backends):
    alpha = tl.tensor(random.randint(1, 5), dtype=tl.float32)
    beta = tl.tensor(random.randint(1, 5), dtype=tl.float32)
