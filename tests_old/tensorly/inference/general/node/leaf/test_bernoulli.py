...
from spflow.inference import likelihood
from spflow.modules.nested_module import log_likelihood
...
def test_inference(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(log_probs, tle.toNumpy(log_probs_general)))
...
def test_gradient_optimization(do_for_all_backends):
    ...
    tc.assertTrue(
        torch.allclose(torch_bernoulli.p, tl.tensor(p_target, dtype=tl.float32), atol=1e-3, rtol=1e-3)
    )
