...
from spflow.structure.spn import CondMultivariateGaussian
...
def test_likelihood_marginalization(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(probs), tl.tensor(1.0, dtype=tl.float64)))
    ...
    if do_for_all_backends == "numpy":
        ...
        tc.assertTrue(np.allclose(tle.toNumpy(mv_probs), targets))
        ...
        # compare
        tc.assertTrue(np.allclose(tle.toNumpy(mv_probs), tle.toNumpy(uv_probs)))
        ...
        tc.assertTrue(np.allclose(tle.toNumpy(mv_probs), targets, atol=1e-6))
