...
from spflow.structure.general.node.leaf.general_binomial import Binomial
...
def test_gradient_computation(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(p_aux_orig - binomial.p_aux.grad), tle.toNumpy(binomial.p_aux)))

    # verify that distribution parameters match parameters
    tc.assertTrue(torch.equal(binomial.n, binomial.dist().total_count.long()))
    tc.assertTrue(np.allclose(tle.toNumpy(binomial.p), tle.toNumpy(binomial.dist().probs)))
...
def test_gradient_optimization(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(binomial.p), tle.toNumpy(p_target), atol=1e-3, rtol=1e-3))
