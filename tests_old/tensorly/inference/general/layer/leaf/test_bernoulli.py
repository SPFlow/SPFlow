...
from spflow.structure.general.layer.leaf import BernoulliLayer
...
def test_layer_gradient_computation(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(p_aux_orig - bernoulli.p_aux.grad), tle.toNumpy(bernoulli.p_aux)))

    # verify that distribution parameters match parameters
    tc.assertTrue(np.allclose(tle.toNumpy(bernoulli.p), tle.toNumpy(bernoulli.dist().probs)))
...
def test_gradient_optimization(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(torch_bernoulli.p), tle.toNumpy(p_target), atol=1e-2, rtol=1e-2))
