...
from ....structure.general.node.dummy_node import DummyNode
...
def test_likelihood(do_for_all_backends):
    ...
    tc.assertTrue(
        np.isclose(
            tle.toNumpy(tl.tensor(l_result[0][0], dtype=tl.float32)), tl.tensor(0.023358, dtype=tl.float32)
        )
    )
    tc.assertTrue(
        np.isclose(
            tle.toNumpy(tl.tensor(ll_result[0][0], dtype=tl.float32)), tl.tensor(-3.7568156, dtype=tl.float32)
        )
    )
...
def test_likelihood_marginalization(do_for_all_backends):
    ...
    tc.assertTrue(
        np.isclose(
            tle.toNumpy(tl.tensor(l_result[0][0], dtype=tl.float32)), tl.tensor(0.09653235, dtype=tl.float32)
        )
    )
    tc.assertTrue(
        np.isclose(
            tle.toNumpy(tl.tensor(ll_result[0][0], dtype=tl.float32)),
            tl.tensor(-2.33787707, dtype=tl.float32),
        )
    )
