...
from spflow.structure.spn import PartitionLayer
from spflow.structure.spn.layer.partition_layer import toLayerBased, toNodeBased
...
def test_partition_layer_initialization(do_for_all_backends):
    ...
    for indices, indices_torch in zip(
        ...
        tle.cartesian_product(tl.tensor([0, 1]), tl.tensor([2, 3, 4]), tl.tensor([5])),
    ):
