...
def test_initialization(do_for_all_backends):
    ...
    # l > 0
    Exponential(Scope([0]), tle.nextafter(tl.tensor(0.0), tl.tensor(1.0)))
    # l = 0 and l < 0
...
def test_initialization_from_signatures(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(exponential.l), tl.tensor(1.0)))
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(exponential.l), tl.tensor(1.5)))
