...
def test_accept(do_for_all_backends):
    ...
    tc.assertTrue(
        NegativeBinomial.accepts([FeatureContext(Scope([0]), [FeatureTypes.NegativeBinomial(n=3)])])
    )
...
def test_initialization_from_signatures(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(negative_binomial.n), tl.tensor(3)))
    tc.assertTrue(np.isclose(tle.toNumpy(negative_binomial.p), tl.tensor(0.5)))

    negative_binomial = NegativeBinomial.from_signatures(
        [FeatureContext(Scope([0]), [FeatureTypes.NegativeBinomial(n=3, p=0.75)])]
    )
    tc.assertTrue(np.isclose(tle.toNumpy(negative_binomial.n), tl.tensor(3)))
    tc.assertTrue(np.isclose(tle.toNumpy(negative_binomial.p), tl.tensor(0.75)))
