...
def test_initialization_from_signatures(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(multivariate_gaussian.mean), tl.zeros(2)))
    tc.assertTrue(np.allclose(tle.toNumpy(multivariate_gaussian.cov), tl.eye(2)))
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(multivariate_gaussian.mean), tl.tensor([-1.0, 1.0])))
    tc.assertTrue(
        np.allclose(
            tle.toNumpy(multivariate_gaussian.cov),
