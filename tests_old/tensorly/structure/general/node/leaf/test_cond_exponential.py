...
from spflow.torch.structure.general.node.leaf.cond_exponential import updateBackend
...
from spflow.modules.node import CondExponential
...
def test_retrieve_params(do_for_all_backends):
    ...
    exponential.set_cond_f(
        lambda data: {"l": tle.nextafter(tl.tensor(0.0, dtype=tl.float32), tl.tensor(1.0, dtype=tl.float32))}
    )
    tc.assertTrue(
        exponential.retrieve_params(np.array([[1.0]]), DispatchContext())
        == tle.nextafter(tl.tensor(0.0, dtype=tl.float32), tl.tensor(1.0, dtype=tl.float32))
    )
    ...
    exponential.set_cond_f(lambda data: {"l": tle.nextafter(tl.tensor(0.0), tl.tensor(-1.0))})
    tc.assertRaises(
        Exception,
        exponential.retrieve_params,
        np.array([[1.0]]),
...
def test_change_device(do_for_all_backends):
    ...
    model_updated = CondExponential(Scope([0], [1]))
    model_updated.set_cond_f(lambda data: {"l": tle.nextafter(tl.tensor(0.0), tl.tensor(1.0))})
    # put model on gpu
    if do_for_all_backends == "numpy":
