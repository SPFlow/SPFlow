...
def test_initialization_from_signatures(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(binomial.n), tl.tensor(3)))
    tc.assertTrue(np.isclose(tle.toNumpy(binomial.p), tl.tensor(0.5)))

    binomial = Binomial.from_signatures([FeatureContext(Scope([0]), [FeatureTypes.Binomial(n=3, p=0.75)])])
    tc.assertTrue(np.isclose(tle.toNumpy(binomial.n), tl.tensor(3)))
    tc.assertTrue(np.isclose(tle.toNumpy(binomial.p), tl.tensor(0.75)))
