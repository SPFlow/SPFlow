...
from spflow.structure.general.node.leaf.general_cond_gamma import CondGamma
...
def test_retrieve_params(do_for_all_backends):
    ...
    gamma.set_cond_f(
        lambda data: {
            "alpha": tle.nextafter(tl.tensor(0.0, dtype=tl.float32), tl.tensor(1.0, dtype=tl.float32)),
    ...
    # alpha = 0
    gamma.set_cond_f(
        lambda data: {"alpha": tl.tensor(0.0, dtype=tl.float32), "beta": tl.tensor(1.0, dtype=tl.float32)}
    )
    tc.assertRaises(
        Exception,
    ...
    gamma.set_cond_f(
        lambda data: {
            "alpha": tle.nextafter(tl.tensor(0.0), -tl.tensor(1.0)),
            "beta": tl.tensor(1.0),
        }
    )
    tc.assertRaises(
    ...
    gamma.set_cond_f(
        lambda data: {
            ...
            "beta": tle.nextafter(tl.tensor(0.0, dtype=tl.float32), tl.tensor(1.0, dtype=tl.float32)),
        }
    )
    tc.assertTrue(
    ...
    # beta = 0
    gamma.set_cond_f(
        lambda data: {"alpha": tl.tensor(1.0, dtype=tl.float32), "beta": tl.tensor(0.0, dtype=tl.float32)}
    )
    tc.assertRaises(
        Exception,
        gamma.retrieve_params,
