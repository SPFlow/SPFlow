...
def test_retrieve_params(do_for_all_backends):
    ...
    # p < 0 and p > 1
    binomial.set_cond_f(lambda data: {"p": tle.nextafter(tl.tensor(1.0), tl.tensor(2.0))})
    tc.assertRaises(
        Exception,
    ...
    binomial.set_cond_f(lambda data: {"p": tle.nextafter(tl.tensor(0.0), -tl.tensor(1.0))})
    tc.assertRaises(
        Exception,
        binomial.retrieve_params,
        np.array([[1.0]]),
