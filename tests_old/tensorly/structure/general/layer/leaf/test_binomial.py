...
from spflow.modules.layer import BinomialLayer
...
def test_layer_initialization(do_for_all_backends):
    ...
    for n_layer_node, p_layer_node, n_value, p_value in zip(l.n, l.p, n_values, p_values):
        tc.assertTrue(np.allclose(tle.toNumpy(n_layer_node), np.array(n_value)))
...
def test_initialization_from_signatures(do_for_all_backends):
    ...
    tc.assertTrue(np.all(tle.toNumpy(binomial.n) == np.array([3, 5])))
    tc.assertTrue(np.allclose(tle.toNumpy(binomial.p), np.array([0.5, 0.5])))
    ...
    tc.assertTrue(np.all(tle.toNumpy(binomial.n) == np.array([3, 5])))
    tc.assertTrue(np.allclose(tle.toNumpy(binomial.p), np.array([0.75, 0.25])))
    tc.assertTrue(binomial.scopes_out == [Scope([0]), Scope([1])])
...
def test_layer_structural_marginalization(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(l_marg.n), np.array(2)))
...
def test_layer_dist(do_for_all_backends):
    ...
    for n_value, p_value, n_dist, p_dist in zip(n_values, p_values, n_list, p_list):
        tc.assertTrue(np.allclose(np.array(n_value), tle.toNumpy(n_dist)))
...
def test_change_dtype(do_for_all_backends):
    ...
    binomial_default = BinomialLayer(
        scope=[Scope([0]), Scope([1]), Scope([0])],
        n=[2, 5, 2],
        p=[0.2, 0.9, 0.31],
    )
    ...
    binomial_updated = BinomialLayer(
        scope=[Scope([0]), Scope([1]), Scope([0])],
        n=[2, 5, 2],
        p=[0.2, 0.9, 0.31],
    )
