...
from spflow.torch.structure import AutoLeaf, marginalize
...
from spflow.structure.general.layer.leaf import ExponentialLayer
...
def test_layer_initialization(do_for_all_backends):
    ...
    for l_layer_node, l_value in zip(l.l, l_values):
        tc.assertTrue(np.allclose(tle.toNumpy(l_layer_node), l_value))
...
def test_initialization_from_signatures(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(exponential.l), np.array([1.0, 1.0])))
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(exponential.l), np.array([1.5, 0.5])))
    tc.assertTrue(exponential.scopes_out == [Scope([0]), Scope([1])])
...
def test_layer_structural_marginalization(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(l_marg.l), np.array(0.29)))
...
def test_layer_dist(do_for_all_backends):
    ...
    for l_value, l_dist in zip(l_values, l_list):
        tc.assertTrue(np.allclose(l_value, tle.toNumpy(l_dist)))
