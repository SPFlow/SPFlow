...
from spflow.torch.structure.general.layer.leaf.cond_binomial import (
    CondBinomialLayer as CondBinomialLayerTorch,
)
...
def test_layer_dist(do_for_all_backends):
    ...
    for n_value, p_value, n_dist, p_dist in zip(n_values, p_values, n_list, p_list):
        ...
        tc.assertTrue(np.allclose(tle.toNumpy(p_value), tle.toNumpy(p_dist)))
...
def test_update_backend(do_for_all_backends):
    ...
    binomial = CondBinomialLayer(scope=[Scope([0], [3]), Scope([1], [3]), Scope([0], [3])], n=[2, 5, 2])
