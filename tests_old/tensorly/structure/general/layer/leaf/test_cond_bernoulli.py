...
from spflow.base.structure.general.layer.leaf.cond_bernoulli import (
    CondBernoulliLayer as CondBernoulliLayerBase,
)
...
from spflow.torch.structure.general.layer.leaf.cond_bernoulli import (
    CondBernoulliLayer as CondBernoulliLayerTorch,
)
...
def test_retrieve_param(do_for_all_backends):
    ...
    for p in l.retrieve_params(np.array([[1.0]]), DispatchContext()):
        tc.assertTrue(np.allclose(tle.toNumpy(p), p_value))
    ...
    for p_layer_node, p_actual in zip(
        ...
        tc.assertTrue(np.allclose(tle.toNumpy(p_layer_node), p_actual))
...
def test_change_dtype(do_for_all_backends):
    ...
    p_value = 0.13
    bernoulli_default = CondBernoulliLayer(
    ...
    bernoulli_updated = CondBernoulliLayer(
        scope=Scope([1], [0]), n_nodes=3, cond_f=lambda data: {"p": p_value}
    )
