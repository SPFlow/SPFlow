...
from spflow.base.structure.general.layer.leaf.cond_exponential import (
    CondExponentialLayer as CondExponentialLayerBase,
)
...
from spflow.torch.structure.general.layer.leaf.cond_exponential import (
    CondExponentialLayer as CondExponentialLayerTorch,
)
...
from spflow.structure.general.layer.leaf import CondExponentialLayer
...
def test_retrieve_params(do_for_all_backends):
    ...
    for l_layer_node in l.retrieve_params(torch.tensor([[1]]), DispatchContext()):
        tc.assertTrue(np.allclose(tle.toNumpy(l_layer_node), np.array(l_value)))
...
def test_layer_dist(do_for_all_backends):
    ...
    for l_value, l_dist in zip(l_values, l_list):
        tc.assertTrue(np.allclose(tle.toNumpy(l_value), tle.toNumpy(l_dist)))
...
def test_change_dtype(do_for_all_backends):
    ...
    l_value = 0.73
    exponential_default = CondExponentialLayer(
        scope=Scope([1], [0]), n_nodes=3, cond_f=lambda data: {"l": l_value}
    )

    exponential_updated = CondExponentialLayer(
        scope=Scope([1], [0]), n_nodes=3, cond_f=lambda data: {"l": l_value}
    )
