...
from spflow.learn.learn_spn import (
    cluster_by_kmeans,
    learn_spn,
    partition_by_rdc,
)
from spflow.structure.spn import (
    CondSumNode,
    ProductNode,
    SumNode,
)
from spflow.structure.spn import Gaussian, CondGaussian
...
def test_kmeans_clustering_1(do_for_all_backends):
    ...
    cluster_mask = cluster_by_kmeans(
        tl.tensor(np.vstack([cluster_1, cluster_2]), dtype=tl.float32), n_clusters=2
    )
...
def test_kmeans_clustering_2(do_for_all_backends):
    ...
    # compute clusters using k-means
    cluster_mask = cluster_by_kmeans(
        tl.tensor(np.vstack([cluster_1, cluster_2, cluster_3]), dtype=tl.float32), n_clusters=3
    )
...
def test_rdc_partitioning_1(do_for_all_backends):
    ...
    partition_mask = partition_by_rdc(
        tl.tensor(np.hstack([data_partition_1, data_partition_2]), dtype=tl.float32), threshold=0.5
    )

    # should be two partitions
    tc.assertTrue(len(tle.unique(partition_mask)) == 2)
