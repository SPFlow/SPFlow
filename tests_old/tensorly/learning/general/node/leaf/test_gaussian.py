...
def test_mle_edge_std_0(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.mean), data[0]))
...
def test_em_step(do_for_all_backends):
    ...
    leaf = Gaussian(Scope([0]))
    data = tl.tensor(np.random.normal(loc=-1.7, scale=0.2, size=(10000, 1)))
...
def test_em_mixture_of_gaussians(do_for_all_backends):
    # em is only implemented for pytorch backend
    if do_for_all_backends == "numpy":
        return
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(l1.mean), tl.tensor(2.0), atol=1e-3, rtol=1e-3))
    tc.assertTrue(np.isclose(tle.toNumpy(l1.std), tl.tensor(1.0), atol=1e-2, rtol=1e-2))
    tc.assertTrue(np.isclose(tle.toNumpy(l2.mean), tl.tensor(-2.0), atol=1e-3, rtol=1e-3))
    tc.assertTrue(np.isclose(tle.toNumpy(l2.std), tl.tensor(1.0), atol=1e-2, rtol=1e-2))
