...
from spflow.structure.spn import NegativeBinomial  # , ProductNode, SumNode
...
def test_mle_2(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.p), tl.tensor(0.7), atol=1e-2, rtol=1e-3))
...
def test_mle_edge_0(do_for_all_backends):
    ...
    tc.assertTrue(tle.toNumpy(leaf.p) > 0.0)
...
def test_mle_nan_strategy_ignore(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.p), tl.tensor((3 * 2) / (1 + 2 + 2 + 2 + 1 + 2))))
...
def test_weighted_mle(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.p), tl.tensor(0.2), atol=1e-3, rtol=1e-2))
...
def test_em_product_of_negative_binomials(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(l1.p), tl.tensor(0.8), atol=1e-3, rtol=1e-2))
    tc.assertTrue(np.isclose(tle.toNumpy(l2.p), tl.tensor(0.2), atol=1e-3, rtol=1e-2))
...
def test_em_sum_of_negative_binomials(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(l1.p), tl.tensor(0.3), atol=1e-2, rtol=1e-2))
