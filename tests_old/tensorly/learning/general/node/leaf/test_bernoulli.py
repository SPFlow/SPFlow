...
from spflow.structure.spn import Bernoulli
...
def test_mle_nan_strategy_ignore(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.p), tl.tensor(2.0 / 3.0)))
...
def test_em_step(do_for_all_backends):
    ...
    leaf = Bernoulli(Scope([0]))
    data = tl.tensor(np.random.binomial(n=1, p=0.2, size=(10000, 1)))
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.p), tl.tensor(0.2), atol=1e-2, rtol=1e-3))
