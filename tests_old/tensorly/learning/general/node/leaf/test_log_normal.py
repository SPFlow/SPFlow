...
from spflow.structure.spn import LogNormal
...
def test_mle_1(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.mean), tl.tensor(-1.7), atol=1e-2, rtol=1e-2))
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.std), tl.tensor(0.2), atol=1e-2, rtol=1e-2))
...
def test_mle_2(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.mean), tl.tensor(0.5), atol=1e-2, rtol=1e-2))
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.std), tl.tensor(1.3), atol=1e-2, rtol=1e-2))
...
def test_mle_bias_correction(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.std), tl.sqrt(tl.tensor(1.0))))

    # perform MLE
    maximum_likelihood_estimation(leaf, data, bias_correction=True)
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.std), tl.sqrt(tl.tensor(2.0))))
...
def test_mle_edge_std_0(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.mean), tl.log(data[0])))
    tc.assertTrue(tle.toNumpy(leaf.std) > 0)
...
def test_mle_edge_std_nan(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.mean), tl.log(data[0])))
    tc.assertFalse(np.isnan(tle.toNumpy(leaf.std)))
    tc.assertTrue(tle.toNumpy(leaf.std) > 0)
...
def test_mle_nan_strategy_ignore(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.mean), tl.tensor(-1.0 / 3.0)))
    tc.assertTrue(
        np.isclose(
            tle.toNumpy(leaf.std),
...
def test_weighted_mle(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.mean), tl.tensor(0.5), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.std), tl.tensor(1.4), atol=1e-2, rtol=1e-2))
...
def test_em_step(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.mean), tl.tensor(-1.7), atol=1e-2, rtol=1e-3))
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.std), tl.tensor(0.2), atol=1e-2, rtol=1e-3))
...
def test_em_product_of_log_normals(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(l1.mean), tl.tensor(2.0), atol=1e-3, rtol=1e-2))
    tc.assertTrue(np.isclose(tle.toNumpy(l1.std), tl.tensor(1.0), atol=1e-2, rtol=1e-2))
    tc.assertTrue(np.isclose(tle.toNumpy(l2.mean), tl.tensor(-2.0), atol=1e-3, rtol=1e-2))
    tc.assertTrue(np.isclose(tle.toNumpy(l2.std), tl.tensor(1.0), atol=1e-2, rtol=1e-2))
...
def test_em_mixture_of_log_normals(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(l1.mean), tl.tensor(2.0), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.isclose(tle.toNumpy(l1.std), tl.tensor(1.0), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.isclose(tle.toNumpy(l2.mean), tl.tensor(-2.0), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.isclose(tle.toNumpy(l2.std), tl.tensor(1.0), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.allclose(tle.toNumpy(sum_node.weights), tl.tensor([0.5, 0.5]), atol=1e-3, rtol=1e-2))
