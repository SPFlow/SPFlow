...
from spflow.structure.spn import Poisson  # , ProductNode, SumNode
...
def test_weighted_mle(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.l), tl.tensor(0.5), atol=1e-2, rtol=1e-1))
...
def test_em_sum_of_poissons(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(l_opt, tle.toNumpy(l_em)))
