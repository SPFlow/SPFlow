...
from spflow.structure.spn import Geometric
...
def test_mle_1(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.p), tl.tensor(0.3), atol=1e-2, rtol=1e-2))
...
def test_mle_2(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.p), tl.tensor(0.7), atol=1e-2, rtol=1e-2))
...
def test_mle_bias_correction(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.p), tl.tensor(2.0 / 4.0)))

    # perform MLE
    maximum_likelihood_estimation(leaf, data, bias_correction=True)
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.p), tl.tensor(1.0 / 4.0)))
...
def test_mle_edge_1(do_for_all_backends):
    ...
    tc.assertTrue(tle.toNumpy(leaf.p) < 1.0)
...
def test_mle_nan_strategy_ignore(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.p), tl.tensor(3.0 / 8.0)))
...
def test_weighted_mle(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.p), tl.tensor(0.2), atol=1e-2, rtol=1e-1))
...
def test_em_step(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.p), tl.tensor(0.3), atol=1e-2, rtol=1e-3))
...
def test_em_product_of_geometrics(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(l1.p), tl.tensor(0.2), atol=1e-2, rtol=1e-2))
    tc.assertTrue(np.isclose(tle.toNumpy(l2.p), tl.tensor(0.8), atol=1e-2, rtol=1e-2))
...
def test_em_sum_of_geometrics(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(l1.p), tl.tensor(0.2), atol=1e-2, rtol=1e-2))
    tc.assertTrue(np.isclose(tle.toNumpy(l2.p), tl.tensor(0.8), atol=1e-2, rtol=1e-2))
    tc.assertTrue(np.allclose(tle.toNumpy(sum_node.weights), tl.tensor([0.5, 0.5]), atol=1e-2, rtol=1e-2))
