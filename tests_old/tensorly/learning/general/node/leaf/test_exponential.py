...
from spflow.structure.spn import Exponential
...
def test_mle_1(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.l), tl.tensor(0.3), atol=1e-2, rtol=1e-3))
...
def test_mle_2(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.l), tl.tensor(2.7), atol=1e-2, rtol=1e-2))
...
def test_mle_bias_correction(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.l), tl.tensor(2.0 / 3.0)))

    # perform MLE
    maximum_likelihood_estimation(leaf, data, bias_correction=True)
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.l), tl.tensor(1.0 / 3.0)))
...
def test_mle_edge_0(do_for_all_backends):
    ...
    tc.assertFalse(np.isnan(tle.toNumpy(leaf.l)))
    tc.assertTrue(tle.toNumpy(leaf.l) > 0.0)
...
def test_mle_nan_strategy_ignore(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.l), tl.tensor(3.0 / 2.7)))
...
def test_weighted_mle(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(leaf.l), tl.tensor(1.4), atol=1e-3, rtol=1e-2))
...
def test_em_product_of_exponentials(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(l1.l), tl.tensor(0.8), atol=1e-3, rtol=1e-2))
    tc.assertTrue(np.isclose(tle.toNumpy(l2.l), tl.tensor(1.4), atol=1e-3, rtol=1e-2))
...
def test_em_sum_of_exponentials(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(l1.l), tl.tensor(0.8), atol=1e-2, rtol=1e-2))
    tc.assertTrue(np.isclose(tle.toNumpy(l2.l), tl.tensor(1.4), atol=1e-2, rtol=1e-2))
