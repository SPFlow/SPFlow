...
from spflow.structure.spn import MultivariateGaussian
...
def test_mle_1(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(leaf.mean), tl.tensor([-1.7, 0.3]), atol=1e-2, rtol=1e-2))
    tc.assertTrue(
        np.allclose(
            tle.toNumpy(leaf.cov),
...
def test_mle_2(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(leaf.mean), tl.tensor([0.5, 0.2]), atol=1e-2, rtol=1e-2))
    tc.assertTrue(
        np.allclose(
            tle.toNumpy(leaf.cov),
...
def test_mle_bias_correction(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(leaf.cov), tl.tensor([[1.0, -0.25], [-0.25, 0.0625]])))

    # perform MLE
    maximum_likelihood_estimation(leaf, data, bias_correction=True)
    tc.assertTrue(np.allclose(tle.toNumpy(leaf.cov), 2 * tl.tensor([[1.0, -0.25], [-0.25, 0.0625]])))
...
def test_mle_edge_cov_zero(do_for_all_backends):
    ...
    tc.assertTrue(np.all(np.diag(tle.toNumpy(leaf.cov)) > 0))
...
def test_mle_nan_strategy_ignore(do_for_all_backends):
    ...
    mean_ignore, cov_ignore = tle.toNumpy(leaf.mean), tle.toNumpy(leaf.cov)
    ...
    mean_none, cov_none = tle.toNumpy(leaf.mean), tle.toNumpy(leaf.cov)
...
def test_weighted_mle(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(leaf.mean), tl.tensor([0.5, -0.3]), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.allclose(tle.toNumpy(leaf.cov), tl.eye(2), atol=1e-1, rtol=1e-1))
...
def test_em_step(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(leaf.mean), tl.tensor([-1.7, 0.6]), atol=1e-2, rtol=1e-1))
    tc.assertTrue(
        np.allclose(
            tle.toNumpy(leaf.cov),
...
def test_em_product_of_multivariate_gaussians(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(l1.mean), tl.tensor([1.0, 1.0]), atol=1e-2, rtol=1e-2))
    tc.assertTrue(np.allclose(tle.toNumpy(l1.cov), tl.eye(2), atol=1e-1, rtol=1e-1))
    tc.assertTrue(np.allclose(tle.toNumpy(l2.mean), tl.tensor([-2.0]), atol=1e-2, rtol=1e-2))
    tc.assertTrue(np.allclose(tle.toNumpy(l2.cov), tl.tensor([[1.0]]), atol=1e-2, rtol=1e-1))
...
def test_em_mixture_of_multivariate_gaussians(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(l1.mean), tl.tensor([2.0, 2.0]), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.allclose(tle.toNumpy(l1.cov), tl.eye(2), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.allclose(tle.toNumpy(l2.mean), tl.tensor([-2.0, -2.0]), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.allclose(tle.toNumpy(l2.cov), tl.eye(2), atol=1e-2, rtol=1e-1))
