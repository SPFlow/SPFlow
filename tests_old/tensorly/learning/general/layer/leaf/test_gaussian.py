...
from spflow.structure.spn import GaussianLayer  # , ProductNode, SumNode
...
def test_mle_edge_std_0(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(layer.mean), data[0]))
...
def test_mle_edge_std_nan(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.mean), data[0]))
    tc.assertFalse(np.any(np.isnan(tle.toNumpy(layer.std))))
    tc.assertTrue(tle.toNumpy(layer.std) > 0)
