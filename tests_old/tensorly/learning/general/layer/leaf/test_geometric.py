...
from spflow.structure.spn import GeometricLayer  # , ProductNode, SumNode
...
def test_mle(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.p), tl.tensor([0.3, 0.7]), atol=1e-2, rtol=1e-2))
...
def test_mle_bias_correction(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.p), tl.tensor(2.0 / 4.0)))

    # perform MLE
    maximum_likelihood_estimation(layer, data, bias_correction=True)
    tc.assertTrue(np.allclose(tle.toNumpy(layer.p), tl.tensor(1.0 / 4.0)))
...
def test_mle_edge_1(do_for_all_backends):
    ...
    # perform MLE
    maximum_likelihood_estimation(layer, data, bias_correction=True)
    tc.assertTrue(np.all(tle.toNumpy(layer.p) < 1.0))
...
def test_mle_nan_strategy_ignore(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.p), tl.tensor(3.0 / 8.0)))
...
def test_em_sum_of_geometrics(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(leaf.p), tl.tensor([0.2, 0.8]), atol=1e-2, rtol=1e-2))
