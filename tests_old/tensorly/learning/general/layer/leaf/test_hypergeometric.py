...
from spflow.structure.spn import HypergeometricLayer  # , ProductNode, SumNode
...
def test_update_backend(do_for_all_backends):
    ...
    M = tle.toNumpy(layer.M)
    N = tle.toNumpy(layer.N)
    n = tle.toNumpy(layer.n)

    layer = HypergeometricLayer(scope=[Scope([0]), Scope([1])], N=[10, 4], M=[7, 2], n=[3, 2])
    prod_node = ProductNode([layer])
    expectation_maximization(prod_node, tl.tensor(data), max_steps=10)
    M_em = tle.toNumpy(layer.M)
    N_em = tle.toNumpy(layer.N)
    n_em = tle.toNumpy(layer.n)
    ...
    for backend in backends:
        with tl.backend_context(backend):
            ...
            M_updated = tle.toNumpy(layer_updated.M)
            N_updated = tle.toNumpy(layer_updated.N)
            n_updated = tle.toNumpy(layer_updated.n)
            ...
            if tl.get_backend() != "pytorch":
                ...
                expectation_maximization(prod_node, tl.tensor(data), max_steps=10)
                M_em_updated = tle.toNumpy(layer_updated.M)
                N_em_updated = tle.toNumpy(layer_updated.N)
                n_em_updated = tle.toNumpy(layer_updated.n)
