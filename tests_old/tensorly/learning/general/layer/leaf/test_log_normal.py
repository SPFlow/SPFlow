...
from spflow.structure.spn import LogNormalLayer  # , ProductNode, SumNode
...
def test_mle(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.mean), tl.tensor([-1.7, 0.5]), atol=1e-2, rtol=1e-2))
    tc.assertTrue(np.allclose(tle.toNumpy(layer.std), tl.tensor([0.2, 1.3]), atol=1e-2, rtol=1e-2))
...
def test_mle_bias_correction(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(layer.std), tl.sqrt(tl.tensor(1.0))))

    # perform MLE
    maximum_likelihood_estimation(layer, data, bias_correction=True)
    tc.assertTrue(np.isclose(tle.toNumpy(layer.std), tl.sqrt(tl.tensor(2.0))))
...
def test_mle_edge_std_0(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.mean), tl.log(data[0])))
    tc.assertTrue(tle.toNumpy(layer.std) > 0)
...
def test_mle_edge_std_nan(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(layer.mean), tl.log(data[0])))
    tc.assertFalse(np.isnan(tle.toNumpy(layer.std)))
    tc.assertTrue(np.all(tle.toNumpy(layer.std) > 0))
...
def test_mle_nan_strategy_ignore(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.mean), tl.tensor(-1.0 / 3.0)))
    tc.assertTrue(
        np.allclose(
            tle.toNumpy(layer.std),
...
def test_weighted_mle(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(leaf.mean), tl.tensor([0.5, 1.3]), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.allclose(tle.toNumpy(leaf.std), tl.tensor([1.4, 1.7]), atol=1e-2, rtol=1e-1))
...
def test_em_step(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.mean), tl.tensor([0.3, 1.4]), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.allclose(tle.toNumpy(layer.std), tl.tensor([1.7, 0.8]), atol=1e-2, rtol=1e-1))
...
def test_em_product_of_log_normals(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.mean), tl.tensor([2.0, -2.0]), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.allclose(tle.toNumpy(layer.std), tl.tensor([1.0, 1.0]), atol=1e-2, rtol=1e-1))
...
def test_update_backend(do_for_all_backends):
    ...
    mean = tle.toNumpy(layer.mean)
    std = tle.toNumpy(layer.std)

    layer = LogNormalLayer(scope=[Scope([0]), Scope([1])])
    prod_node = ProductNode([layer])
    expectation_maximization(prod_node, tl.tensor(data), max_steps=10)
    mean_em = tle.toNumpy(layer.mean)
    std_em = tle.toNumpy(layer.std)
    ...
    for backend in backends:
        with tl.backend_context(backend):
            ...
            mean_updated = tle.toNumpy(layer_updated.mean)
            std_updated = tle.toNumpy(layer_updated.std)
            ...
            if tl.get_backend() != "pytorch":
                ...
                expectation_maximization(prod_node, tl.tensor(data), max_steps=10)
                mean_em_updated = tle.toNumpy(layer_updated.mean)
                std_em_updated = tle.toNumpy(layer_updated.std)
