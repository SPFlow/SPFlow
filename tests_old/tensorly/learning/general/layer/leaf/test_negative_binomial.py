...
from spflow.structure.spn import NegativeBinomialLayer  # , ProductNode, SumNode
...
def test_mle_edge_0(do_for_all_backends):
    ...
    tc.assertTrue(np.all(tle.toNumpy(layer.p) > 0.0))
...
def test_mle_nan_strategy_ignore(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(layer.p), tl.tensor((2 * 3) / (1 + 2 + 2 + 2 + 1 + 2))))
...
def test_em_sum_of_binomials(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.p), tl.tensor([0.3, 0.8]), atol=1e-2, rtol=1e-2))
