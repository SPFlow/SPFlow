...
from spflow.structure.spn import PoissonLayer
...
def test_mle_edge_0(do_for_all_backends):
    ...
    tc.assertTrue(np.all(tle.toNumpy(layer.l) > 0.0))
...
def test_em_step(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.l), tl.tensor([1.7, 0.5]), atol=1e-2, rtol=1e-2))
...
def test_em_product_of_poissons(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.l), tl.tensor([0.8, 1.4]), atol=1e-2, rtol=1e-1))
...
def test_em_sum_of_poissons(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(l_opt, tle.toNumpy(l_em)))
