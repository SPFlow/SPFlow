...
from spflow.structure.spn import GammaLayer  # , ProductNode, SumNode
...
def test_mle(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.alpha), tl.tensor([0.3, 1.9]), atol=1e-3, rtol=1e-2))
    tc.assertTrue(np.allclose(tle.toNumpy(layer.beta), tl.tensor([1.7, 0.7]), atol=1e-3, rtol=1e-2))
...
def test_mle_nan_strategy_ignore(do_for_all_backends):
    ...
    alpha_ignore, beta_ignore = tle.toNumpy(layer.alpha), tle.toNumpy(layer.beta)
    ...
    alpha_none, beta_none = tle.toNumpy(layer.alpha), tle.toNumpy(layer.beta)
...
def test_weighted_mle(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(leaf.alpha), tl.tensor([0.5, 1.3]), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.allclose(tle.toNumpy(leaf.beta), tl.tensor([1.4, 1.7]), atol=1e-2, rtol=1e-1))
...
def test_em_step(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.alpha), tl.tensor([0.3, 1.4]), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.allclose(tle.toNumpy(layer.beta), tl.tensor([1.7, 0.8]), atol=1e-2, rtol=1e-1))
...
def test_em_sum_of_gammas(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer.alpha), tl.tensor([1.4, 0.9]), atol=1e-2, rtol=1e-1))
    tc.assertTrue(np.allclose(tle.toNumpy(layer.beta), tl.tensor([0.8, 1.9]), atol=1e-2, rtol=1e-1))
...
def test_update_backend(do_for_all_backends):
    ...
    alpha = tle.toNumpy(tle.toNumpy(layer.alpha))
    beta = tle.toNumpy(tle.toNumpy(layer.beta))

    layer = GammaLayer(scope=[Scope([0]), Scope([1])])
    prod_node = ProductNode([layer])
    expectation_maximization(prod_node, tl.tensor(data), max_steps=10)
    alpha_em = tle.toNumpy(layer.alpha)
    beta_em = tle.toNumpy(layer.beta)
    ...
    for backend in backends:
        with tl.backend_context(backend):
            ...
            alpha_updated = tle.toNumpy(layer_updated.alpha)
            beta_updated = tle.toNumpy(layer_updated.beta)
            ...
            if tl.get_backend() != "pytorch":
                ...
                expectation_maximization(prod_node, tl.tensor(data), max_steps=10)
                alpha_em_updated = tle.toNumpy(layer_updated.alpha)
                beta_em_updated = tle.toNumpy(layer_updated.beta)
