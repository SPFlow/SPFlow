...
def test_mle_edge_std_0(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tl_toNumpy(leaf.mean), data[0]))
...
def test_em_mixture_of_gaussians(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tl_toNumpy(l1.mean), tl.tensor(2.0), atol=1e-3, rtol=1e-3))
    tc.assertTrue(np.isclose(tl_toNumpy(l1.std), tl.tensor(1.0), atol=1e-2, rtol=1e-2))
    tc.assertTrue(np.isclose(tl_toNumpy(l2.mean), tl.tensor(-2.0), atol=1e-3, rtol=1e-3))
    tc.assertTrue(np.isclose(tl_toNumpy(l2.std), tl.tensor(1.0), atol=1e-2, rtol=1e-2))
