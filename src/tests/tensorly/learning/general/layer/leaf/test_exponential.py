...
from spflow.tensorly.structure.spn import ExponentialLayer#, ProductNode, SumNode
...
def test_mle_bias_correction(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tl_toNumpy(layer.l), tl.tensor(2.0 / 3.0)))

    # perform MLE
    maximum_likelihood_estimation(layer, data, bias_correction=True)
    tc.assertTrue(np.isclose(tl_toNumpy(layer.l), tl.tensor(1.0 / 3.0)))
...
def test_mle_nan_strategy_ignore(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tl_toNumpy(layer.l), tl.tensor(3.0 / 2.7)))
...
def test_em_step(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tl_toNumpy(layer.l), tl.tensor([0.3, 1.7]), atol=1e-2, rtol=1e-2))
...
def test_em_product_of_exponentials(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tl_toNumpy(layer.l), tl.tensor([0.8, 1.4]), atol=1e-3, rtol=1e-2))
...
def test_em_sum_of_exponentials(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tl_toNumpy(layer.l), tl.tensor([0.8, 1.4]), atol=1e-2, rtol=1e-2))
