...
from spflow.tensorly.structure.spn import HadamardLayer, marginalize
...
class TestLayer(unittest.TestCase):
    def test_hadamard_layer_initialization(self):
        ...
        self.assertTrue(tl.all(l.scopes_out == [Scope([0, 1, 2, 3, 4]) for _ in range(len(l.nodes))]))
        ...
        self.assertEqual(len(l.nodes), 3)
        # make sure scopes are correct
        self.assertTrue(tl.all(l.scopes_out == [Scope([1, 3]) for _ in range(len(l.nodes))]))
        # make sure order of nodes is correct (important)
