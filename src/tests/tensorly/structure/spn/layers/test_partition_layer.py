...
from spflow.tensorly.structure.spn import PartitionLayer, marginalize
...
class TestLayer(unittest.TestCase):
    def test_partition_layer_initialization(self):
        ...
        self.assertEqual(
            len(l.nodes),
            tl.prod([len(partition) for partition in input_partitions]),
        )
        # make sure scopes are correct
        self.assertTrue(tl.all(l.scopes_out == [Scope([0, 1, 2, 3]) for _ in range(len(l.nodes))]))
