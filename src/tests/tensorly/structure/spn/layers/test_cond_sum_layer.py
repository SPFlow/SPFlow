...
from spflow.tensorly.structure.spn import CondSumLayer, marginalize
...
class TestLayer(unittest.TestCase):
    ...
    def test_retrieve_params(self):
        ...
        for node_weights in l.retrieve_params(tl.tensor([[1]]), DispatchContext()):
            self.assertTrue(tl.all(node_weights == weights))

        # one dimensional weight array
        l.set_cond_f(lambda data: {"weights": tl_squeeze(weights, 0)})
        ...
        for weights_actual, node_weights in zip(weights, l.retrieve_params(tl.tensor([[1]]), DispatchContext())):
            self.assertTrue(tl.all(node_weights == weights_actual))
        ...
        l.set_cond_f(lambda data: {"weights": tl_unsqueeze(weights, 0)})
        self.assertRaises(ValueError, l.retrieve_params, tl.tensor([[1]]), DispatchContext())

        l.set_cond_f(lambda data: {"weights": tl_unsqueeze(weights, -1)})
        self.assertRaises(ValueError, l.retrieve_params, tl.tensor([[1]]), DispatchContext())

        # ----- incorrect number of weights -----
        l.set_cond_f(lambda data: {"weights": tl.tensor([[0.3, 0.3, 0.3, 0.1], [0.5, 0.2, 0.2, 0.1]])})
        self.assertRaises(ValueError, l.retrieve_params, tl.tensor([[1]]), DispatchContext())

        l.set_cond_f(lambda data: {"weights": tl.tensor([[0.3, 0.7], [0.5, 0.5]])})
        self.assertRaises(ValueError, l.retrieve_params, tl.tensor([[1]]), DispatchContext())

        # ----- weights not summing up to one per row -----
        l.set_cond_f(lambda data: {"weights": tl.tensor([[0.3, 0.3, 0.4], [0.5, 0.7, 0.3], [0.1, 0.7, 0.2]])})
        self.assertRaises(ValueError, l.retrieve_params, tl.tensor([[1]]), DispatchContext())

        # ----- non-positive weights -----
        l.set_cond_f(lambda data: {"weights": tl.tensor([[0.3, 0.3, 0.4], [0.5, 0.0, 0.5], [0.1, 0.7, 0.2]])})
        self.assertRaises(ValueError, l.retrieve_params, tl.tensor([[1]]), DispatchContext())
