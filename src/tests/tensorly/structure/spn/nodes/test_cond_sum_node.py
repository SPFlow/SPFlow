...
from spflow.tensorly.structure.spn import CondSumNode, ProductNode, marginalize
...
class TestSumNode(unittest.TestCase):
    ...
    def test_retrieve_params(self):
        ...
        sum_node.set_cond_f(lambda data: {"weights": [0.5, 0.5]})
        self.assertTrue(tl.all(sum_node.retrieve_params(tl.tensor([[1.0]]), DispatchContext()) == tl.tensor([0.5, 0.5])))
        # weights as numpy array
        sum_node.set_cond_f(lambda data: {"weights": tl.tensor([0.5, 0.5])})
        self.assertTrue(tl.all(sum_node.retrieve_params(tl.tensor([[1.0]]), DispatchContext()) == tl.tensor([0.5, 0.5])))
