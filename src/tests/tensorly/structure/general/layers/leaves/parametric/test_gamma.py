...
class TestLayer(unittest.TestCase):
    def test_layer_initialization_1(self):
        ...
        for node, node_alpha, node_beta in zip(l.nodes, alpha_values, beta_values):
            self.assertTrue(tl.all(node.alpha == node_alpha))
            self.assertTrue(tl.all(node.beta == node_beta))
        ...
        for node in l.nodes:
            self.assertTrue(tl.all(node.alpha == alpha_value))
            self.assertTrue(tl.all(node.beta == beta_value))
        ...
        l = GammaLayer(
            scope=Scope([1]),
            n_nodes=3,
            alpha=tl.tensor(alpha_values),
            beta=tl.tensor(beta_values),
        )
        ...
        self.assertRaises(
            ...
            GammaLayer,
            Scope([0]),
            tl.tensor(alpha_values),
            tl.tensor(beta_values[:-1]),
            n_nodes=3,
        )
        self.assertRaises(
            ValueError,
            GammaLayer,
            Scope([0]),
            tl.tensor(alpha_values[:-1]),
            tl.tensor(beta_values),
            n_nodes=3,
        )
        # wrong number of dimensions (nested list)
        self.assertRaises(
            ...
            GammaLayer,
            Scope([0]),
            tl.tensor([alpha_values for _ in range(3)]),
            beta_values,
            n_nodes=3,
        )
        self.assertRaises(
            ...
            Scope([0]),
            alpha_values,
            tl.tensor([beta_values for _ in range(3)]),
            n_nodes=3,
        )
    ...
    def test_layer_structural_marginalization(self):
        ...
        self.assertTrue(tl.all(l.alpha == l_marg.alpha))
        self.assertTrue(tl.all(l.beta == l_marg.beta))
        ...
        self.assertEqual(l_marg.alpha, tl.tensor([1.3]))
        self.assertEqual(l_marg.beta, tl.tensor([0.3]))
