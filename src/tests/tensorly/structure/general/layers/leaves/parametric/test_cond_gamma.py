...
from spflow.tensorly.structure.spn import CondGamma, CondGammaLayer, marginalize
...
class TestLayer(unittest.TestCase):
    ...
    def test_retrieve_params(self):
        ...
        for alpha_node, beta_node in zip(*l.retrieve_params(tl.tensor([[1.0]]), DispatchContext())):
        ...
        l.set_cond_f(
            lambda data: {
                "alpha": tl.tensor(alpha_values[:-1]),
                "beta": tl.tensor(beta_values),
            }
        )
        self.assertRaises(ValueError, l.retrieve_params, tl.tensor([[1]]), DispatchContext())
        ...
        l.set_cond_f(
            lambda data: {
                "alpha": tl.tensor([alpha_values for _ in range(3)]),
                "beta": tl.tensor(beta_values),
            }
        )
        self.assertRaises(ValueError, l.retrieve_params, tl.tensor([[1]]), DispatchContext())
        l.set_cond_f(
            lambda data: {
                "alpha": tl.tensor(alpha_values),
                "beta": tl.tensor([beta_values for _ in range(3)]),
            }
        )
        self.assertRaises(ValueError, l.retrieve_params, tl.tensor([[1]]), DispatchContext())
