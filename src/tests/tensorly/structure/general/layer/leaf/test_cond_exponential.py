...
from spflow.base.structure.general.node.leaf.cond_exponential import CondExponential as CondExponentialBase
from spflow.base.structure.general.layer.leaf.cond_exponential import CondExponentialLayer as CondExponentialLayerBase
from spflow.meta.data import FeatureContext, FeatureTypes, Scope
from spflow.meta.dispatch import DispatchContext
from spflow.torch.structure import marginalize, toBase, toTorch
from spflow.torch.structure.general.node.leaf.cond_exponential import CondExponential as CondExponentialTorch
from spflow.torch.structure.general.layer.leaf.cond_exponential import CondExponentialLayer as CondExponentialLayerTorch
...
def test_change_dtype(do_for_all_backends):
    ...
    l_value = 0.73
    exponential_default = CondExponentialLayer(scope=Scope([1], [0]), n_nodes=3, cond_f=lambda data: {"l": l_value})

    exponential_updated = CondExponentialLayer(scope=Scope([1], [0]), n_nodes=3, cond_f=lambda data: {"l": l_value})
    tc.assertTrue(exponential_default.dtype == tl.float32)
    l = exponential_default.retrieve_params(np.array([[1.0]]), DispatchContext())
    tc.assertTrue(l.dtype == tl.float32)

    # change to float64 model
    exponential_updated.to_dtype(tl.float64)
    l_up = exponential_updated.retrieve_params(np.array([[1.0]]), DispatchContext())
