...
from spflow.tensorly.structure.general.nodes.leaves.parametric.cond_negative_binomial import (
...
class TestNegativeBinomial(unittest.TestCase):
    def test_initialization(self):
        ...
        # n = inf and n = nan
        self.assertRaises(Exception, CondNegativeBinomial, Scope([0], [1]), tl.inf)
        self.assertRaises(Exception, CondNegativeBinomial, Scope([0], [1]), tl.nan)
    ...
    def test_retrieve_params(self):
        ...
        self.assertTrue(negative_binomial.retrieve_params(tl.tensor([[1.0]]), DispatchContext()) == 1.0)
        # p = 0
        ...
        # p = inf and p = nan
        negative_binomial.set_cond_f(lambda data: {"p": tl.inf})
        self.assertRaises(
            ValueError,
            negative_binomial.retrieve_params,
            tl.tensor([[1.0]]),
        ...
        negative_binomial.set_cond_f(lambda data: {"p": tl.nan})
        self.assertRaises(
            ValueError,
            negative_binomial.retrieve_params,
            tl.tensor([[1.0]]),
