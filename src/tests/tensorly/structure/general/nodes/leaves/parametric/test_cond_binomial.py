...
class TestCondBinomial(unittest.TestCase):
    def test_initialization(self):
        ...
        # n = inf and n = nan
        self.assertRaises(Exception, CondBinomial, Scope([0], [1]), tl.inf)
        self.assertRaises(Exception, CondBinomial, Scope([0], [1]), tl.nan)
    ...
    def test_retrieve_params(self):
        ...
        self.assertTrue(binomial.retrieve_params(tl.tensor([[1.0]]), DispatchContext()) == 0.0)
        # p = 1
        binomial.set_cond_f(lambda data: {"p": 1.0})
        self.assertTrue(binomial.retrieve_params(tl.tensor([[1.0]]), DispatchContext()) == 1.0)
        # p < 0 and p > 1
        ...
        # p = inf and p = nan
        binomial.set_cond_f(lambda data: {"p": tl.inf})
        self.assertRaises(
            ValueError,
            binomial.retrieve_params,
            tl.tensor([[1.0]]),
        ...
        binomial.set_cond_f(lambda data: {"p": tl.nan})
        self.assertRaises(
            ValueError,
            binomial.retrieve_params,
            tl.tensor([[1.0]]),
