...
class TestCondBernoulli(unittest.TestCase):
    ...
    def test_retrieve_params(self):
        ...
        self.assertTrue(bernoulli.retrieve_params(tl.tensor([[1.0]]), DispatchContext()) == 0.0)
        # p = 1
        bernoulli.set_cond_f(lambda data: {"p": 1.0})
        self.assertTrue(bernoulli.retrieve_params(tl.tensor([[1.0]]), DispatchContext()) == 1.0)
        # p < 0 and p > 1
        ...
        # p = inf and p = nan
        bernoulli.set_cond_f(lambda data: {"p": tl.inf})
        self.assertRaises(
            ValueError,
            bernoulli.retrieve_params,
            tl.tensor([[1.0]]),
        ...
        bernoulli.set_cond_f(lambda data: {"p": tl.nan})
        self.assertRaises(
            ValueError,
            bernoulli.retrieve_params,
            tl.tensor([[1.0]]),
