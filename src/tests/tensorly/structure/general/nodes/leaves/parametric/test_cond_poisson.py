...
from spflow.tensorly.structure.general.nodes.leaves.parametric.cond_poisson import (
    CondPoisson,
)
...
class TestPoisson(unittest.TestCase):
    ...
    def test_retrieve_params(self):
        ...
        self.assertTrue(poisson.retrieve_params(tl.tensor([[1.0]]), DispatchContext()) == 0.0)
        # l > 0
        poisson.set_cond_f(lambda data: {"l": np.nextafter(0.0, 1.0)})
        self.assertTrue(poisson.retrieve_params(tl.tensor([[1.0]]), DispatchContext()) == np.nextafter(0.0, 1.0))
        # l = -inf and l = inf
        poisson.set_cond_f(lambda data: {"l": -tl.inf})
        self.assertRaises(
            ValueError,
        ...
        # l = nan
        poisson.set_cond_f(lambda data: {"l": tl.nan})
        self.assertRaises(
            ValueError,
            poisson.retrieve_params,
            tl.tensor([[1.0]]),
