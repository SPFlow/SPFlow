...
from spflow.tensorly.structure.general.node.leaf.general_geometric import Geometric
...
def test_layer_sampling(do_for_all_backends):
    ...
    layer = GeometricLayer(scope=[Scope([0]), Scope([1]), Scope([0])], p=[0.2, 0.8, 0.2])

    nodes = [
        Geometric(Scope([0]), p=0.2),
        Geometric(Scope([1]), p=0.8),
        Geometric(Scope([0]), p=0.2),
    ]
    ...
    expected_mean = tl.tensor([1.0 / 0.2, 1.0 / 0.8])
