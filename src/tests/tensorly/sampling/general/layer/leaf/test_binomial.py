...
from spflow.tensorly.structure.general.node.leaf.general_binomial import Binomial
from spflow.torch.structure.general.node.leaf.bernoulli import updateBackend
from spflow.tensorly.utils.helper_functions import tl_isnan, tl_toNumpy
...
def test_layer_sampling(do_for_all_backends):
    ...
    expected_mean = tl.tensor([3 * 0.2, 2 * 0.8])
...
def test_update_backend(do_for_all_backends):
    ...
    for backend in backends:
        with tl.backend_context(backend):
            ...
            binomial_updated = updateBackend(layer)
