...
from spflow.tensorly.structure.general.layer.leaf.general_cond_gaussian import CondGaussianLayer
...
def test_layer_sampling(do_for_all_backends):
    ...
    nodes = [
        CondGaussian(Scope([0], [2]), cond_f=lambda data: {"mean": 0.2, "std": 0.01}),
        CondGaussian(Scope([1], [2]), cond_f=lambda data: {"mean": 1.8, "std": 0.05}),
        CondGaussian(Scope([0], [2]), cond_f=lambda data: {"mean": 0.2, "std": 0.01}),
    ]
