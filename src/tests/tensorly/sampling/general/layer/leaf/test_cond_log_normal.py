...
from spflow.tensorly.structure.general.node.leaf.general_cond_log_normal import CondLogNormal
...
def test_layer_sampling(do_for_all_backends):
    ...
    nodes = [
        CondLogNormal(Scope([0], [2]), cond_f=lambda data: {"mean": 0.2, "std": 0.01}),
        CondLogNormal(Scope([1], [2]), cond_f=lambda data: {"mean": 1.8, "std": 0.05}),
        CondLogNormal(Scope([0], [2]), cond_f=lambda data: {"mean": 0.2, "std": 0.01}),
    ]
