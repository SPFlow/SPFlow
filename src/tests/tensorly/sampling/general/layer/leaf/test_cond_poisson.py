...
from spflow.tensorly.structure.general.node.leaf.general_cond_poisson import CondPoisson
...
def test_layer_sampling(do_for_all_backends):
    ...
    nodes = [
        ...
        CondPoisson(Scope([1], [2]), cond_f=lambda data: {"l": 1.8}),
        CondPoisson(Scope([0], [2]), cond_f=lambda data: {"l": 0.2}),
    ]
    ...
    expected_mean = tl.tensor([0.2, 1.8])
