...
from spflow.tensorly.structure.general.node.leaf.general_cond_multivariate_gaussian import CondMultivariateGaussian
...
def test_conditional_sampling(do_for_all_backends):
    ...
    for i in range(10):
        ...
        marg_cov_inv = tl_inv(cov[tuple(tl.tensor(tensor, dtype=tl.int64) for tensor in tl_ix_(cond_rvs, cond_rvs, indexing="ij"))])
        cond_cov = cov[tuple(tl.tensor(tensor, dtype=tl.int64) for tensor in tl_ix_(cond_rvs, non_cond_rvs, indexing="ij"))]

        mean_exact = mean[0, ~cond_mask] + (
            (data[i * 10000, cond_mask] - mean[:, cond_mask]) @ (tl.tensor(marg_cov_inv, dtype=tl.float32) @ cond_cov)
        )
        cov_exact = cov[tuple(tl.tensor(tensor, dtype=tl.int64) for tensor in tl_ix_(non_cond_rvs, non_cond_rvs, indexing="ij"))] - (
