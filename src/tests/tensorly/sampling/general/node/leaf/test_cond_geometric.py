...
from spflow.tensorly.structure.general.node.leaf.general_cond_geometric import CondGeometric
...
def test_sampling_3(do_for_all_backends):
    ...
    samples = sample(geometric, 1000)
    tc.assertTrue(np.isclose(samples.mean(), tl.tensor(1.0 / 0.8), rtol=0.1))
...
def test_update_backend(do_for_all_backends):
    ...
    for backend in backends:
        with tl.backend_context(backend):
            bernoulli_updated = updateBackend(geometric)
...
def test_change_dtype(do_for_all_backends):
    ...
    layer = CondGeometric(Scope([0], [1]), cond_f=lambda data: {"p": 1.0})
    # get samples
