...
from spflow.base.inference.layers.leaves.parametric.cond_log_normal import log_likelihood
from spflow.base.structure.nodes.leaves.parametric.cond_log_normal import CondLogNormal
from spflow.base.inference.nodes.leaves.parametric.cond_log_normal import log_likelihood
...
class TestNode(unittest.TestCase):
    ...
    def test_layer_likelihood_1(self):
        ...
        log_normal_layer = CondLogNormalLayer(scope=Scope([0]), cond_f=lambda data: {'mean': [0.8, 0.3], 'std': [1.3, 0.4]}, n_nodes=2)
        s1 = SPNSumNode(children=[log_normal_layer], weights=[0.3, 0.7])

        log_normal_nodes = [CondLogNormal(Scope([0]), cond_f=lambda data: {'mean': 0.8, 'std': 1.3}), CondLogNormal(Scope([0]), cond_f=lambda data: {'mean': 0.3, 'std': 0.4})]
    ...
    def test_layer_likelihood_2(self):
        ...
        log_normal_layer = CondLogNormalLayer(scope=[Scope([0]), Scope([1])], cond_f=lambda data: {'mean': [0.8, 0.3], 'std': [1.3, 0.4]})
        p1 = SPNProductNode(children=[log_normal_layer])

        log_normal_nodes = [CondLogNormal(Scope([0]), cond_f=lambda data: {'mean': 0.8, 'std': 1.3}), CondLogNormal(Scope([1]), cond_f=lambda data: {'mean': 0.3, 'std': 0.4})]
