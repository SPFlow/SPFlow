...
class TestNode(unittest.TestCase):
    ...
    def test_layer_likelihood_1(self):
        ...
        geometric_layer = CondGeometricLayer(scope=Scope([0]), cond_f=lambda data: {'p': [0.2, 0.5]}, n_nodes=2)
        s1 = SPNSumNode(children=[geometric_layer], weights=[0.3, 0.7])

        geometric_nodes = [CondGeometric(Scope([0]), cond_f=lambda data: {'p': 0.2}), CondGeometric(Scope([0]), cond_f=lambda data: {'p': 0.5})]
    ...
    def test_layer_likelihood_2(self):
        ...
        geometric_layer = CondGeometricLayer(scope=[Scope([0]), Scope([1])], cond_f=lambda data: {'p': [0.2, 0.5]})
        p1 = SPNProductNode(children=[geometric_layer])

        geometric_nodes = [CondGeometric(Scope([0]), cond_f=lambda data: {'p': 0.2}), CondGeometric(Scope([1]), cond_f=lambda data: {'p': 0.5})]
