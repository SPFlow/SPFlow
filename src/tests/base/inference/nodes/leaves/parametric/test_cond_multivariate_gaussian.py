...
class TestMultivariateGaussian(unittest.TestCase):
    def test_likelihood_no_mean(self):
        ...
        multivariate_gaussian = CondMultivariateGaussian(Scope([0,1]), cond_f=lambda data: {'cov': np.eye(2)})
        self.assertRaises(KeyError, log_likelihood, multivariate_gaussian, np.array([[0], [1]]))
    
    def test_likelihood_no_cov(self):

        multivariate_gaussian = CondMultivariateGaussian(Scope([0,1]), cond_f=lambda data: {'mean': np.zeros(2)})
        self.assertRaises(KeyError, log_likelihood, multivariate_gaussian, np.array([[0, 0], [1, 1]]))

    def test_likelihood_no_mean_std(self):

        multivariate_gaussian = CondMultivariateGaussian(Scope([0,1]))
        self.assertRaises(ValueError, log_likelihood, multivariate_gaussian, np.array([[0, 0], [1, 1]]))

    def test_likelihood_module_cond_f(self):

        cond_f = lambda data: {'mean': np.zeros(2), 'cov': np.eye(2)}

        multivariate_gaussian = CondMultivariateGaussian(Scope([0,1]), cond_f=cond_f)
    ...
    def test_likelihood_args(self):
        ...
        dispatch_ctx.args[multivariate_gaussian] = {'mean': np.zeros(2), 'cov': np.eye(2)}
    ...
    def test_likelihood_mean_none(self):
        ...
        multivariate_gaussian = CondMultivariateGaussian(Scope([0, 1]), cond_f=lambda data: {'mean': None, 'cov': np.eye(2)})
        data = np.stack([np.zeros(2), np.ones(2)], axis=0)
    ...
    def test_likelihood_cov_none(self):
        ...
        # dummy distribution and data
        multivariate_gaussian = CondMultivariateGaussian(Scope([0, 1]), cond_f=lambda data: {'mean': np.zeros(2), 'cov': None})
        data = np.stack([np.zeros(2), np.ones(2)], axis=0)
    ...
    def test_likelihood_marginalization(self):
        ...
        multivariate_gaussian = CondMultivariateGaussian(Scope([0, 1]), cond_f=lambda data: {'mean': np.zeros(2), 'cov': np.eye(2)})
