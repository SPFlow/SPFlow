...
class TestCondGaussian(unittest.TestCase):
    def test_likelihood_no_mean(self):
        ...
        gaussian = CondGaussian(Scope([0]), cond_f=lambda data: {'std': 1.0})
        self.assertRaises(KeyError, log_likelihood, gaussian, np.array([[0], [1]]))
    
    def test_likelihood_no_std(self):

        gaussian = CondGaussian(Scope([0]), cond_f=lambda data: {'mean': 0.0})
        self.assertRaises(KeyError, log_likelihood, gaussian, np.array([[0], [1]]))
    ...
    def test_likelihood_1(self):
        ...
        gaussian = CondGaussian(Scope([0]), cond_f=lambda data: {'mean': mean, 'std': math.sqrt(var)})
    ...
    def test_likelihood_mean_none(self):
        ...
        gaussian = CondGaussian(Scope([0]), cond_f=lambda data: {'mean': None, 'std': 1.0})
    ...
    def test_likelihood_std_none(self):
        ...
        # dummy distribution and data
        gaussian = CondGaussian(Scope([0]), cond_f=lambda data: {'mean': 0.0, 'std': None})
        data = np.random.randn(1, 3)
