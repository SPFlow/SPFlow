...
class TestCondLogNormal(unittest.TestCase):
    def test_likelihood_no_mean(self):
        ...
        log_normal = CondLogNormal(Scope([0]), cond_f=lambda data: {'std': 1.0})
        self.assertRaises(KeyError, log_likelihood, log_normal, np.array([[0], [1]]))
    
    def test_likelihood_no_std(self):

        log_normal = CondLogNormal(Scope([0]), cond_f=lambda data: {'mean': 0.0})
        self.assertRaises(KeyError, log_likelihood, log_normal, np.array([[0], [1]]))
    ...
    def test_likelihood_1(self):
        ...
        # ----- configuration 1 -----
        log_normal = CondLogNormal(Scope([0]), cond_f=lambda data: {'mean': 0.0, 'std': 0.25})
    ...
    def test_likelihood_2(self):
        ...
        # ----- configuration 2 -----
        log_normal = CondLogNormal(Scope([0]), cond_f=lambda data: {'mean': 0.0, 'std': 0.5})
    ...
    def test_likelihood_mean_none(self):
        ...
        log_normal = CondLogNormal(Scope([0]), cond_f=lambda data: {'mean': None, 'std': 1.0})
        data = np.array([[0.5], [1.0], [1.5]])
    ...
    def test_likelihood_std_none(self):
        ...
        # dummy distribution and data
        log_normal = CondLogNormal(Scope([0]), cond_f=lambda data: {'mean': 0.0, 'std': None})
        data = np.array([[0.5], [1.0], [1.5]])
