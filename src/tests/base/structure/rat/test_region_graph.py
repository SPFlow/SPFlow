...
from spflow.base.structure.rat.region_graph import Region, Partition, RegionGraph, random_region_graph
import unittest


def get_region_graph_properties(region_graph: RegionGraph) -> Tuple[int, int, int, int]:
    ...
    return correct_scopes, max_depth, n_internal_regions, n_partitions, n_leaf_regions
...
class TestRegionGraph(unittest.TestCase):
    ...
    def test_random_region_graph_none(self):
        ...
        self.assertRaises(AttributeError, random_region_graph, scope=None, depth=depth, replicas=replicas, n_splits=n_splits)
    ...
    def test_random_region_graph_structure_1(self):
        ...
        region_graph = random_region_graph(scope=Scope(random_variables), depth=depth, replicas=replicas, n_splits=n_splits)
        
        # get region graph properties
        correct_scopes, max_depth, n_regions, n_partitions, n_leaf_regions = get_region_graph_properties(region_graph)
    ...
    def test_region_graph_structure_4(self):
        ...
        region_graph = random_region_graph(
            scope=Scope(random_variables), depth=depth, replicas=replicas, n_splits=n_splits
        )
