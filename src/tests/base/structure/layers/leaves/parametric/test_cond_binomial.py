from spflow.base.structure.layers.leaves.parametric.cond_binomial import CondBinomialLayer, marginalize
...
class TestLayer(unittest.TestCase):
    def test_layer_initialization_1(self):
        ...
        # ---- different scopes -----
        l = CondBinomialLayer(scope=[Scope([0]), Scope([1]), Scope([2])], n=5, n_nodes=3)
        for node, node_scope in zip(l.nodes, l.scopes_out):
            self.assertEqual(node.n, 5)
            self.assertEqual(node.scope, node_scope)

        # ----- invalid number of nodes -----
        self.assertRaises(ValueError, CondBinomialLayer, Scope([0]), 2, n_nodes=0)
        ...
        self.assertRaises(ValueError, CondBinomialLayer, Scope([0]), n=[2, 5], n_nodes=2)
        ...
        CondBinomialLayer(Scope([0]), n=3, n_nodes=2, cond_f=[lambda data: {'p': 0.5}, lambda data: {'p': 0.5}])
        self.assertRaises(ValueError, CondBinomialLayer, Scope([0]), n=3, n_nodes=2, cond_f=[lambda data: {'p': 0.5}])
    ...
    def test_retrieve_params(self):
        ...
        l = CondBinomialLayer(scope=[Scope([0]),Scope([1]),Scope([2])], n_nodes=3, n=n_values, cond_f=lambda data: {'p': p_values})
