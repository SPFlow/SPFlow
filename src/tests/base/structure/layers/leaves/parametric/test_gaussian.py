...
class TestLayer(unittest.TestCase):
    def test_layer_initialization_1(self):
        ...
        # wrong number of dimensions (nested list)
        self.assertRaises(ValueError, GaussianLayer, Scope([0]), mean_values, np.array([std_values for _ in range(3)]), n_nodes=3)
        self.assertRaises(ValueError, GaussianLayer, Scope([0]), np.array([mean_values for _ in range(3)]), std_values, n_nodes=3)
