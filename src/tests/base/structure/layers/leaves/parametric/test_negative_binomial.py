...
class TestLayer(unittest.TestCase):
    def test_layer_initialization_1(self):
        ...
        l = NegativeBinomialLayer(scope=Scope([1]), n_nodes=3, n=n_values, p=p_values)
        ...
        self.assertRaises(ValueError, NegativeBinomialLayer, Scope([0]), n_values, p_values[:-1], n_nodes=3)
        # wrong number of dimensions (nested list)
        self.assertRaises(ValueError, NegativeBinomialLayer, Scope([0]), [n_values for _ in range(3)], [p_values for _ in range(3)], n_nodes=3)

        # ----- numpy parameter values -----

        l = NegativeBinomialLayer(scope=Scope([1]), n_nodes=3, n=np.array(n_values), p=np.array(p_values))
        ...
        # wrong number of values
        self.assertRaises(ValueError, NegativeBinomialLayer, Scope([0]), np.array(n_values), np.array(p_values[:-1]), n_nodes=3)
        self.assertRaises(ValueError, NegativeBinomialLayer, Scope([0]), np.array([n_values for _ in range(3)]), np.array([p_values for _ in range(3)]), n_nodes=3)
    ...
    def test_layer_structural_marginalization(self):
        ...
        l = NegativeBinomialLayer(scope=Scope([1]), n=[2, 6], p=[0.5, 0.3], n_nodes=2)
