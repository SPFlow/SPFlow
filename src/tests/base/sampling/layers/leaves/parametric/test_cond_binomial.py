...
from spflow.base.inference.layers.leaves.parametric.cond_binomial import log_likelihood
from spflow.base.sampling.layers.leaves.parametric.cond_binomial import sample
...
from spflow.base.sampling.nodes.leaves.parametric.cond_binomial import sample
...
class TestNode(unittest.TestCase):
    def test_sampling_1(self):
        ...
        binomial_layer = CondBinomialLayer(scope=Scope([0]), n=3, cond_f=lambda data: {'p': [0.8, 0.3]}, n_nodes=2)
        s1 = SPNSumNode(children=[binomial_layer], weights=[0.3, 0.7])

        binomial_nodes = [CondBinomial(Scope([0]), n=3, cond_f=lambda data: {'p': 0.8}), CondBinomial(Scope([0]), n=3, cond_f=lambda data: {'p': 0.3})]
    ...
    def test_sampling_2(self):
        ...
        binomial_layer = CondBinomialLayer(scope=[Scope([0]), Scope([1])], n=[3, 5], cond_f=lambda data: {'p': [0.8, 0.3]})
        p1 = SPNProductNode(children=[binomial_layer])

        binomial_nodes = [CondBinomial(Scope([0]), n=3, cond_f=lambda data: {'p': 0.8}), CondBinomial(Scope([1]), n=5, cond_f=lambda data: {'p': 0.3})]
