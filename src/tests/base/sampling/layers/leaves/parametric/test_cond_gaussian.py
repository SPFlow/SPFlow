...
from spflow.base.inference.layers.leaves.parametric.cond_gaussian import log_likelihood
...
from spflow.base.inference.nodes.leaves.parametric.cond_gaussian import log_likelihood
...
class TestNode(unittest.TestCase):
    def test_sampling_1(self):
        ...
        gaussian_layer = CondGaussianLayer(scope=Scope([0]), cond_f=lambda data: {'mean': [0.8, 0.3], 'std': [1.3, 0.4]}, n_nodes=2)
        s1 = SPNSumNode(children=[gaussian_layer], weights=[0.3, 0.7])

        gaussian_nodes = [CondGaussian(Scope([0]), cond_f=lambda data: {'mean': 0.8, 'std': 1.3}), CondGaussian(Scope([0]), cond_f=lambda data: {'mean': 0.3, 'std': 0.4})]
    ...
    def test_sampling_2(self):
        ...
        gaussian_layer = CondGaussianLayer(scope=[Scope([0]), Scope([1])], cond_f=lambda data: {'mean': [0.8, 0.3], 'std': [1.3, 0.4]})
        p1 = SPNProductNode(children=[gaussian_layer])

        gaussian_nodes = [CondGaussian(Scope([0]), cond_f=lambda data: {'mean': 0.8, 'std': 1.3}), CondGaussian(Scope([1]), cond_f=lambda data: {'mean': 0.3, 'std': 0.4})]
