...
from spflow.base.sampling.layers.leaves.parametric.cond_exponential import sample
...
class TestNode(unittest.TestCase):
    def test_sampling_1(self):
        ...
        exponential_layer = CondExponentialLayer(scope=Scope([0]), cond_f=lambda data: {'l': [0.8, 0.3]}, n_nodes=2)
        s1 = SPNSumNode(children=[exponential_layer], weights=[0.3, 0.7])

        exponential_nodes = [CondExponential(Scope([0]), cond_f=lambda data: {'l': 0.8}), CondExponential(Scope([0]), cond_f=lambda data: {'l': 0.3})]
    ...
    def test_sampling_2(self):
        ...
        exponential_layer = CondExponentialLayer(scope=[Scope([0]), Scope([1])], cond_f=lambda data: {'l': [0.8, 0.3]})
        p1 = SPNProductNode(children=[exponential_layer])

        exponential_nodes = [CondExponential(Scope([0]), cond_f=lambda data: {'l': 0.8}), CondExponential(Scope([1]), cond_f=lambda data: {'l': 0.3})]
