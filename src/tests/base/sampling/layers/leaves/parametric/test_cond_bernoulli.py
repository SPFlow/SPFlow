...
from spflow.base.inference.layers.leaves.parametric.cond_bernoulli import log_likelihood
...
from spflow.base.inference.nodes.leaves.parametric.cond_bernoulli import log_likelihood
...
class TestNode(unittest.TestCase):
    def test_sampling_1(self):
        ...
        bernoulli_layer = CondBernoulliLayer(scope=Scope([0]), cond_f=lambda data: {'p': [0.8, 0.3]}, n_nodes=2)
        s1 = SPNSumNode(children=[bernoulli_layer], weights=[0.3, 0.7])

        bernoulli_nodes = [CondBernoulli(Scope([0]), cond_f=lambda data: {'p': 0.8}), CondBernoulli(Scope([0]), cond_f=lambda data: {'p': 0.3})]
    ...
    def test_sampling_2(self):
        ...
        bernoulli_layer = CondBernoulliLayer(scope=[Scope([0]), Scope([1])], cond_f=lambda data: {'p': [0.8, 0.3]})
        p1 = SPNProductNode(children=[bernoulli_layer])

        bernoulli_nodes = [CondBernoulli(Scope([0]), cond_f=lambda data: {'p': 0.8}), CondBernoulli(Scope([1]), cond_f=lambda data: {'p': 0.3})]
    ...
    def test_sampling_3(self):
        ...
        bernoulli_layer = CondBernoulliLayer(scope=[Scope([0]), Scope([0])], cond_f=lambda data: {'p': [0.8, 0.3]})
