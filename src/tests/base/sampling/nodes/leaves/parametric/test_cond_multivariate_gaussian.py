...
class TestCondMultivariateGaussian(unittest.TestCase):
    def test_joint_sampling(self):
        ...
        # create distribution
        mv = CondMultivariateGaussian(Scope([0,1,2,3,4]), cond_f=lambda data: {'mean': mean, 'cov': cov})
    ...
    def test_sampling(self):
        ...
        mv = CondMultivariateGaussian(Scope([0,1]), cond_f=lambda data: {'mean': np.zeros(2), 'cov': np.eye(2)})
