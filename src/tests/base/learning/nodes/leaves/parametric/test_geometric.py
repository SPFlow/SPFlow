...
from spflow.base.learning.nodes.leaves.parametric.geometric import maximum_likelihood_estimation
...
class TestNode(unittest.TestCase):
    ...
    def test_mle_invalid_support(self):
        ...
        self.assertRaises(ValueError, maximum_likelihood_estimation, leaf, np.array([[0]]), bias_correction=True)

    def test_mle_nan_strategy_none(self):

        leaf = Geometric(Scope([0]))
        self.assertRaises(ValueError, maximum_likelihood_estimation, leaf, np.array([[np.nan], [1], [4], [3]]), nan_strategy=None)
    
    def test_mle_nan_strategy_ignore(self):

        leaf = Geometric(Scope([0]))
        maximum_likelihood_estimation(leaf, np.array([[np.nan], [1], [4], [3]]), nan_strategy='ignore', bias_correction=False)
    ...
    def test_mle_nan_strategy_invalid(self):
        ...
        leaf = Geometric(Scope([0]))
        self.assertRaises(ValueError, maximum_likelihood_estimation, leaf, np.array([[np.nan], [1], [4], [3]]), nan_strategy='invalid_string')
