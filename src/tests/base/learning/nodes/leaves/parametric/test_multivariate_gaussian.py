...
from spflow.base.learning.nodes.leaves.parametric.multivariate_gaussian import maximum_likelihood_estimation
...
class TestNode(unittest.TestCase):
    def test_mle_1(self):
        ...
        self.assertTrue(np.allclose(leaf.cov, np.array([[1.0, 0.25], [0.25, 0.5]]), atol=1e-2, rtol=1e-2))
    ...
    def test_mle_invalid_support(self):
        ...
        self.assertRaises(ValueError, maximum_likelihood_estimation, leaf, np.array([[np.inf, 0.0]]), bias_correction=True)

    def test_mle_nan_strategy_none(self):

        leaf = MultivariateGaussian(Scope([0,1]))
        self.assertRaises(ValueError, maximum_likelihood_estimation, leaf, np.array([[np.nan, 0.0], [-2.3, 0.1], [-1.8, 1.9], [0.9, 0.7]]), nan_strategy=None)
    ...
    def test_mle_nan_strategy_ignore(self):
        ...
        maximum_likelihood_estimation(leaf, np.exp(np.array([[np.nan, np.nan], [-2.3, 0.1], [-1.8, 1.9], [0.9, 0.7]])), nan_strategy='ignore', bias_correction=False)
        mean_ignore, cov_ignore = leaf.mean, leaf.cov

        maximum_likelihood_estimation(leaf, np.exp(np.array([[-2.3, 0.1], [-1.8, 1.9], [0.9, 0.7]])), nan_strategy=None, bias_correction=False)
    ...
    def test_mle_nan_strategy_invalid(self):
        ...
        leaf = MultivariateGaussian(Scope([0,1]))
        self.assertRaises(ValueError, maximum_likelihood_estimation, leaf, np.array([[np.nan, 0.0], [1, 0.1], [1.9, -0.2]]), nan_strategy='invalid_string')
        self.assertRaises(ValueError, maximum_likelihood_estimation, leaf, np.array([[np.nan, 0.0], [1, 0.1], [1.9, -0.2]]), nan_strategy=1)
