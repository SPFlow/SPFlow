...
from spflow.torch.sampling.nodes.leaves.parametric.cond_multivariate_gaussian import sample
...
from spflow.torch.sampling.layers.leaves.parametric.cond_multivariate_gaussian import sample
...
class TestNode(unittest.TestCase):
    ...
    def test_layer_sampling(self):
        ...
        layer = CondMultivariateGaussianLayer(scope=[Scope([0,1]), Scope([2,3]), Scope([0,1])], cond_f=lambda data: {'mean': [[0.8, 0.3], [0.2, -0.1], [0.8, 0.3]], 'cov': [[[0.13, 0.08], [0.08, 0.05]], [[0.17, 0.054], [0.054, 0.0296]], [[0.13, 0.08], [0.08, 0.05]]]})

        nodes = [
                CondMultivariateGaussian(Scope([0,1]), cond_f=lambda data: {'mean': [0.8, 0.3], 'cov': [[0.13, 0.08], [0.08, 0.05]]}),
                CondMultivariateGaussian(Scope([2,3]), cond_f=lambda data: {'mean': [0.2, -0.1], 'cov': [[0.17, 0.054], [0.054, 0.0296]]}),
                CondMultivariateGaussian(Scope([0,1]), cond_f=lambda data: {'mean': [0.8, 0.3], 'cov': [[0.13, 0.08], [0.08, 0.05]]}),
        ]
