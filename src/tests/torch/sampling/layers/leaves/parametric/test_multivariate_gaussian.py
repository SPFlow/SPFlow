...
from spflow.torch.sampling.layers.leaves.parametric.multivariate_gaussian import sample
...
class TestNode(unittest.TestCase):
    ...
    def test_layer_sampling(self):
        ...
        layer = MultivariateGaussianLayer(scope=[Scope([0,1]), Scope([2,3]), Scope([0,1])], mean=[[0.8, 0.3], [0.2, -0.1], [0.8, 0.3]], cov=[[[0.13, 0.08], [0.08, 0.05]], [[0.17, 0.054], [0.054, 0.0296]], [[0.13, 0.08], [0.08, 0.05]]])

        nodes = [
                MultivariateGaussian(Scope([0,1]), mean=[0.8, 0.3], cov=[[0.13, 0.08], [0.08, 0.05]]),
                MultivariateGaussian(Scope([2,3]), mean=[0.2, -0.1], cov=[[0.17, 0.054], [0.054, 0.0296]]),
                MultivariateGaussian(Scope([0,1]), mean=[0.8, 0.3], cov=[[0.13, 0.08], [0.08, 0.05]]),
        ]
