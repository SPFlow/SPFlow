...
from spflow.torch.sampling.nodes.leaves.parametric.cond_gaussian import sample
...
from spflow.torch.sampling.layers.leaves.parametric.cond_gaussian import sample
...
class TestNode(unittest.TestCase):
    ...
    def test_layer_sampling(self):
        ...
        layer = CondGaussianLayer(scope=[Scope([0]), Scope([1]), Scope([0])], cond_f=lambda data: {'mean': [0.2, 1.8, 0.2], 'std': [0.01, 0.05, 0.01]})

        nodes = [
                CondGaussian(Scope([0]), cond_f=lambda data: {'mean': 0.2, 'std': 0.01}),
                CondGaussian(Scope([1]), cond_f=lambda data: {'mean': 1.8, 'std': 0.05}),
                CondGaussian(Scope([0]), cond_f=lambda data: {'mean': 0.2, 'std': 0.01})
        ]
