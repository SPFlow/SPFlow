...
from spflow.torch.sampling.layers.leaves.parametric.cond_geometric import sample
...
class TestNode(unittest.TestCase):
    ...
    def test_layer_sampling(self):
        ...
        layer = CondGeometricLayer(scope=[Scope([0]), Scope([1]), Scope([0])], cond_f=lambda data: {'p': [0.2, 0.8, 0.2]})
        ...
        expected_mean = torch.tensor([1.0/0.2, 1.0/0.8])
