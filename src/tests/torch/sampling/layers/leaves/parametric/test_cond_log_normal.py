...
from spflow.torch.sampling.layers.leaves.parametric.cond_log_normal import sample
...
class TestNode(unittest.TestCase):
    ...
    def test_layer_sampling(self):
        ...
        layer = CondLogNormalLayer(scope=[Scope([0]), Scope([1]), Scope([0])], cond_f=lambda data: {'mean': [0.2, 1.8, 0.2], 'std': [0.01, 0.05, 0.01]})

        nodes = [
                CondLogNormal(Scope([0]), cond_f=lambda data: {'mean': 0.2, 'std': 0.01}),
                CondLogNormal(Scope([1]), cond_f=lambda data: {'mean': 1.8, 'std': 0.05}),
                CondLogNormal(Scope([0]), cond_f=lambda data: {'mean': 0.2, 'std': 0.01})
        ]
        ...
        expected_mean = torch.tensor([0.2+(0.01**2)/2, 1.8+(0.05**2)/2]).exp()
