...
from spflow.torch.sampling.nodes.leaves.parametric.cond_gamma import sample
...
from spflow.torch.sampling.layers.leaves.parametric.cond_gamma import sample
...
class TestNode(unittest.TestCase):
    ...
    def test_layer_sampling(self):
        ...
        layer = CondGammaLayer(scope=[Scope([0]), Scope([1]), Scope([0])], cond_f=lambda data: {'alpha': [0.2, 1.8, 0.2], 'beta': [1.9, 0.4, 1.9]})

        nodes = [
                CondGamma(Scope([0]), cond_f=lambda data: {'alpha': 0.2, 'beta': 1.9}),
                CondGamma(Scope([1]), cond_f=lambda data: {'alpha': 1.8, 'beta': 0.4}),
                CondGamma(Scope([0]), cond_f=lambda data: {'alpha': 0.2, 'beta': 1.9})
        ]
        ...
        expected_mean = torch.tensor([0.2/1.9, 1.8/0.4])
