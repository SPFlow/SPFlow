...
from spflow.torch.sampling.nodes.leaves.parametric.cond_negative_binomial import sample
...
from spflow.torch.sampling.layers.leaves.parametric.cond_negative_binomial import sample
...
class TestNode(unittest.TestCase):
    ...
    def test_layer_sampling(self):
        ...
        layer = CondNegativeBinomialLayer(scope=[Scope([0]), Scope([1]), Scope([0])], n=[3, 2, 3], cond_f=lambda data: {'p': [0.2, 0.8, 0.2]})

        nodes = [
                CondNegativeBinomial(Scope([0]), n=3, cond_f=lambda data: {'p': 0.2}),
                CondNegativeBinomial(Scope([1]), n=2, cond_f=lambda data: {'p': 0.8}),
                CondNegativeBinomial(Scope([0]), n=3, cond_f=lambda data: {'p': 0.2})
        ]
