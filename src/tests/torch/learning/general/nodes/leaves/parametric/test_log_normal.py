...
from spflow.torch.structure.spn import LogNormal, ProductNode, SumNode
...
class TestNode(unittest.TestCase):
    ...
    def test_em_mixture_of_log_normals(self):
        ...
        self.assertTrue(torch.isclose(l1.mean, torch.tensor(2.0), atol=1e-2, rtol=1e-1))
        self.assertTrue(torch.isclose(l1.std, torch.tensor(1.0), atol=1e-2, rtol=1e-1))
        self.assertTrue(torch.isclose(l2.mean, torch.tensor(-2.0), atol=1e-2, rtol=1e-1))
        self.assertTrue(torch.isclose(l2.std, torch.tensor(1.0), atol=1e-2, rtol=1e-1))
