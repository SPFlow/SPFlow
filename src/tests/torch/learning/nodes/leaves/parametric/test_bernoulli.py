...
from spflow.torch.learning.nodes.leaves.parametric.bernoulli import maximum_likelihood_estimation, em
...
class TestNode(unittest.TestCase):
    ...
    def test_mle_invalid_support(self):
        ...
        self.assertRaises(ValueError, maximum_likelihood_estimation, leaf, torch.tensor([[2]]), bias_correction=True)

    def test_mle_nan_strategy_none(self):

        leaf = Bernoulli(Scope([0]))
        self.assertRaises(ValueError, maximum_likelihood_estimation, leaf, torch.tensor([[float("nan")], [1], [0], [1]]), nan_strategy=None)

    def test_mle_nan_strategy_ignore(self):

        leaf = Bernoulli(Scope([0]))
        maximum_likelihood_estimation(leaf, torch.tensor([[float("nan")], [1], [0], [1]]), nan_strategy='ignore')
    ...
    def test_em_mixture_of_bernoullis(self):
        pass
