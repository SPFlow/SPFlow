...
from spflow.torch.learning.nodes.leaves.parametric.log_normal import (
...
class TestNode(unittest.TestCase):
    ...
    def test_em_step(self):
        ...
        leaf = LogNormal(Scope([0]))
        data = torch.tensor(
            np.random.lognormal(mean=-1.7, sigma=0.2, size=(10000, 1))
        )
    ...
    def test_em_mixture_of_log_normals(self):
        ...
        self.assertTrue(
            torch.isclose(l1.mean, torch.tensor(2.0), atol=1e-2, rtol=1e-1)
        )
        self.assertTrue(
            torch.isclose(l1.std, torch.tensor(1.0), atol=1e-2, rtol=1e-1)
        )
        self.assertTrue(
            torch.isclose(l2.mean, torch.tensor(-2.0), atol=1e-2, rtol=1e-1)
        )
        self.assertTrue(
            torch.isclose(l2.std, torch.tensor(1.0), atol=1e-2, rtol=1e-1)
        )
        self.assertTrue(
            torch.allclose(
