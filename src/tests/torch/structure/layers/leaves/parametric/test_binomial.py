from spflow.torch.structure.layers.leaves.parametric.binomial import BinomialLayer, marginalize, toTorch, toBase
...
class TestNode(unittest.TestCase):
    ...
    def test_layer_initialization(self):
        ...
        self.assertRaises(ValueError, BinomialLayer, [Scope([0]), Scope([1]), Scope([2])], n_values, p_values[:-1], n_nodes=3)
        self.assertRaises(ValueError, BinomialLayer, [Scope([0]), Scope([1]), Scope([2])], n_values[:-1], p_values, n_nodes=3)
        # wrong number of dimensions (nested list)
        self.assertRaises(ValueError, BinomialLayer, [Scope([0]), Scope([1]), Scope([2])], [n_values for _ in range(3)], p_values, n_nodes=3)
        self.assertRaises(ValueError, BinomialLayer, [Scope([0]), Scope([1]), Scope([2])], n_values, [p_values for _ in range(3)], n_nodes=3)

        # ----- numpy parameter values -----

        l = BinomialLayer(scope=[Scope([0]), Scope([1]), Scope([2])], n=np.array(n_values), p=np.array(p_values))
        ...
        # wrong number of values
        self.assertRaises(ValueError, BinomialLayer, [Scope([0]), Scope([1]), Scope([2])], np.array(n_values[:-1]), np.array(p_values))
        self.assertRaises(ValueError, BinomialLayer, [Scope([0]), Scope([1]), Scope([2])], np.array(n_values), np.array(p_values[:-1]))
        # wrong number of dimensions (nested list)
        self.assertRaises(ValueError, BinomialLayer, [Scope([0]), Scope([1]), Scope([2])], n_values, np.array([p_values for _ in range(3)]))
        self.assertRaises(ValueError, BinomialLayer, [Scope([0]), Scope([1]), Scope([2])], np.array([n_values for _ in range(3)]), p_values)
    ...
    def test_layer_dist(self):
        ...
        l = BinomialLayer(scope=[Scope([1]), Scope([0]), Scope([2])], n=n_values, p=p_values, n_nodes=3)
    ...
    def test_layer_backend_conversion_1(self):
        ...
        torch_layer = BinomialLayer(scope=[Scope([0]), Scope([1]), Scope([0])], n=[2, 5, 2], p=[0.2, 0.9, 0.31])
    ...
    def test_layer_backend_conversion_2(self):
        ...
        base_layer = BaseBinomialLayer(scope=[Scope([0]), Scope([1]), Scope([0])], n=[2, 5, 2], p=[0.2, 0.9, 0.31])
