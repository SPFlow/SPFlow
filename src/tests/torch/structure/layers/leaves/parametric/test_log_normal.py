from spflow.torch.structure.layers.leaves.parametric.log_normal import LogNormalLayer, marginalize, toTorch, toBase
...
class TestNode(unittest.TestCase):
    ...
    def test_layer_initialization(self):
        ...
        l = LogNormalLayer(scope=Scope([1]), n_nodes=3, mean=np.array(mean_values), std=np.array(std_values))
        ...
        # wrong number of dimensions (nested list)
        self.assertRaises(ValueError, LogNormalLayer, Scope([0]), np.array(mean_values), np.array([std_values for _ in range(3)]), n_nodes=3)
        self.assertRaises(ValueError, LogNormalLayer, Scope([0]), np.array([mean_values for _ in range(3)]), np.array(std_values), n_nodes=3)
    ...
    def test_layer_backend_conversion_1(self):
        ...
        torch_layer = LogNormalLayer(scope=[Scope([0]), Scope([1]), Scope([0])], mean=[0.2, 0.9, 0.31], std=[1.9, 0.3, 0.71])
    ...
    def test_layer_backend_conversion_2(self):
        ...
        base_layer = BaseLogNormalLayer(scope=[Scope([0]), Scope([1]), Scope([0])], mean=[0.2, 0.9, 0.31], std=[1.9, 0.3, 0.71])
