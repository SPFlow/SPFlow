from spflow.torch.structure.layers.leaves.parametric.cond_gamma import CondGammaLayer, marginalize, toTorch, toBase
...
class TestNode(unittest.TestCase):
    ...
    def test_retrieve_params(self):
        ...
        l = CondGammaLayer(scope=Scope([1]), n_nodes=3, cond_f=lambda data: {'alpha': alpha_values, 'beta': beta_values})

        for alpha_value, beta_value, alpha_layer_node, beta_layer_node in zip(alpha_values, beta_values, *l.retrieve_params(torch.tensor([[1.0]]), DispatchContext())):
            self.assertTrue(torch.allclose(alpha_layer_node, torch.tensor(alpha_value)))
            self.assertTrue(torch.allclose(beta_layer_node, torch.tensor(beta_value)))
        ...
        for alpha_actual, beta_actual, alpha_node, beta_node in zip(alpha_values, beta_values, *l.retrieve_params(torch.tensor([[1.0]]), DispatchContext())):
    ...
    def test_layer_backend_conversion_1(self):
        ...
        torch_layer = CondGammaLayer(scope=[Scope([0]), Scope([1]), Scope([0])])
    ...
    def test_layer_backend_conversion_2(self):
        ...
        base_layer = BaseCondGammaLayer(scope=[Scope([0]), Scope([1]), Scope([0])])
