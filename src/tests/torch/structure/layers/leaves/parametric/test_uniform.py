from spflow.torch.structure.layers.leaves.parametric.uniform import UniformLayer, marginalize, toTorch, toBase
...
class TestNode(unittest.TestCase):
    ...
    def test_layer_initialization(self):
        ...
        l = UniformLayer(scope=Scope([1]), n_nodes=3, start=start_values, end=end_values, support_outside=support_outside_values)
        # make sure number of creates nodes is correct
        self.assertEqual(len(l.scopes_out), 3)
        # make sure scopes are correct
        self.assertTrue(np.all(l.scopes_out == [Scope([1]), Scope([1]), Scope([1])]))
        # make sure parameter properties works correctly
        for start_layer_node, end_layer_node, support_outside_layer_node, start_value, end_value, support_outside_value in zip(l.start, l.end, l.support_outside, start_values, end_values, support_outside_values):
        ...
        l = UniformLayer(scope=Scope([1]), n_nodes=3, start=start_value, end=end_value, support_outside=support_outside_value)
        ...
        l = UniformLayer(scope=Scope([1]), n_nodes=3, start=np.array(start_values), end=np.array(end_values), support_outside=np.array(support_outside_values))
    ...
    def test_layer_structural_marginalization(self):
        ...
        l = UniformLayer(scope=Scope([1]), start=[0.73, 0.29], end=[1.41, 0.9], support_outside=[True, False], n_nodes=2)
        ...
        l = UniformLayer(scope=[Scope([1]), Scope([0])], start=[0.73, 0.29], end=[1.41, 0.9], support_outside=[True, False])
    ...
    def test_layer_dist(self):
        ...
        end_values = [0.9, 1.34, 0.98]
        support_outside_values = [True, False, True]
        l = UniformLayer(scope=Scope([1]), start=start_values, end=end_values, support_outside=support_outside_values, n_nodes=3)
        ...
        for start_value, end_value, start_dist, end_dist in zip(reversed(start_values[:-1]), reversed(end_values[:-1]), dist.low, dist.high):
            self.assertTrue(torch.allclose(torch.tensor(start_value), start_dist))
            self.assertTrue(torch.allclose(torch.tensor(end_value), end_dist))

    def test_layer_backend_conversion_1(self):
        
        torch_layer = UniformLayer(scope=[Scope([0]), Scope([1]), Scope([0])], start=[0.2, -0.9, 0.31], end=[0.3, 1.0, 0.5], support_outside=[True, False, True])
    ...
    def test_layer_backend_conversion_2(self):
        ...
        base_layer = BaseUniformLayer(scope=[Scope([0]), Scope([1]), Scope([0])], start=[0.2, -0.9, 0.31], end=[0.3, 1.0, 0.5], support_outside=[True, False, True])
