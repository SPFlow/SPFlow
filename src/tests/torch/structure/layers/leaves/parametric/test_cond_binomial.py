from spflow.torch.structure.layers.leaves.parametric.cond_binomial import CondBinomialLayer, marginalize, toTorch, toBase
...
class TestNode(unittest.TestCase):
    ...
    def test_layer_initialization(self):
        ...
        n_values = [3, 2, 7]
        l = CondBinomialLayer(scope=[Scope([1]), Scope([0]), Scope([2])], n=n_values)
        ...
        # ----- n initialization -----
        l = CondBinomialLayer(scope=[Scope([1]), Scope([0]), Scope([2])], n=[3, 5, 2])
        # wrong number of n values
        self.assertRaises(ValueError, CondBinomialLayer, scope=[Scope([1]), Scope([0]), Scope([2])], n=[3, 5])
        # wrong shape of n values
        self.assertRaises(ValueError, CondBinomialLayer, scope=[Scope([1]), Scope([0]), Scope([2])], n=[[3, 5, 2]])

        # n numpy array
        l = CondBinomialLayer(scope=[Scope([1]), Scope([0]), Scope([2])], n=np.array([3, 5, 2]))
        # wrong number of n values
        self.assertRaises(ValueError, CondBinomialLayer, scope=[Scope([1]), Scope([0]), Scope([2])], n=np.array([3, 5]))
        # wrong shape of n values
        self.assertRaises(ValueError, CondBinomialLayer, scope=[Scope([1]), Scope([0]), Scope([2])], n=np.array([[3, 5, 2]]))
        ...
        self.assertRaises(ValueError, CondBinomialLayer, Scope([0]), n_nodes=0, n=2)
        ...
        l = CondBinomialLayer(scope=[Scope([1]), Scope([0])], n_nodes=3, n=2)
        
        for layer_scope, node_scope in zip(l.scopes_out, scopes):
            self.assertEqual(layer_scope, node_scope)
        
        # -----number of cond_f functions -----
        CondBinomialLayer(Scope([0]), n=2, n_nodes=2, cond_f=[lambda data: {'p': 0.5}, lambda data: {'p': 0.5}])
        self.assertRaises(ValueError, CondBinomialLayer, Scope([0]), n=2, n_nodes=2, cond_f=[lambda data: {'p': 0.5}])
    ...
    def test_retrieve_params(self):
        ...
        n_value = 5
        p_value = 0.13
        l = CondBinomialLayer(scope=Scope([1]), n_nodes=3, n=n_value, cond_f=lambda data: {'p': p_value})
        ...
        l = CondBinomialLayer(scope=[Scope([0]), Scope([1]), Scope([2])], n=n_values, cond_f=lambda data: {'p': p_values})

        for n_layer_node, p_layer_node, n_value, p_value in zip(l.n, l.retrieve_params(torch.tensor([[1]]), DispatchContext()), n_values, p_values):
            self.assertTrue(torch.allclose(n_layer_node, torch.tensor(n_value)))
    ...
    def test_layer_structural_marginalization(self):
        ...
        l = CondBinomialLayer(scope=Scope([1]), n_nodes=2, n=2)
        ...
        l = CondBinomialLayer(scope=[Scope([1]), Scope([0])], n=[3, 2])
    ...
    def test_layer_dist(self):
        ...
        l = CondBinomialLayer(scope=[Scope([1]), Scope([0]), Scope([2])], n=n_values, n_nodes=3)
    ...
    def test_layer_backend_conversion_1(self):
        ...
        torch_layer = CondBinomialLayer(scope=[Scope([0]), Scope([1]), Scope([0])], n=[2, 5, 2])
    ...
    def test_layer_backend_conversion_2(self):
        ...
        base_layer = BaseCondBinomialLayer(scope=[Scope([0]), Scope([1]), Scope([0])], n=[2, 5, 2])
