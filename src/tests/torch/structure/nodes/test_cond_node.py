from spflow.torch.structure.nodes.cond_node import SPNCondSumNode, marginalize, toBase, torch
from spflow.torch.structure.nodes.node import SPNProductNode, marginalize, toBase, toTorch
...
class TestTorchNode(unittest.TestCase):
    ...
    def test_retrieve_params(self):
        ...
        self.assertRaises(ValueError, node.retrieve_params, torch.tensor([[1]]), DispatchContext())
    ...
    def test_backend_conversion_2(self):
        ...
        s_base = BaseSPNCondSumNode([BaseGaussian(Scope([0])), BaseGaussian(Scope([0]))])
