...
class TestExponential(unittest.TestCase):
    ...
    def test_retrieve_params(self):
        ...
        self.assertTrue(exponential.retrieve_params(np.array([[1.0]]), DispatchContext()) == torch.nextafter(torch.tensor(0.0), torch.tensor(1.0)))
        
        # l = 0 and l < 0
        exponential.set_cond_f(lambda data: {'l': torch.tensor(0.0)})
        self.assertRaises(Exception, exponential.retrieve_params, np.array([[1.0]]), DispatchContext())
    ...
    def test_base_backend_conversion(self):
        ...
        self.assertTrue(
            np.all(torch_exponential.scopes_out == toBase(torch_exponential).scopes_out)
        )
        # check conversion from python to torch
        self.assertTrue(
            np.all(node_exponential.scopes_out == toTorch(node_exponential).scopes_out)
        )
