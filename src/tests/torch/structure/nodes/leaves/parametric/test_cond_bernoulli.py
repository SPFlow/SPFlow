...
class TestBernoulli(unittest.TestCase):
    ...
    def test_retrieve_params(self):
        ...
        self.assertRaises(Exception, bernoulli.retrieve_params, np.array([[1.0]]), DispatchContext())
    ...
    def test_base_backend_conversion(self):
        ...
        self.assertTrue(
            np.all(torch_bernoulli.scopes_out == toBase(torch_bernoulli).scopes_out)
        )
        # check conversion from python to torch
        self.assertTrue(
            np.all(node_bernoulli.scopes_out == toTorch(node_bernoulli).scopes_out)
        )
