...
class TestNegativeBinomial(unittest.TestCase):
    def test_initialization(self):
        ...
        binomial = CondNegativeBinomial(Scope([0]), n=1, cond_f=lambda x: {'p': 0.5})
    ...
    def test_retrieve_params(self):
        ...
        self.assertRaises(Exception, negative_binomial.retrieve_params, np.array([[1.0]]), DispatchContext())
        # p < 0 and p > 1
    ...
    def test_base_backend_conversion(self):
        ...
        self.assertTrue(
            np.all(torch_negative_binomial.scopes_out == toBase(torch_negative_binomial).scopes_out)
        )
        # check conversion from python to torch
        self.assertTrue(
            np.all(node_negative_binomial.scopes_out == toTorch(node_negative_binomial).scopes_out)
        )
