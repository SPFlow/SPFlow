...
from spflow.torch.structure.nodes.leaves.parametric.cond_negative_binomial import CondNegativeBinomial, toBase, toTorch
...
class TestNegativeBinomial(unittest.TestCase):
    ...
    def test_inference(self):
        ...
        torch_negative_binomial = CondNegativeBinomial(Scope([0]), n, cond_f=lambda data: {'p': p})
        node_negative_binomial = BaseCondNegativeBinomial(Scope([0]), n, cond_f=lambda data: {'p': p})
    ...
    def test_likelihood_p_1(self):
        ...
        # p = 1
        negative_binomial = CondNegativeBinomial(Scope([0]), 1, cond_f=lambda data: {'p': 1.0})
    ...
    def test_likelihood_n_float(self):
        ...
        negative_binomial = CondNegativeBinomial(Scope([0]), 1, cond_f=lambda data: {'p': 0.5})
        self.assertRaises(Exception, likelihood, negative_binomial, 0.5)

    def test_likelihood_marginalization(self):

        negative_binomial = CondNegativeBinomial(Scope([0]), 20, cond_f=lambda data: {'p': 0.3})
    ...
    def test_support(self):
        ...
        negative_binomial = CondNegativeBinomial(Scope([0]), n, cond_f=lambda data: {'p': p})
