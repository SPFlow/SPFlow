...
class TestGamma(unittest.TestCase):
    ...
    def test_inference(self):
        ...
        torch_gamma = CondGamma(Scope([0]), cond_f=lambda data: {'alpha': alpha, 'beta': beta})
        node_gamma = BaseCondGamma(Scope([0]), cond_f=lambda data: {'alpha': alpha, 'beta': beta})
    ...
    def test_gradient_computation(self):
        ...
        alpha = torch.tensor(random.randint(1, 5), dtype=torch.get_default_dtype(), requires_grad=True)
        beta = torch.tensor(random.randint(1, 5), dtype=torch.get_default_dtype(), requires_grad=True)
