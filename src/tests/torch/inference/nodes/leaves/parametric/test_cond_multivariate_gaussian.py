...
from spflow.torch.structure.nodes.leaves.parametric.cond_multivariate_gaussian import CondMultivariateGaussian, toBase, toTorch
...
class TestMultivariateGaussian(unittest.TestCase):
    ...
    def test_likelihood_args_p(self):
        ...
        dispatch_ctx.args[multivariate_gaussian] = {'mean': torch.zeros(2), 'cov': torch.eye(2)}
    ...
    def test_gradient_computation(self):
        ...
        cov = torch.tensor([[2.0, 2.0, 1.0], [2.0, 3.0, 2.0], [1.0, 2.0, 3.0]], requires_grad=True)
    ...
    def test_likelihood_marginalization(self):
        ...
        multivariate_gaussian = CondMultivariateGaussian(
            Scope([0, 1]), cond_f=lambda data: {'mean': torch.zeros(2), 'cov': torch.tensor([[2.0, 0.0], [0.0, 1.0]])}
        )
        ...
        univariate_gaussians = SPNProductNode(
            children=[
                CondGaussian(
                    Scope([0]), cond_f=lambda data: {'mean': 0.0, 'std': math.sqrt(2.0)}
                ),  # requires standard deviation instead of variance
                CondGaussian(Scope([1]), cond_f=lambda data: {'mean': 0.0, 'std': 1.0}),
            ],
        )
