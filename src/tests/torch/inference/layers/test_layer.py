...
class TestNode(unittest.TestCase):
    ...
    def test_sum_layer_gradient_optimization(self):
        ...
        sum_layer = SPNSumLayer(n_nodes=3, children=[gaussian_1, gaussian_2], weights=weights)
        ...
        for i in range(100):
            ...
            if i == 0:
                ...
                self.assertTrue(torch.allclose(sum_layer.weights.sum(dim=-1), torch.tensor([1.0, 1.0, 1.0])))
            else:
                # update parameters
                optimizer.step()

        self.assertTrue(
            torch.allclose(sum_layer.weights, torch.tensor([[0.7, 0.3], [0.7, 0.3], [0.7, 0.3]]), atol=1e-3, rtol=1e-3)
        )
