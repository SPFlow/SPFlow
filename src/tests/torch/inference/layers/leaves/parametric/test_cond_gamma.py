...
from spflow.torch.inference.layers.leaves.parametric.cond_gamma import log_likelihood
from spflow.torch.structure.nodes.leaves.parametric.cond_gamma import CondGamma
from spflow.torch.inference.nodes.leaves.parametric.cond_gamma import log_likelihood
...
class TestNode(unittest.TestCase):
    ...
    def test_likelihood_no_alpha(self):
        ...
        gamma = CondGammaLayer(Scope([0]), cond_f=lambda data: {'beta': [1.0, 1.0]}, n_nodes=2)
        self.assertRaises(KeyError, log_likelihood, gamma, torch.tensor([[0], [1]]))

    def test_likelihood_no_beta(self):

        gamma = CondGammaLayer(Scope([0]), cond_f=lambda data: {'alpha': [1.0, 1.0]}, n_nodes=2)
        self.assertRaises(KeyError, log_likelihood, gamma, torch.tensor([[0], [1]]))
    ...
    def test_layer_likelihood(self):
        ...
        layer = CondGammaLayer(scope=[Scope([0]), Scope([1]), Scope([0])], cond_f=lambda data: {'alpha': [0.2, 1.0, 2.3], 'beta': [1.0, 0.3, 0.97]})

        nodes = [
            CondGamma(Scope([0]), cond_f=lambda data: {'alpha': 0.2, 'beta': 1.0}),
            CondGamma(Scope([1]), cond_f=lambda data: {'alpha': 1.0, 'beta': 0.3}),
            CondGamma(Scope([0]), cond_f=lambda data: {'alpha': 2.3, 'beta': 0.97}),
        ]
    ...
    def test_gradient_computation(self):
        ...
        alpha = torch.tensor([random.randint(1, 5), random.randint(1, 3)], dtype=torch.get_default_dtype(), requires_grad=True)
        beta = torch.tensor([random.randint(1, 5), random.randint(2, 4)], dtype=torch.get_default_dtype(), requires_grad=True)

        torch_gamma = CondGammaLayer(scope=[Scope([0]), Scope([1])], cond_f=lambda data: {'alpha': alpha, 'beta': beta})
    ...
    def test_likelihood_marginalization(self):
        ...
        gamma = CondGammaLayer(scope=[Scope([0]), Scope([1])], cond_f=lambda data: {'alpha': random.random()+1e-7, 'beta': random.random()+1e-7})
