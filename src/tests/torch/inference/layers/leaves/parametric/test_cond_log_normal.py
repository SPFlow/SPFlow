...
from spflow.torch.structure.layers.leaves.parametric.cond_log_normal import CondLogNormalLayer
from spflow.torch.inference.layers.leaves.parametric.cond_log_normal import log_likelihood
from spflow.torch.structure.nodes.leaves.parametric.cond_log_normal import CondLogNormal
from spflow.torch.inference.nodes.leaves.parametric.cond_log_normal import log_likelihood
...
class TestNode(unittest.TestCase):
    ...
    def test_likelihood_no_mean(self):
        ...
        log_normal = CondLogNormalLayer(Scope([0]), cond_f=lambda data: {'std': [0.25, 0.25]}, n_nodes=2)
        self.assertRaises(KeyError, log_likelihood, log_normal, torch.tensor([[0], [1]]))

    def test_likelihood_no_std(self):

        log_normal = CondLogNormalLayer(Scope([0]), cond_f=lambda data: {'mean': [0.0, 0.0]}, n_nodes=2)
        self.assertRaises(KeyError, log_likelihood, log_normal, torch.tensor([[0], [1]]))
    ...
    def test_likelihood_args(self):
        ...
        dispatch_ctx.args[log_normal] = {'mean': [0.0, 0.0], 'std': [0.25, 0.25]}
    ...
    def test_layer_likelihood(self):
        ...
        layer = CondLogNormalLayer(scope=[Scope([0]), Scope([1]), Scope([0])], cond_f=lambda data: {'mean': [0.2, 1.0, 2.3], 'std': [1.0, 0.3, 0.97]})

        nodes = [
            CondLogNormal(Scope([0]), cond_f=lambda data: {'mean': 0.2, 'std': 1.0}),
            CondLogNormal(Scope([1]), cond_f=lambda data: {'mean': 1.0, 'std': 0.3}),
            CondLogNormal(Scope([0]), cond_f=lambda data: {'mean': 2.3, 'std': 0.97}),
        ]
    ...
    def test_gradient_computation(self):
        ...
        torch_log_normal = CondLogNormalLayer(scope=[Scope([0]), Scope([1])], cond_f=lambda data: {'mean': mean, 'std': std})
    ...
    def test_likelihood_marginalization(self):
        ...
        log_normal = CondLogNormalLayer(scope=[Scope([0]), Scope([1])], cond_f=lambda data: {'mean': random.random(), 'std': random.random()+1e-7})
