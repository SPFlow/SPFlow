...
class TestNode(unittest.TestCase):
    ...
    def test_layer_likelihood(self):
        ...
        layer = MultivariateGaussianLayer(scope=[Scope([0,1]), Scope([2,3,4])], mean=mean_values, cov=cov_values)
    ...
    def test_gradient_computation(self):
        ...
        torch_multivariate_gaussian = MultivariateGaussianLayer(scope=[Scope([0,1]), Scope([2,3,4])], mean=mean, cov=cov)
        ...
        self.assertTrue(all([torch.allclose(m_orig - m_current.grad, m_current) for m_orig, m_current in zip(mean_orig, torch_multivariate_gaussian.mean)]))
        self.assertTrue(all([torch.allclose(t_orig - t_current.grad, t_current) for t_orig, t_current in zip(tril_diag_aux_orig, [n.tril_diag_aux for n in torch_multivariate_gaussian.nodes])]))
        self.assertTrue(all([torch.allclose(t_orig - t_current.grad, t_current) for t_orig, t_current in zip(tril_nondiag_orig, [n.tril_nondiag for n in torch_multivariate_gaussian.nodes])]))
    ...
    def test_likelihood_marginalization(self):
        ...
        gaussian = MultivariateGaussianLayer(scope=[Scope([0,1]), Scope([1,2])], mean=torch.zeros(2,2), cov=torch.stack([torch.eye(2), torch.eye(2)]))
