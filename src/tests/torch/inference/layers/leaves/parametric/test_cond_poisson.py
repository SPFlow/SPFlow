...
class TestNode(unittest.TestCase):
    ...
    def test_layer_likelihood(self):
        ...
        layer = CondPoissonLayer(scope=[Scope([0]), Scope([1]), Scope([0])], cond_f=lambda data: {'l': [0.2, 1.0, 2.3]})
    ...
    def test_gradient_computation(self):
        ...
        l = torch.tensor([random.randint(1, 10), random.randint(1, 10)], dtype=torch.get_default_dtype(), requires_grad=True)

        torch_poisson = CondPoissonLayer(scope=[Scope([0]), Scope([1])], cond_f=lambda data: {'l': l})
    ...
    def test_likelihood_marginalization(self):
        ...
        poisson = CondPoissonLayer(scope=[Scope([0]), Scope([1])], cond_f=lambda data: {'l': random.random()+1e-7})
