...
class TestNode(unittest.TestCase):
    ...
    def test_mle_nan_strategy_ignore(self):
        ...
        maximum_likelihood_estimation(
            leaf,
            torch.exp(torch.tensor([[-2.3, 0.1], [-1.8, 1.9], [0.9, 0.7]])),
            nan_strategy=None,
            bias_correction=False,
        )
    ...
    def test_mle_nan_strategy_invalid(self):
        ...
        self.assertRaises(
            ...
            maximum_likelihood_estimation,
            leaf,
            torch.tensor([[float("nan"), 0.0], [1, 0.1], [1.9, -0.2]]),
    ...
    def test_update_backend(self):
        ...
        for backend in backends:
            ...
            self.assertTrue(np.allclose(leaf_updated.get_params()[0], params, atol=1e-2, rtol=1e-3))
            self.assertTrue(np.allclose(leaf_updated.get_params()[1], params2, atol=1e-2, rtol=1e-3))
            ...
            if tl.get_backend() != "pytorch":
                ...
                expectation_maximization(prod_node, tl.tensor(data), max_steps=10)
                self.assertTrue(np.allclose(leaf_updated.get_params()[0], params_em, atol=1e-3, rtol=1e-2))
                self.assertTrue(np.allclose(leaf_updated.get_params()[1], params_em2, atol=1e-3, rtol=1e-2))
