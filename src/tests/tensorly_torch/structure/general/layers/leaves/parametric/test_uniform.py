...
from spflow.base.structure.general.layers.leaves.parametric.uniform import UniformLayer as UniformLayerBase
...
from spflow.torch.structure.general.layers.leaves.parametric.uniform import UniformLayer as UniformLayerTorch
...
def test_layer_initialization(do_for_all_backends):
    ...
    for (
        ...
        tc.assertTrue(np.allclose(tl_toNumpy(start_layer_node), start_value))
        tc.assertTrue(np.allclose(tl_toNumpy(end_layer_node), end_value))
...
def test_layer_dist(do_for_all_backends):
    ...
    for start_value, end_value, start_dist, end_dist in zip(start_values[1:], end_values[1:], start_list, end_list):
        tc.assertTrue(np.allclose(tl.tensor(start_value), start_dist))
        tc.assertTrue(np.allclose(tl.tensor(end_value), end_dist))
