...
def maximum_likelihood_estimation(
    ...
    if check_support:
        if torch.any(~layer.check_support(scope_data, is_scope_data=True)):
            raise ValueError(
                "Encountered values outside of the support for 'NegativeBinomialLayer'."
            )
    ...
    if isinstance(nan_strategy, str):
        ...
        if nan_strategy == "ignore":
            ...
            n_total = (
                weights * torch.nan_to_num(scope_data + layer.n, nan=0.0)
            ).sum(dim=0)
        else:
            raise ValueError(
                "Unknown strategy for handling missing (NaN) values for 'NegativeBinomialLayer'."
            )
