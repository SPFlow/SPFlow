...
def maximum_likelihood_estimation(layer: ExponentialLayer, data: torch.Tensor, weights: Optional[torch.Tensor]=None, bias_correction: bool=True, nan_strategy: Optional[Union[str, Callable]]=None, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> None:
    r"""Maximum (weighted) likelihood estimation (MLE) of ``ExponentialLayer`` leaves' parameters in the ``torch`` backend.
    ...
    Weights are normalized to sum up to :math:`N` for each row.
...
@dispatch(memoize=True)  # type: ignore
def em(layer: ExponentialLayer, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> None:
