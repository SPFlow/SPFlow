...
def maximum_likelihood_estimation(
    ...
    if check_support:
        if torch.any(~layer.check_support(scope_data)):
            raise ValueError(
                "Encountered values outside of the support for 'GaussianLayer'."
            )
    ...
    if isinstance(nan_strategy, str):
        ...
        if nan_strategy == "ignore":
            ...
            mean_est = (weights * torch.nan_to_num(scope_data, nan=0.0)).sum(
                dim=0
            ) / n_total
            ...
            std_est = torch.sqrt(
                (
                    weights
                    * torch.nan_to_num(scope_data - mean_est, nan=0.0).pow(2)
                ).sum(dim=0)
                / n_total
            )
        else:
        ...
        std_est = torch.sqrt(
            (weights * torch.pow(scope_data - mean_est, 2)).sum(dim=0) / n_total
        )
    else:
        raise ValueError(
