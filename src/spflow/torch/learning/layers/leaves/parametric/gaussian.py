...
def maximum_likelihood_estimation(layer: GaussianLayer, data: torch.Tensor, weights: Optional[torch.Tensor]=None, bias_correction: bool=True, nan_strategy: Optional[Union[str, Callable]]=None, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> None:
    r"""Maximum (weighted) likelihood estimation (MLE) of ``GaussianLayer`` leaves' parameters in the ``torch`` backend.
...
@dispatch(memoize=True)  # type: ignore
def em(layer: GaussianLayer, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> None:
    """Performs a single expectation maximizaton (EM) step for ``GaussianLayer`` in the ``torch`` backend.
