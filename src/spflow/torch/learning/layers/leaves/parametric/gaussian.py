...
def maximum_likelihood_estimation(layer: GaussianLayer, data: torch.Tensor, bias_correction: bool=True, nan_strategy: Optional[Union[str, Callable]]=None) -> None:
    """TODO."""
    ...
    if isinstance(nan_strategy, str):
        ...
        if nan_strategy == "ignore":
            ...
            mean_est = torch.nan_to_num(scope_data, nan=0.0).sum(dim=0)/n_total

            if bias_correction:
                n_total -= 1
            
            std_est = torch.sqrt(
                torch.nan_to_num(scope_data-mean_est, nan=0.0).pow(2).sum(dim=0) / n_total
            )
        else:
        ...
        mean_est = scope_data.mean(dim=0)
        std_est = scope_data.std(unbiased=True if bias_correction else False, dim=0)
    else:
