...
def maximum_likelihood_estimation(layer: BinomialLayer, data: torch.Tensor, bias_correction: bool=True, nan_strategy: Optional[Union[str, Callable]]=None) -> None:
    """TODO."""
    ...
    if isinstance(nan_strategy, str):
        ...
        if nan_strategy == "ignore":
            ...
            n_total = (~nan_mask).sum(dim=0) * layer.n # never zero since we checked that earlier

            # count number of total successes
            n_success = torch.nan_to_num(scope_data, nan=0.0).sum(dim=0, dtype=torch.get_default_dtype())
        ...
        n_total = (scope_data.shape[0] * layer.n).type(dtype=torch.get_default_dtype())

        # count number of total successes
        n_success = scope_data.sum(dim=0, dtype=torch.get_default_dtype())
