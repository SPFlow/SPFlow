...
def maximum_likelihood_estimation(leaf: Binomial, data: torch.Tensor, bias_correction: bool=True, nan_strategy: Optional[Union[str, Callable]]=None) -> None:
    """TODO."""
    ...
    n_total = (scope_data.shape[0] * leaf.n).type(dtype=torch.get_default_dtype())

    # count number of total successes
    n_success = scope_data.sum(dtype=torch.get_default_dtype())
