...
def maximum_likelihood_estimation(leaf: LogNormal, data: torch.Tensor, bias_correction: bool=True, nan_strategy: Optional[Union[str, Callable]]=None) -> None:
    """TODO."""
    ...
    mean_est = torch.log(scope_data).mean()
    std_est = torch.log(scope_data).std(unbiased=True if bias_correction else False)
