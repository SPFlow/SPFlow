"""Contains sampling methods for ``CategoricalLayer`` leaves for SPFlow in the ``torch`` backend.
"""
import itertools
...
def sample(
    ...
    r"""Samples from ``CategoricalLayer`` leaves in the ``torch`` backend given potential evidence.
    ...
    for node_id, instances in sampling_ctx.group_output_ids(layer.n_out):
        ...
        data[torch.meshgrid(sampling_ids, torch.tensor(node_scope.query), indexing="ij")] = (
            layer.dist([node_id]).sample((sampling_mask.sum(),)).type(torch.float).to(layer.p_aux.device)
        )
