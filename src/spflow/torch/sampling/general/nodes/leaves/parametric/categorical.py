"""Contains sampling methods for ``Categorical`` nodes for SPFlow in the ``torch`` backend.
"""
...
def sample(
    ...
    data[sampling_ids, leaf.scope.query] = leaf.dist.sample((sampling_ids.sum(),)).type(torch.float).to(leaf.p_aux.device)
