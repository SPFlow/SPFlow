"""Contains sampling methods for ``CondCategorical`` nodes for SPFlow in the ``torch`` backend.
"""
...
def sample(
    ...
    r"""Samples from ``CondCategorical`` nodes in the ``torch`` backend given potential evidence.
    ...
    data[sampling_ids, leaf.scope.query] = leaf.dist(p=p).sample((sampling_ids.sum(),)).type(torch.float).to(p.device)
