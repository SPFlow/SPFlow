...
def sample(layer: CondBernoulliLayer, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None, sampling_ctx: Optional[SamplingContext]=None) -> torch.Tensor:
    r"""Samples from ``CondBernoulliLayer`` leaves in the ``torch`` backend given potential evidence.
    ...
    for output_ids in unique_output_signatures:
        ...
        if not Scope.all_pairwise_disjoint([layer.scopes_out[id] for id in output_ids]):
            raise ValueError("Sampling from output with non-pair-wise disjoint scopes is not permitted for 'BernoulliLayer'.")
