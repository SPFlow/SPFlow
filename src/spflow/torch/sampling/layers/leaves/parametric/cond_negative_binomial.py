...
def sample(layer: CondNegativeBinomialLayer, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None, sampling_ctx: Optional[SamplingContext]=None) -> torch.Tensor:
    r"""Samples from ``CondNegativeBinomialLayer`` leaves in the ``torch`` backend given potential evidence.
    ...
    for node_id, instances in sampling_ctx.group_output_ids(layer.n_out):
        ...
        data[torch.meshgrid(sampling_ids, torch.tensor(node_scope.query), indexing='ij')] = layer.dist(p=p, node_ids=[node_id]).sample((sampling_mask.sum(),)).to(p.device)
