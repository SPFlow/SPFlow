...
def sample(layer: PoissonLayer, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None, sampling_ctx: Optional[SamplingContext]=None) -> torch.Tensor:
    r"""Samples from ``PoissonLayer`` leaves in the ``torch`` backend given potential evidence.
