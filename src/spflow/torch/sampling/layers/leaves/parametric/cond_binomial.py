...
def sample(layer: CondBinomialLayer, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None, sampling_ctx: Optional[SamplingContext]=None) -> torch.Tensor:
    r"""Samples from ``CondBinomialLayer`` leaves in the ``torch`` backend given potential evidence.
