...
def sample(layer: GeometricLayer, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None, sampling_ctx: Optional[SamplingContext]=None) -> torch.Tensor:
    r"""Samples from ``GeometricLayer`` leaves in the ``torch`` backend given potential evidence.
    ...
    for node_id, instances in sampling_ctx.group_output_ids(layer.n_out):
        ...
        data[torch.meshgrid(sampling_ids, torch.tensor(node_scope.query), indexing='ij')] = layer.dist([node_id]).sample((sampling_mask.sum(),)).to(layer.p_aux.device) + 1
