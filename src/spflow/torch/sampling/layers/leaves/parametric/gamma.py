...
def sample(layer: GammaLayer, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None, sampling_ctx: Optional[SamplingContext]=None) -> torch.Tensor:
    r"""Samples from ``GammaLayer`` leaves in the ``torch`` backend given potential evidence.
    ...
    for node_id, instances in sampling_ctx.group_output_ids(layer.n_out):
        ...
        sampling_mask = marg_ids & instance_ids_mask.bool().to(layer.alpha_aux.device)
        sampling_ids = torch.where(sampling_mask)[0]

        data[torch.meshgrid(sampling_ids, torch.tensor(node_scope.query), indexing='ij')] = layer.dist([node_id]).sample((sampling_mask.sum(),)).to(layer.alpha_aux.device)
