...
@dispatch(TorchBinomial, torch.Tensor, ll_cache=dict, instance_ids=list)  # type: ignore[no-redef]
def sample(
    leaf: TorchBinomial, data: torch.Tensor, ll_cache: Dict, instance_ids: List[int]
) -> torch.Tensor:
