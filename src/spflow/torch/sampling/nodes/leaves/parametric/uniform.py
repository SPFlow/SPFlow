...
@dispatch(TorchUniform, torch.Tensor, ll_cache=dict, sampling_ctx=SamplingContext)  # type: ignore[no-redef]
def sample(
    leaf: TorchUniform, data: torch.Tensor, ll_cache: Dict[TorchModule, torch.Tensor], sampling_ctx: SamplingContext
) -> torch.Tensor:
    ...
    sampling_ids = marg_ids & instance_ids_mask.bool().to(leaf.start.device)

    data[sampling_ids, leaf.scope] = leaf.dist.sample((sampling_ids.sum(),)).to(leaf.start.device)
