...
def sample(
    ...
    for nan_mask in torch.unique(nan_data, dim=0):
        ...
        cond_rvs = torch.tensor(leaf.scope.query)[
            torch.where(~nan_mask)[0]
        ]  # ids for evidence RVs
        non_cond_rvs = torch.tensor(leaf.scope.query)[
            torch.where(nan_mask)[0]
        ]  # RVs to be sampled
        ...
        # sample from full distribution
            ...
            ] = leaf.dist.sample((sampling_ids.shape[0],)).squeeze(1)
        # sample from conditioned distribution
            ...
            marg_cov_inv = torch.linalg.inv(
                leaf.cov[torch.meshgrid(cond_rvs, cond_rvs, indexing="ij")]
            )
