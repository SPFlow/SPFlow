...
@dispatch(TorchBernoulli, torch.Tensor, ll_cache=dict, instance_ids=list)  # type: ignore[no-redef]
def sample(
    leaf: TorchBernoulli, data: torch.Tensor, ll_cache: Dict, instance_ids: List[int]
) -> torch.Tensor:
