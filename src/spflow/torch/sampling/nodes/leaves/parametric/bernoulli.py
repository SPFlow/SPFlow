...
@dispatch(TorchBernoulli, torch.Tensor, ll_cache=dict, sampling_ctx=SamplingContext)  # type: ignore[no-redef]
def sample(
    leaf: TorchBernoulli, data: torch.Tensor, ll_cache: Dict[TorchModule, torch.Tensor], sampling_ctx: SamplingContext
) -> torch.Tensor:
    ...
    data[sampling_ids, leaf.scope] = leaf.dist.sample((sampling_ids.sum(),)).to(leaf.p_aux.device)
