...
@dispatch(TorchNegativeBinomial, torch.Tensor, ll_cache=dict, sampling_ctx=SamplingContext)  # type: ignore[no-redef]
def sample(
    leaf: TorchNegativeBinomial, data: torch.Tensor, ll_cache: Dict[TorchModule, torch.Tensor], sampling_ctx: SamplingContext
) -> torch.Tensor:
    ...
    data[sampling_ids, leaf.scope] = leaf.dist.sample((sampling_ids.sum(),)).squeeze(-1).to(leaf.p_aux.device)
