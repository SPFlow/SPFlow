...
@dispatch(TorchNegativeBinomial, torch.Tensor, ll_cache=dict, instance_ids=list)  # type: ignore[no-redef]
def sample(
    leaf: TorchNegativeBinomial, data: torch.Tensor, ll_cache: Dict, instance_ids: List[int]
) -> torch.Tensor:
    ...
    data[sampling_ids, leaf.scope] = leaf.dist.sample((sampling_ids.sum(),)).squeeze(-1)
