...
def sample(leaf: CondExponential, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None, sampling_ctx: Optional[SamplingContext]=None) -> torch.Tensor:
    r"""Samples from ``CondExponential`` nodes in the ``torch`` backend given potential evidence.
    ...
    sampling_ids = marg_ids & instance_ids_mask.bool().to(l.device)

    data[sampling_ids, leaf.scope.query] = leaf.dist(l=l).sample((sampling_ids.sum(),)).to(l.device)
