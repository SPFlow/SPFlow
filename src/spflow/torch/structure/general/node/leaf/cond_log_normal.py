...
from spflow.base.structure.general.node.leaf.cond_log_normal import (
    CondLogNormal as BaseCondLogNormal,
)
from spflow.tensorly.structure.general.node.leaf.general_cond_log_normal import CondLogNormal as GeneralCondLogNormal
...
class CondLogNormal(LeafNode):
    ...
    def __init__(self, scope: Scope, cond_f: Optional[Callable] = None) -> None:
        ...
        if len(scope.evidence) == 0:
            raise ValueError(f"Evidence scope for 'CondLogNormal' should not be empty.")
    ...
    @classmethod
    def from_signatures(cls, signatures: List[FeatureContext]) -> "CondLogNormal":
...
@dispatch(memoize=True)  # type: ignore
def updateBackend(leaf_node: CondLogNormal, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    return GeneralCondLogNormal(scope=leaf_node.scope, cond_f=cond_f)