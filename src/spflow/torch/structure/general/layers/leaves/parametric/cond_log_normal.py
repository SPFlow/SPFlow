...
from spflow.tensorly.structure.general.layers.leaves.parametric.general_cond_log_normal import CondLogNormalLayer as GeneralCondLogNormalLayer
...
class CondLogNormalLayer(Module):
    ...
    def retrieve_params(self, data: np.ndarray, dispatch_ctx: DispatchContext) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if torch.any(std <= 0.0) or not torch.any(torch.isfinite(std)):
            raise ValueError(f"Value of 'std' for 'CondLogNormalLayer' must be greater than 0, but was: {std}")
...
def updateBackend(leaf_node: CondLogNormalLayer, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralCondLogNormalLayer(scope=leaf_node.scopes_out)