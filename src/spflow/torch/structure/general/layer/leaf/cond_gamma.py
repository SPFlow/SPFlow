...
from spflow.tensorly.structure.general.layer.leaf.general_cond_gamma import CondGammaLayer as GeneralCondGammaLayer
...
class CondGammaLayer(Module):
    r"""Layer of multiple conditional (univariate) Gamma distribution leaf nodes in the ``torch`` backend.
    ...
    Attributes:
        cond_f:
            ...
            a floating point, a list of floats or a one-dimensional NumPy array or PyTorch tensor, containing the shape and rate parameters (greater than 0), respectively.
    ...
    def dist(
        ...
        Args:
            alpha:
                One-dimensional PyTorch tensor representing the shape parameters of all distributions (not just the ones specified by ``node_ids``).
            beta:
    ...
    def retrieve_params(self, data: np.ndarray, dispatch_ctx: DispatchContext) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            Tuple of one-dimensional PyTorch tensor representing the shape and rate parameters, respectively.
...
def marginalize(
    ...
    """Structural marginalization for ``CondGammaLayer`` objects in the ``torch`` backend.
