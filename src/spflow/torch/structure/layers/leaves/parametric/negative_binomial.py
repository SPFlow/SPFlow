...
class NegativeBinomialLayer(Module):
    """Layer representing multiple (univariate) negative binomial leaf nodes in the Torch backend.
    ...
    def dist(self, node_ids: Optional[List[int]]=None) -> D.Distribution:
        ...
        return D.NegativeBinomial(total_count=self.n[node_ids], probs=torch.ones(len(node_ids))-self.p[node_ids])
...
def marginalize(layer: NegativeBinomialLayer, marg_rvs: Iterable[int], prune: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> Union[NegativeBinomialLayer, NegativeBinomial, None]:
    ...
    if len(marginalized_node_ids) == 0:
        ...
        return NegativeBinomial(scope=marginalized_scopes[0], n=layer.n[node_id].item(), p=layer.p[node_id].item())
    else:
        return NegativeBinomialLayer(scope=marginalized_scopes, n=layer.n[marginalized_node_ids].detach(), p=layer.p[marginalized_node_ids].detach())
