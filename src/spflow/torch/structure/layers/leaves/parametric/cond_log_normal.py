...
class CondLogNormalLayer(Module):
    ...
    @classmethod
    def from_signatures(self, signatures: List[Tuple[List[Union[MetaType, FeatureType, Type[FeatureType]]], Scope]]) -> "CondLogNormalLayer":
        """TODO"""
        if not self.accepts(signatures):
            raise ValueError(f"'CondLogNormalLayer' cannot be instantiated from the following signatures: {signatures}.")
        ...
        for types, scope in signatures:
            ...
            if type == MetaType.Continuous or type == FeatureTypes.LogNormal or isinstance(type, FeatureTypes.LogNormal):
                ...
                raise ValueError(f"Unknown signature type {type} for 'CondLogNormalLayer' that was not caught during acception checking.")
