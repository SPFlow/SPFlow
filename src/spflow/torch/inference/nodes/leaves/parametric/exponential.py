...
def log_likelihood(leaf: Exponential, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> torch.Tensor:
    r"""Computes log-likelihoods for ``Exponential`` node in the ``torch`` backend given input data.
    ...
    log_prob: torch.Tensor = torch.empty(batch_size, 1).to(leaf.l.device)
