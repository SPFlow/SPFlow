...
@memoize(TorchGamma)
def log_likelihood(leaf: TorchGamma, data: torch.Tensor, cache: Dict) -> torch.Tensor:
    ...
    if not all(valid_ids):
        raise ValueError(
            f"Encountered data instances that are not in the support of the TorchGamma distribution."
        )
