...
def log_likelihood(layer: ExponentialLayer, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> torch.Tensor:
    r"""Computes log-likelihoods for ``ExponentialLayer`` leaves in the ``torch`` backend given input data.
    ...
    log_prob: torch.Tensor = torch.empty(batch_size, layer.n_out).to(layer.l_aux.device)
