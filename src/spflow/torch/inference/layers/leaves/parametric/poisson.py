...
def log_likelihood(layer: PoissonLayer, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> torch.Tensor:
    r"""Computes log-likelihoods for ``PoissonLayer`` leaves in the ``torch`` backend given input data.
