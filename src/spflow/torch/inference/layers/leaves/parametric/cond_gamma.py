...
def log_likelihood(layer: CondGammaLayer, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> torch.Tensor:
    r"""Computes log-likelihoods for ``CondGammaLayer`` leaves in the ``torch`` backend given input data.
    ...
    for query_signature in np.unique(query_rvs, axis=0):
        ...
        log_prob[torch.meshgrid(non_marg_ids, node_ids_tensor, indexing='ij')] = layer.dist(alpha=alpha, beta=beta, node_ids=node_ids).log_prob(
