...
def log_likelihood(layer: CondLogNormalLayer, data: torch.Tensor, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> torch.Tensor:
    r"""Computes log-likelihoods for ``CondLogNormalLayer`` leaves in the ``torch`` backend given input data.
