"""Contains inference methods for ``CategoricalLayer`` leaves for SPFlow in the ``torch`` backend.
"""
...
def log_likelihood(
    ...
    r"""Computes log-likelihoods for ``CategoricalLayer`` leaves in the ``torch`` backend given input data.

    Log-likelihood for ``CategoricalLayer`` is given by the logarithm of its individual probability mass functions (PMFs):
    ...
    log_prob: torch.Tensor = torch.empty(batch_size, layer.n_out).to(layer.p.device)
