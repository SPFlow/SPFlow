...
def dispatch(*args, memoize=False, substitutable=True) -> Callable:
    ...
    def dispatch_decorator(f):
        ...
        if(substitutable): _f = substitutable_decorator(_f)
        if(memoize): _f = memoize_decorator(_f)

        if f.__name__ in ['log_likelihood'] and not memoize:
            warnings.warn(f"Function '{f.__name__}' was dispatched without memoization, but is recommended to save redundant computations and to correctly compute gradients in certain backends. Ignore this message if this is intended.")
        elif f.__name__ in ['em', 'maximum_likelihood_estimation'] and not memoize:
            warnings.warn(f"Function '{f.__name__}' was dispatched without memoization, but is recommended to not optimize the same module twice in one pass. Ignore this message if this is intended.")
        elif f.__name__ in ['toBase', 'toTorch', 'marginalize'] and not memoize:
            warnings.warn(f"Function '{f.__name__}' was dispatched without memoization, but is recommended to not handle the same module twice in one pass and maintain correct structure. Ignore this message if this is intended.")
