"""Contains inference methods for ``CondCategoricalLayer`` leaves for SPFlow in the ``base`` backend.
"""
...
def log_likelihood(
    ...
    r"""Computes log-likelihoods for ``CondCategoricalLayer`` leaves in the ``base`` backend given input data.
    ...
    k_nodes, p_nodes = layer.retrieve_params(data, dispatch_ctx)

    for node, k, p in zip(layer.nodes, k_nodes, p_nodes):
        dispatch_ctx.update_args(node, {"k":k , "p": p})
