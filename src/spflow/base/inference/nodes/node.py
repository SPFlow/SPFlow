...
def log_likelihood(sum_node: SPNSumNode, data: np.ndarray, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> np.ndarray:
...
@dispatch(memoize=True)  # type: ignore
def log_likelihood(product_node: SPNProductNode, data: np.ndarray, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> np.ndarray:
    ...
    # compute child log-likelihoods
    child_lls = np.concatenate([log_likelihood(child, data, check_support=check_support, dispatch_ctx=dispatch_ctx) for child in product_node.children], axis=1)
