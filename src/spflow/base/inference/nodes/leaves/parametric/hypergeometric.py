...
def log_likelihood(node: Hypergeometric, data: np.ndarray, dispatch_ctx: Optional[DispatchContext]=None) -> np.ndarray:
    """TODO"""
    ...
    probs[~marg_ids] = hypergeom.logpmf(k=data[~marg_ids], M=node.N, n=node.M, N=node.n)
