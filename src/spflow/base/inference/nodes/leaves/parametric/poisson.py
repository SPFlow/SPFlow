...
@dispatch(Poisson, data=np.ndarray)  # type: ignore[no-redef]
def node_likelihood(node: Poisson, data: np.ndarray = None) -> np.ndarray:
...
@dispatch(Poisson, data=np.ndarray)  # type: ignore[no-redef]
def node_log_likelihood(node: Poisson, data: np.ndarray = None) -> np.ndarray:
    probs = np.zeros((data.shape[0], 1))
    data = data[:, node.scope]
    marg_ids = np.isnan(data)
