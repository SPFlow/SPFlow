...
def log_likelihood(layer: CondMultivariateGaussianLayer, data: np.ndarray, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> np.ndarray:
    r"""Computes log-likelihoods for ``CondMultivariateGaussianLayer`` leaves in the ``base`` backend given input data.
