...
from spflow.base.learning.nodes.leaves.parametric.hypergeometric import maximum_likelihood_estimation
from spflow.base.structure.layers.leaves.parametric.hypergeometric import HypergeometricLayer


@dispatch(memoize=True)  # type: ignore
def maximum_likelihood_estimation(layer: HypergeometricLayer, data: np.ndarray, weights: Optional[np.ndarray]=None, bias_correction: bool=True, nan_strategy: Optional[Union[str, Callable]]=None, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None) -> None:
    r"""Maximum (weighted) likelihood estimation (MLE) of ``HypergeometricLayer`` leaves' parameters in the ``base`` backend.
    ...
    Args:
        ...
        bias_corrections:
            ...
            Has no effects for ``HypergeometricLayer`` leaves.
            Defaults to True.
        nan_strategy:
