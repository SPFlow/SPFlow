...
def maximum_likelihood_estimation(leaf: LogNormal, data: np.ndarray, bias_correction: bool=True, nan_strategy: Optional[Union[str, Callable]]=None) -> None:
    """TODO."""
    ...
    mean_est = np.log(scope_data).mean()
    std_est = np.log(scope_data).std(ddof=1 if bias_correction else 0)
