...
def sample(leaf: CondLogNormal, data: np.ndarray, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None, sampling_ctx: Optional[SamplingContext]=None) -> np.ndarray:
    r"""Samples from ``CondLogNormal`` nodes in the ``base`` backend given potential evidence.
    ...
    data[sampling_ids, leaf.scope.query] = leaf.dist(mean=mean, std=std).rvs(size=sampling_ids.sum())
