...
def sample(leaf: CondGeometric, data: np.ndarray, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None, sampling_ctx: Optional[SamplingContext]=None) -> np.ndarray:
    r"""Samples from ``CondGeometric`` nodes in the ``base`` backend given potential evidence.
