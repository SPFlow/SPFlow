...
def sample(layer: CondPoissonLayer, data: np.ndarray, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None, sampling_ctx: Optional[SamplingContext]=None) -> np.ndarray:
    r"""Samples from ``CondPoissonLayer`` leaves in the ``base`` backend given potential evidence.
