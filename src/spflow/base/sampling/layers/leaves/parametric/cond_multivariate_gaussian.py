...
def sample(layer: CondMultivariateGaussianLayer, data: np.ndarray, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None, sampling_ctx: Optional[SamplingContext]=None) -> np.ndarray:
    r"""Samples from ``CondMultivariateGaussianLayer`` leaves in the ``base`` backend given potential evidence.
