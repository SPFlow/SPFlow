...
def sample(layer: LogNormalLayer, data: np.ndarray, check_support: bool=True, dispatch_ctx: Optional[DispatchContext]=None, sampling_ctx: Optional[SamplingContext]=None) -> np.ndarray:
    r"""Samples from ``LogNormalLayer`` leaves in the ``base`` backend given potential evidence.
