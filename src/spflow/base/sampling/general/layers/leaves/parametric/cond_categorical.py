"""Contains sampling methods for ``CondCategoricalLayer`` leaves for SPFlow in the ``base`` backend.
"""
...
def sample(
    ...
    r"""Samples from ``CondCategoricalLayer`` leaves in the ``base`` backend given potential evidence.
    ...
    k_values, p_values = layer.retrieve_params(data, dispatch_ctx)

    for node, k, p in zip(layer.nodes, k_values, p_values):
