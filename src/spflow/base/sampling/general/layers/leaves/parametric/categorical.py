"""Contains sampling methods for ``CategoricalLayer`` leaves for SPFlow in the ``base`` backend.
"""
...
def sample(
    ...
    r"""Samples from ``CategoricalLayer`` leaves in the ``base`` backend given potential evidence.
