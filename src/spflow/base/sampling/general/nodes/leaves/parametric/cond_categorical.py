"""Contains sampling methods for ``CondCategorical`` nodes for SPFlow in the ``base`` backend.
"""
...
def sample(
    ...
    r"""Samples from ``CondCategorical`` nodes in the ``base`` backend given potential evidence.
    ...
    data[sampling_ids, leaf.scope.query] = leaf.dist(p=p).rvs(size=sampling_ids.sum()).argmax(axis=1).reshape(-1)
