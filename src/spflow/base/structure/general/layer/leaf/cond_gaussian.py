...
from spflow.base.structure.general.node.leaf.cond_gaussian import (
    CondGaussian,
)
from spflow.tensorly.structure.spn.layer.leaf import CondGaussianLayer as GeneralCondGaussianLayer
...
class CondGaussianLayer(Module):
    ...
    def accepts(cls, signatures: List[FeatureContext]) -> bool:
        ...
        for signature in signatures:
            if not CondGaussian.accepts([signature]):
                return False
    ...
    def from_signatures(cls, signatures: List[FeatureContext]) -> "CondGaussianLayer":
        ...
        for feature_ctx in signatures:
            ...
            if (
                ...
                raise ValueError(
                    f"Unknown signature type {domain} for 'CondGaussianLayer' that was not caught during acception checking."
                )

            scopes.append(feature_ctx.scope)

        return CondGaussianLayer(scopes)
...
def marginalize(
    ...
    r"""Structural marginalization for ``CondGaussianLayer`` objects in the ``base`` backend.
...
@dispatch(memoize=True)  # type: ignore
def updateBackend(leaf_node: CondGaussianLayer, dispatch_ctx: Optional[DispatchContext] = None):
