...
class CondNegativeBinomial(LeafNode):
    r"""Conditional (univariate) Negative Binomial distribution.
    ...
    def set_params(self, n: int) -> None:
        ...
        if n < 0 or not np.isfinite(n):
            raise ValueError(
                f"Value of n for conditional Negative Binomial distribution must to greater of equal to 0, but was: {n}"
            )

        if not (np.remainder(n, 1.0) == 0.0):
            raise ValueError(
                f"Value of n for conditional Negative Binomial distribution must be (equal to) an integer value, but was: {n}"
            )
