...
from spflow.tensorly.utils.helper_functions import T, tl_unique, tl_unsqueeze, tl_multinomial
...
def sample(
    ...
    for node_id, instances in sampling_ctx.group_output_ids(sum_layer.n_out):
        ...
        for child_id in tl_unique(tl.tensor(child_ids)):
            ...
            child_instance_ids = tl.tensor(instances)[tl.tensor(child_ids) == child_id].tolist()
            child_output_ids = tl_unsqueeze(tl.tensor(output_ids)[tl.tensor(child_ids) == child_id], 1).tolist()
