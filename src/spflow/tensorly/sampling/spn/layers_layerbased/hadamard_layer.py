...
from spflow.tensorly.utils.helper_functions import T, tl_array_split, tl_cartesian_product, tl_tolist, tl_hstack
...
def sample(
    ...
    partition_indices = tl_array_split(
        tl.arange(0, hadamard_layer.n_in),
        tl.cumsum(tl.tensor(hadamard_layer.partition_sizes), axis=0, dtype=int)[:-1],
    )
    ...
    input_ids_per_node = [tl_hstack(id_tuple) for id_tuple in zip(*partition_indices)]
