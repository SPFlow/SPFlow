...
from spflow.tensorly.utils.helper_functions import T, tl_softmax
import torch
import tensorly


def proj_convex_to_real(x: T) -> T:
    """TODO"""
    # convex coefficients are already normalized, so taking the log is sufficient
    return tensorly.log(x)


def proj_real_to_convex(x: T) -> T:
    """TODO"""
    return tl_softmax(x, axis=-1)
