...
class GammaLayer(Module):
    ...
    def __init__(
        ...
        alpha: Union[float, List[float], tl.tensor] = 1.0,
        beta: Union[float, List[float], tl.tensor] = 1.0,
        n_nodes: int = 1,
        **kwargs,
    ) -> None:
    ...
    @property
    def alpha(self) -> tl.tensor:
        """Returns the shape parameters of the represented distributions."""
        return tl.tensor([node.alpha for node in self.nodes])

    @property
    def beta(self) -> tl.tensor:
        """Returns the rate parameters of the represented distributions."""
        return tl.tensor([node.beta for node in self.nodes])
    ...
    def set_params(
        self,
        alpha: Union[int, float, List[float], tl.tensor] = 1.0,
        beta: Union[int, float, List[float], tl.tensor] = 1.0,
    ) -> None:
        ...
        if isinstance(alpha, int) or isinstance(alpha, float):
            alpha = tl.tensor([alpha for _ in range(self.n_out)])
        if isinstance(alpha, list):
            alpha = tl.tensor(alpha)
        if tl.ndim(alpha) != 1:
            raise ValueError(
                f"Numpy array of 'alpha' values for 'GammaLayer' is expected to be one-dimensional, but is {tl.ndim(alpha)}-dimensional."
            )
        ...
        if isinstance(beta, int) or isinstance(beta, float):
            beta = tl.tensor([float(beta) for _ in range(self.n_out)])
        if isinstance(beta, list):
            beta = tl.tensor(beta)
        if tl.ndim(beta) != 1:
            raise ValueError(
                f"Numpy array of 'beta' values for 'GammaLayer' is expected to be one-dimensional, but is {tl.ndim(beta)}-dimensional."
            )
...
def marginalize(
    ...
    if len(marg_scopes) == 0:
        ...
        new_layer = GammaLayer(marg_scopes, *[tl.tensor(p) for p in zip(*marg_params)])
        return new_layer
