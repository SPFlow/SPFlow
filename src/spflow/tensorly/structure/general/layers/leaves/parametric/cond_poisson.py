...
class CondPoissonLayer(Module):
    ...
    def retrieve_params(self, data: tl.tensor, dispatch_ctx: DispatchContext) -> tl.tensor:
        ...
        if l is None:
            ...
            if isinstance(cond_f, List):
                l = tl.tensor([f(data)["l"] for f in cond_f])
            else:
                l = cond_f(data)["l"]
        ...
        if tl.ndim(l) != 1:
            raise ValueError(
                f"Numpy array of 'l' values for 'CondPoissonLayer' is expected to be one-dimensional, but is {tl.ndim(l)}-dimensional."
            )
    ...
    def dist(self, l: tl.tensor, node_ids: Optional[List[int]] = None) -> List[rv_frozen]:
