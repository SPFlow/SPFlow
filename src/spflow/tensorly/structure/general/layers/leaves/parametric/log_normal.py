...
class LogNormalLayer(Module):
    ...
    def set_params(
        ...
        if tl.ndim(mean) != 1:
            raise ValueError(
                f"Numpy array of 'mean' values for 'LogNormalLayer' is expected to be one-dimensional, but is {tl.ndim(mean)}-dimensional."
            )
        ...
        if tl.ndim(std) != 1:
            raise ValueError(
                f"Numpy array of 'std' values for 'LogNormalLayer' is expected to be one-dimensional, but is {tl.ndim(std)}-dimensional."
            )
...
def marginalize(
    ...
    if len(marg_scopes) == 0:
        ...
        new_layer = LogNormalLayer(marg_scopes, *[tl.tensor(p) for p in zip(*marg_params)])
        return new_layer
