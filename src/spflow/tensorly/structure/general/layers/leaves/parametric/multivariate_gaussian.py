...
class MultivariateGaussianLayer(Module):
    ...
    def __init__(
        ...
        mean: Optional[Union[List[float], List[List[float]], List[tl.tensor]]] = None,
        cov: Optional[Union[List[List[float]], List[List[List[float]]], List[tl.tensor]]] = None,
        n_nodes: int = 1,
        **kwargs,
    ) -> None:
        ...
        if mean is None:
            mean = [tl.zeros(len(s.query)) for s in scope]
        if cov is None:
            cov = [tl.eye(len(s.query)) for s in scope]
    ...
    def from_signatures(cls, signatures: List[FeatureContext]) -> "MultivariateGaussianLayer":
        ...
        for feature_ctx in signatures:
            ...
            mean, cov = tl.zeros(len(feature_ctx.scope.query)), tl.eye(len(feature_ctx.scope.query))
    ...
    def set_params(
        self,
        mean: Optional[Union[List[float], tl.tensor, List[List[float]], List[tl.tensor]]] = None,
        cov: Optional[
            Union[
                List[List[float]],
                tl.tensor,
        ...
        for m, c, s in zip(mean, cov, self.scopes_out):
            if tl.ndim(m) != 1:
                raise ValueError(
                    f"All numpy arrays of 'mean' values for 'MultivariateGaussianLayer' are expected to be one-dimensional, but at least one is {tl.ndim(m)}-dimensional."
                )
            ...
            if tl.ndim(c) != 2:
                raise ValueError(
                    f"All numpy arrays of 'cov' values for 'MultivariateGaussianLayer' are expected to be two-dimensional, but at least one is {tl.ndim(c)}-dimensional."
                )
    ...
    def get_params(self) -> Tuple[List[tl.tensor], List[tl.tensor]]:
...
def marginalize(
    ...
    if len(marg_scopes) == 0:
        ...
        new_layer = MultivariateGaussianLayer(marg_scopes, *[tl.tensor(p) for p in zip(*marg_params)])
        return new_layer
