...
class GeometricLayer(Module):
    ...
    def set_params(self, p: Union[int, float, List[float], tl.tensor] = 0.5) -> None:
        ...
        if tl.ndim(p) != 1:
            raise ValueError(
                f"Numpy array of 'p' values for 'GeometricLayer' is expected to be one-dimensional, but is {tl.ndim(p)}-dimensional."
            )
...
def marginalize(
    ...
    if len(marg_scopes) == 0:
        ...
        new_layer = GeometricLayer(marg_scopes, *[tl.tensor(p) for p in zip(*marg_params)])
        return new_layer
