...
class UniformLayer(Module):
    ...
    def __init__(
        ...
        start: Union[int, float, List[float], tl.tensor],
        end: Union[int, float, List[float], tl.tensor],
        support_outside: Union[bool, List[bool], tl.tensor] = True,
        n_nodes: int = 1,
        **kwargs,
    ) -> None:
    ...
    @property
    def start(self) -> tl.tensor:
        """Returns the starts of the intervals of the represented distributions."""
        return tl.tensor([node.start for node in self.nodes])

    @property
    def end(self) -> tl.tensor:
        """Returns the ends of the intervals of the represented distributions."""
        return tl.tensor([node.end for node in self.nodes])
    ...
    @property
    def support_outside(self) -> tl.tensor:
        ...
        return tl.tensor([node.support_outside for node in self.nodes])
    ...
    def set_params(
        ...
        if isinstance(start, int) or isinstance(start, float):
            start = tl.tensor([float(start) for _ in range(self.n_out)])
        if isinstance(start, list):
            start = tl.tensor(start)
        if tl.ndim(start) != 1:
            raise ValueError(
                f"Numpy array of start values for 'UniformLayer' is expected to be one-dimensional, but is {tl.ndim(start)}-dimensional."
            )
        ...
        if isinstance(end, int) or isinstance(end, float):
            end = tl.tensor([float(end) for _ in range(self.n_out)])
        if isinstance(end, list):
            end = tl.tensor(end)
        if tl.ndim(end) != 1:
            raise ValueError(
                f"Numpy array of end values for 'UniformLayer' is expected to be one-dimensional, but is {tl.ndim(end)}-dimensional."
            )
        ...
        if isinstance(support_outside, bool):
            support_outside = tl.tensor([support_outside for _ in range(self.n_out)])
        if isinstance(support_outside, list):
            support_outside = tl.tensor(support_outside)
        if tl.ndim(support_outside) != 1:
            raise ValueError(
                f"Numpy array of 'support_outside' values for 'UniformLayer' is expected to be one-dimensional, but is {tl.ndim(support_outside)}-dimensional."
            )
...
def marginalize(
    ...
    if len(marg_scopes) == 0:
        ...
        new_layer = UniformLayer(marg_scopes, *[tl.tensor(p) for p in zip(*marg_params)])
        return new_layer
