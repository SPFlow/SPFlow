...
class NegativeBinomialLayer(Module):
    ...
    def __init__(
        ...
        p: Union[int, float, List[float], tl.tensor] = 0.5,
        n_nodes: int = 1,
        **kwargs,
    ) -> None:
    ...
    @property
    def p(self) -> tl.tensor:
        """Returns the success probabilities of the represented distributions."""
        return tl.tensor([node.p for node in self.nodes])
    ...
    def set_params(
        ...
        if tl.ndim(n) != 1:
            raise ValueError(
                f"Numpy array of 'n' values for 'NegativeBinomialLayer' is expected to be one-dimensional, but is {tl.ndim(n)}-dimensional."
            )
        ...
        if tl.ndim(p) != 1:
            raise ValueError(
                f"Numpy array of 'p' values for 'NegativeBinomialLayer' is expected to be one-dimensional, but is {tl.ndim(p)}-dimensional."
            )
...
def marginalize(
    ...
    if len(marg_scopes) == 0:
        ...
        new_layer = NegativeBinomialLayer(marg_scopes, *[tl.tensor(p) for p in zip(*marg_params)])
        return new_layer
