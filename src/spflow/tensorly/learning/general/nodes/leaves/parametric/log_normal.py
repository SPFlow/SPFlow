...
def maximum_likelihood_estimation(
    ...
    mean_est = tl.sum(weights * np.log(scope_data)) / n_total

    if bias_correction:
        std_est = tl.sqrt(tl.sum(weights * np.power((np.log(scope_data) - mean_est), 2)) / (n_total - 1))
    else:
        std_est = tl.sqrt(tl.sum(weights * np.power((np.log(scope_data) - mean_est), 2)) / n_total)
