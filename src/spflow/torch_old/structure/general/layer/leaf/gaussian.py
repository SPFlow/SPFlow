...
from spflow.modules.layer import GaussianLayer as GeneralGaussianLayer
...
from spflow.torch.structure.general.node.leaf.gaussian import Gaussian
...
class GaussianLayer(Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for s in scope:
            ...
            if len(s.evidence) != 0:
                raise ValueError(f"Evidence scope for 'GaussianLayer' should be empty, but was {s.evidence}.")
    ...
    def set_params(
        ...
        if torch.any(std <= 0.0) or not torch.any(torch.isfinite(std)):
            raise ValueError(f"Value of 'std' for 'GaussianLayer' must be greater than 0, but was: {std}")
...
@dispatch(memoize=True)  # type: ignore
def toBase(layer: GaussianLayer, dispatch_ctx: Optional[DispatchContext] = None) -> BaseGaussianLayer:
...
def updateBackend(leaf_node: GaussianLayer, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralGaussianLayer(
        scope=leaf_node.scopes_out,
