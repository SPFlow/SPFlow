...
from spflow.structure.general.node.leaf.general_cond_negative_binomial import (
    CondNegativeBinomial as GeneralCondNegativeBinomial,
)
...
class CondNegativeBinomial(LeafNode):
    ...
    def dist(self, p: torch.Tensor) -> D.Distribution:
        ...
        return D.NegativeBinomial(
            total_count=self.n, probs=torch.ones(1, dtype=self.dtype, device=self.device) - p
        )
