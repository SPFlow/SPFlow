...
from spflow.base.structure.general.node.leaf.gaussian import Gaussian
from spflow.base.structure.general.node.leaf.multivariate_gaussian import (
    MultivariateGaussian,
)
from spflow.structure.spn.layer.leaf import MultivariateGaussianLayer as GeneralMultivariateGaussianLayer
...
class MultivariateGaussianLayer(Module):
    ...
    def set_params(
        ...
        for node_mean, node_cov, node in zip(mean, cov, self.nodes):
            ...
            node.set_parameters(node_mean.astype(self.dtype), node_cov.astype(self.dtype))
...
def updateBackend(leaf_node: MultivariateGaussianLayer, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralMultivariateGaussianLayer(
        scope=leaf_node.scopes_out, mean=leaf_node.mean, cov=leaf_node.cov
    )
