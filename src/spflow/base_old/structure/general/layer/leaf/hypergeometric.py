...
from spflow.structure.spn.layer.leaf import HypergeometricLayer as GeneralHypergeometricLayer
...
class HypergeometricLayer(Module):
    ...
    def set_params(
        ...
        for node_N, node_M, node_n, node in zip(N, M, n, self.nodes):
            ...
            node.set_parameters(node_N, node_M, node_n)
...
def updateBackend(leaf_node: HypergeometricLayer, dispatch_ctx: Optional[DispatchContext] = None):
    ...
    """
    dispatch_ctx = init_default_dispatch_context(dispatch_ctx)
    return GeneralHypergeometricLayer(scope=leaf_node.scopes_out, N=leaf_node.N, M=leaf_node.M, n=leaf_node.n)
