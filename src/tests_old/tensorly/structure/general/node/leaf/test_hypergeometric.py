...
def test_initialization_from_signatures(do_for_all_backends):
    ...
    tc.assertTrue(np.isclose(tle.toNumpy(hypergeometric.N), tl.tensor(4)))
    tc.assertTrue(np.isclose(tle.toNumpy(hypergeometric.M), tl.tensor(2)))
    tc.assertTrue(np.isclose(tle.toNumpy(hypergeometric.n), tl.tensor(3)))
