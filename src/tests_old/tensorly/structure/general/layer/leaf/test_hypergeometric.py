...
from spflow.base.structure.general.layer.leaf.hypergeometric import (
    HypergeometricLayer as HypergeometricLayerBase,
)
...
from spflow.torch.structure.general.layer.leaf.hypergeometric import (
    HypergeometricLayer as HypergeometricLayerTorch,
)
from spflow.torch.structure.general.layer.leaf.hypergeometric import updateBackend

from spflow.structure import AutoLeaf
from spflow.structure.general.layer.leaf import HypergeometricLayer
from spflow.structure.general.node.leaf.general_hypergeometric import Hypergeometric
...
def test_layer_initialization(do_for_all_backends):
    ...
    for (
        ...
        tc.assertTrue(np.allclose(tle.toNumpy(M_layer_node), M_value))
        tc.assertTrue(np.allclose(tle.toNumpy(n_layer_node), n_value))
...
def test_layer_structural_marginalization(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(l.M), tle.toNumpy(l_marg.M)))
    tc.assertTrue(np.allclose(tle.toNumpy(l.n), tle.toNumpy(l_marg.n)))
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(l_marg.N), tl.tensor(7)))
    tc.assertTrue(np.allclose(tle.toNumpy(l_marg.M), tl.tensor(6)))
    tc.assertTrue(np.allclose(tle.toNumpy(l_marg.n), tl.tensor(3)))
...
def test_update_backend(do_for_all_backends):
    ...
    hypergeometric = HypergeometricLayer(
        scope=[Scope([0]), Scope([1]), Scope([0])], N=[10, 5, 10], M=[8, 2, 8], n=[3, 4, 3]
    )
...
def test_change_device(do_for_all_backends):
    ...
    # create model on cpu
    torch.set_default_dtype(torch.float32)
    hypergeometric_default = HypergeometricLayer(
        scope=[Scope([0]), Scope([1]), Scope([0])], N=[10, 5, 10], M=[8, 2, 8], n=[3, 4, 3]
    )
    hypergeometric_updated = HypergeometricLayer(
        scope=[Scope([0]), Scope([1]), Scope([0])], N=[10, 5, 10], M=[8, 2, 8], n=[3, 4, 3]
    )
