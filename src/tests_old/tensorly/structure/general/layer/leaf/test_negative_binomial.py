...
from spflow.base.structure.general.layer.leaf.negative_binomial import (
    NegativeBinomialLayer as NegativeBinomialLayerBase,
)
...
from spflow.torch.structure.general.node.leaf.negative_binomial import (
    NegativeBinomial as NegativeBinomialTorch,
)
from spflow.torch.structure.general.layer.leaf.negative_binomial import (
    NegativeBinomialLayer as NegativeBinomialLayerTorch,
)
...
def test_layer_initialization(do_for_all_backends):
    ...
    for n_layer_node, p_layer_node, n_value, p_value in zip(l.n, l.p, n_values, p_values):
        tc.assertTrue(np.allclose(tle.toNumpy(n_layer_node), n_value))
...
def test_initialization_from_signatures(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(negative_binomial.p), np.array([0.5, 0.5])))
...
def test_autoleaf(do_for_all_backends):
    ...
    tc.assertTrue(tl.all(negative_binomial.n == tl.tensor([3, 5])))
    tc.assertTrue(np.allclose(tle.toNumpy(negative_binomial.p), np.array([0.75, 0.25])))
    tc.assertTrue(negative_binomial.scopes_out == [Scope([0]), Scope([1])])
...
def test_layer_structural_marginalization(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(l_marg.n), tl.tensor(2)))
    tc.assertTrue(np.allclose(tle.toNumpy(l_marg.p), tl.tensor(0.29)))
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(l.n), tle.toNumpy(l_marg.n)))
    tc.assertTrue(np.allclose(tle.toNumpy(l.p), tle.toNumpy(l_marg.p)))
...
def test_layer_dist(do_for_all_backends):
    ...
    for n_value, p_value, n_dist, p_dist in zip(n_values, p_values, n_list, p_list):
        tc.assertTrue(np.allclose(tl.tensor(n_value), tle.toNumpy(n_dist)))
        tc.assertTrue(np.allclose(tl.tensor(1 - p_value), tle.toNumpy(p_dist)))
...
def test_update_backend(do_for_all_backends):
    ...
    negativeBinomial = NegativeBinomialLayer(
        scope=[Scope([0]), Scope([1]), Scope([0])], n=[2, 5, 2], p=[0.2, 0.9, 0.31]
    )
...
def test_change_dtype(do_for_all_backends):
    ...
    binomial_default = NegativeBinomialLayer(
    ...
    binomial_updated = NegativeBinomialLayer(
        scope=[Scope([0]), Scope([1]), Scope([0])],
        n=[2, 5, 2],
        p=[0.2, 0.9, 0.31],
    )
