...
from spflow.base.structure.general.layer.leaf.gamma import GammaLayer as GammaLayerBase
...
from spflow.torch.structure.general.layer.leaf.gamma import GammaLayer as GammaLayerTorch
...
def test_layer_structural_marginalization(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(l.alpha), tle.toNumpy(l_marg.alpha)))
    tc.assertTrue(np.allclose(tle.toNumpy(l.beta), tle.toNumpy(l_marg.beta)))
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(l_marg.alpha), tl.tensor(0.29)))
    tc.assertTrue(np.allclose(tle.toNumpy(l_marg.beta), tl.tensor(1.9)))
...
def test_layer_dist(do_for_all_backends):
    ...
    for alpha_value, beta_value, alpha_dist, beta_dist in zip(
        ...
        tc.assertTrue(np.allclose(tl.tensor(alpha_value), tle.toNumpy(alpha_dist)))
        tc.assertTrue(np.allclose(tl.tensor(beta_value), tle.toNumpy(beta_dist)))
...
def test_update_backend(do_for_all_backends):
    ...
    gamma = GammaLayer(
        scope=[Scope([0]), Scope([1]), Scope([0])], alpha=[0.2, 0.9, 0.31], beta=[1.9, 0.3, 0.71]
    )
