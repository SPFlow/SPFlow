...
from spflow.structure.spn import SumLayer
...
def test_sum_layer_initialization(do_for_all_backends):
    ...
    for i in range(3):
        tc.assertTrue(np.allclose(tle.toNumpy(l.weights[i]), weights))
    ...
    l = SumLayer(n_nodes=3, children=input_nodes, weights=weights)
    for i in range(3):
        tc.assertTrue(np.allclose(tle.toNumpy(l.weights[i]), weights[i]))
...
def test_sum_layer_structural_marginalization(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(l.weights), tle.toNumpy(l_marg.weights)))
...
def test_sum_layer_layerbased_conversion(do_for_all_backends):
    ...
    tc.assertTrue(np.allclose(tle.toNumpy(layer_based_sum_layer.weights), tle.toNumpy(sum_layer.weights)))
    tc.assertEqual(layer_based_sum_layer.n_out, sum_layer.n_out)
    node_based_sum_layer = toNodeBased(layer_based_sum_layer)
    tc.assertTrue(np.allclose(tle.toNumpy(node_based_sum_layer.weights), tle.toNumpy(sum_layer.weights)))
    tc.assertEqual(node_based_sum_layer.n_out, sum_layer.n_out)

    node_based_sum_layer2 = toNodeBased(sum_layer)
    tc.assertTrue(np.allclose(tle.toNumpy(node_based_sum_layer2.weights), tle.toNumpy(sum_layer.weights)))
    tc.assertEqual(node_based_sum_layer2.n_out, sum_layer.n_out)
    layer_based_sum_layer2 = toLayerBased(layer_based_sum_layer)
    tc.assertTrue(np.allclose(tle.toNumpy(layer_based_sum_layer2.weights), tle.toNumpy(sum_layer.weights)))
...
def test_update_backend(do_for_all_backends):
    ...
    weights = tle.toNumpy(sum_layer.weights)
    ...
    for backend in backends:
        with tl.backend_context(backend):
            ...
            tc.assertTrue(np.allclose(weights, tle.toNumpy(sum_layer_updated.weights)))
