...
from spflow.structure.spn import UniformLayer
...
def test_update_backend(do_for_all_backends):
    ...
    start = tle.toNumpy(layer.start)
    end = tle.toNumpy(layer.end)

    layer = UniformLayer(scope=[Scope([0]), Scope([1])], start=[0.0, -5.0], end=[1.0, -2.0])
    prod_node = ProductNode([layer])
    expectation_maximization(prod_node, tl.tensor(data), max_steps=10)
    start_em = tle.toNumpy(layer.start)
    end_em = tle.toNumpy(layer.end)
    ...
    for backend in backends:
        with tl.backend_context(backend):
            ...
            start_updated = tle.toNumpy(layer_updated.start)
            end_updated = tle.toNumpy(layer_updated.end)
            ...
            if tl.get_backend() != "pytorch":
                ...
                expectation_maximization(prod_node, tl.tensor(data), max_steps=10)
                start_em_updated = tle.toNumpy(layer_updated.start)
                end_em_updated = tle.toNumpy(layer_updated.end)
