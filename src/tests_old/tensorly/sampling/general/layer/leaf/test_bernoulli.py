...
def test_layer_sampling(do_for_all_backends):
    ...
    layer = BernoulliLayer(scope=[Scope([0]), Scope([1]), Scope([0])], p=[0.2, 0.8, 0.2])

    nodes = [
        Bernoulli(Scope([0]), p=0.2),
        Bernoulli(Scope([1]), p=0.8),
        Bernoulli(Scope([0]), p=0.2),
    ]
...
def test_update_backend(do_for_all_backends):
    ...
    bernoulli = BernoulliLayer(scope=[Scope([0]), Scope([1]), Scope([0])], p=[0.2, 0.8, 0.2])
