...
from nalgene.generate import fix_sentence, generate_from_file
from ba_functions import printmd
...
def means_table(spn):
    ...
    vars = spn.root.moment(2, spn.numFeatures) - spn.root.moment(1,spn.numFeatures) ** 2
    stds = np.sqrt(vars)
    feature_range = [(domain[0], domain[-1]) for domain in spn.domains]
    #print('{:>20}: {:>7} {:>7}'.format('FEATURE', 'MEAN', 'STD'))
    #for i, _ in enumerate(means):
    #    print('{:>20}: {:>7} {:>7} {:>7} {:>7}'.format(
    #        spn.featureNames[i], np.round(means[i], 2), np.round(stds[i], 2), np.round(feature_range[i]), np.round(stds[i]/feature_range[i], 2)))
    header = ['Feature', 'Mean', 'STD', 'range']
    cells = [spn.featureNames, 
             [np.round(means[i], 2) for i in range(spn.numFeatures)],
             [np.round(stds[i], 2) for i in range(spn.numFeatures)],
             ['{:>4} - {:>4}'.format(*np.round(feature_range[i], 2)) for i in range(spn.numFeatures)]]
    plot = p.plot_table(header, cells)
    #plot['layout']['title'] = 'Mean, standard deviation and domain range for all features'
    iplot(plot)
