...
def load_20newsgroups(data_path='data/20ng.pickle',
    ...
    try:
        ...
        for c in categories:
            assert c in set(class_names), "\t\t\tunrecognized category {} (available {})".format(
                c, class_names)
        ...
        logging.info('\t\t\tfor a tot of {} docs over {} total docs'.format(
            len(all_docs), len(dataset.data)))
        ...
        logging.info('\t\t\tadditional filtering  {} docs over {} total docs'.format(
            len(all_pp_documents), len(all_docs)))
        ...
        if continuous:
            ...
            vectorizer = TfidfVectorizer(lowercase=False,
                                         ...
                                         strip_accents='ascii',
                                         max_features=max_features, dtype=_dtype)
        else:
            _dtype = np.int8
            vectorizer = TfidfVectorizer(lowercase=False,
                                         strip_accents='ascii',
                                         ...
                                         binary=True,
                                         use_idf=False, norm=None, dtype=_dtype)
        ...
        trainv_x, test_x, trainv_y, test_y = train_test_split(X, y, test_size=train_valid_test_splits[2],
                                                              random_state=rand_gen)

        logging.info('Reserving {} samples for testing'.format(len(test_x)))

        train_x, valid_x, train_y, valid_y = train_test_split(trainv_x, trainv_y, test_size=train_valid_test_splits[1],
                                                              random_state=rand_gen)
    ...
    print('Loaded dataset with {}/{}/{} train/valid/test splits'.format(len(train_x),
                                                                        len(valid_x),
                                                                        len(test_x)))
...
if __name__ == '__main__':
    ...
    load_20newsgroups(data_path='data/20ng.pickle',
                      continuous=False,
                      min_words=1,
                      max_features=500,
                      categories=None,
