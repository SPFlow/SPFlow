...
from spn.io.Text import to_str_ref_graph, to_str_equation
...
def learn(data, ds_context, min_instances_slice=200, threshold=0.00000001, linear=False):
    split_cols = lambda data, ds_context, scope: split_cols_RDC(data, ds_context, scope, threshold=threshold,
                                                                linear=linear)
...
def get_RL_data():
    ...
    return (
        "FROZEN_LAKE", D, train, test, np.asarray(words), np.asarray(["discrete"] * F), np.asarray(["categorical"] * F))
