...
if __name__ == '__main__':
    ...
    blocked_images = (data[:, :horizontal_middle, :vertical_middle].reshape(len(data), -1),   # top left
                      ...
                      data[:, horizontal_middle:, vertical_middle:].reshape(len(data), -1))     # bottom right
    ...
    samples_placeholder = np.concatenate((np.array([[np.nan] * num_half_image_pixels] * top_left_samples.shape[0]), top_left_samples), axis=1)
    ...
    final_samples[:, :horizontal_middle, :vertical_middle] = top_left_samples.reshape(num_samples, horizontal_middle, vertical_middle)
    final_samples[:, :horizontal_middle, vertical_middle:] = top_right_samples.reshape(num_samples, horizontal_middle, vertical_middle)
