...
def add_parametric_mpe_support():
    ...
    add_node_mpe(Gamma, get_parametric_bottom_up_ll(gamma_likelihood, gamma_mode),
                 get_parametric_top_down_ll(gamma_mode))

    def lognormal_mode(node):
        return np.exp(node.mean - node.variance)

    add_node_mpe(LogNormal, get_parametric_bottom_up_ll(lognormal_likelihood, lognormal_mode),
    ...
    add_node_mpe(Bernoulli, get_parametric_bottom_up_ll(bernoulli_likelihood, bernoulli_mode),
                 get_parametric_top_down_ll(bernoulli_mode))

    def categorical_mode(node):
        return np.argmax(node.p)

    add_node_mpe(Categorical, get_parametric_bottom_up_ll(categorical_likelihood, categorical_mode),
                 get_parametric_top_down_ll(categorical_mode))

    def geometric_mode(node):
        return 1

    add_node_mpe(Geometric, get_parametric_bottom_up_ll(geometric_likelihood, geometric_mode),
    ...
    add_node_mpe(NegativeBinomial, get_parametric_bottom_up_ll(geometric_likelihood, negative_binomial_mode),
                 get_parametric_top_down_ll(negative_binomial_mode))

    def exponential_mode(node):
        return 0

    add_node_mpe(Exponential, get_parametric_bottom_up_ll(exponential_likelihood, exponential_mode),
                 get_parametric_top_down_ll(exponential_mode))

    def hypergeometric_mode(node):
        return np.floor((node.n + 1) * (node.K + 1 / (node.N + 2)))

    add_node_mpe(Hypergeometric, get_parametric_bottom_up_ll(exponential_likelihood, hypergeometric_mode),
    ...
    add_node_mpe(CategoricalDictionary,
