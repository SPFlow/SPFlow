...
def gamma_to_tf_graph(
    ...
    with tf.variable_scope("%s_%s" % (node.__class__.__name__, node.id)):
        alpha = tf.maximum(
            tf.get_variable(
                "alpha",
                initializer=node.alpha,
                dtype=dtype),
            0.001)
        beta = tf.maximum(
            tf.get_variable(
                "beta",
                initializer=node.beta,
                dtype=dtype),
            0.001)
...
def categorical_to_tf_graph(
    ...
    with tf.variable_scope("%s_%s" % (node.__class__.__name__, node.id)):
        ...
        if log_space:
            return tf.distributions.Categorical(probs=probs).log_prob(
                data_placeholder[:, node.scope[0]])

        return tf.distributions.Categorical(probs=probs).prob(
            data_placeholder[:, node.scope[0]])
...
def mvg_to_tf_graph(
    ...
    with tf.variable_scope("%s_%s" % (node.__class__.__name__, node.id)):
        ...
        # Put some line here that checks diagonal entries of cov and raises
        # them
        dist = tf.contrib.distributions.MultivariateNormalTriL(
            loc=mean, scale_tril=sigma)
