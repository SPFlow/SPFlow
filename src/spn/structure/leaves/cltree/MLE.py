...
def compute_log_probs(node, data, alpha):
    ...
    for i in range(node.n_features):
        for j in range(node.n_features):
            ...
            log_j_probs[i,j,0,1] = np.log((cooccurences[j,j] - cooccurences[i,j] + alpha) / (n_samples + 4*alpha))
            log_j_probs[i,j,1,0] = np.log((cooccurences[i,i] - cooccurences[i,j] + alpha) / (n_samples + 4*alpha))
            log_j_probs[i,j,0,0] = np.log((n_samples - cooccurences[j,j] - cooccurences[i,i] + cooccurences[i,j] + alpha)  / (n_samples + 4*alpha))
...
def update_cltree_parameters_mle(node, data, alpha=1.0):
