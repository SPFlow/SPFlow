...
def create_network1():
    ...
    edges1 = net.add_sum_edges(lower_level=leaves, upper_level=sum1, weights=weights1, parameters=parameters)

    prod1 = net.add_product_nodes(2)
    mask1 = np.array([[1,0],
                      [0,1],
                      [1,1]])
    edges2 = net.add_product_edges(sum1, prod1, connections=mask1)

    sum_final = net.add_sum_nodes(1)
    weights_final = np.array([[.3, .7]], dtype='float32').T
    edges_final = net.add_sum_edges(lower_level=prod1, upper_level=sum_final, weights=weights_final,
